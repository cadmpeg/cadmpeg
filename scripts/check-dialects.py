...
"""Check the dialect identity registry, ``crates/cadmpeg-registry/docs/dialects.toml``.
...
``crates/cadmpeg-registry/docs/dialect-support.toml`` and compares it byte for byte.

Capability -- what cadmpeg does with each dialect, including fixture gating
-- lives in ``crates/cadmpeg-registry/docs/dialect-support.toml`` and is checked by the sibling
...
REGISTRY_REL = Path("crates") / "cadmpeg-registry" / "docs" / "dialects.toml"
...
# names come only from crates/cadmpeg-registry/docs/dialects.toml. Each format with dialect rows has
