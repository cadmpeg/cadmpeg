...
# These rows describe bytes rejected before a report exists. They stay in the
# identity and support registries but have no codec-enum variant.
DETECT_UNREACHABLE_IDS = frozenset({"nx:unknown", "sat:unknown"})
...
def render(ids: list[str]) -> str:
    """Render the reportable registry rows one codec enum must cover exactly."""
    lines = [
        "// SPDX-License-Identifier: Apache-2.0",
...
def outputs(root: Path) -> dict[Path, str]:
    ...
    for row in rows:
        ...
        if (
            ...
            and dialect_id in DETECT_UNREACHABLE_IDS
        ):
            continue
