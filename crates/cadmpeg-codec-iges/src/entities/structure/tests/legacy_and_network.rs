...
fn decode_resolves_legacy_text_node_font_pointer() {
    ...
    assert_eq!(
        text.fields().unwrap()["font_definition"],
...
fn decode_preserves_recalculable_dimension_geometry_points() {
    ...
    assert_eq!(
        associativity.fields().unwrap()["geometry"]
            .as_array()
            .unwrap()
            .len(),
        2
    ...
    assert_eq!(
        associativity.fields().unwrap()["geometry"][0]["location_flag"],
        0
    );
    assert_eq!(
    ...
    assert_eq!(
        associativity.fields().unwrap()["geometry"][1]["location_flag"],
        1
    );
    assert_eq!(
        associativity.fields().unwrap()["geometry"][1]["point"][0],
        4.0
    );
    assert!(
        result.report().losses.is_empty(),
...
fn decode_types_fundamental_units_and_property_owner() {
    ...
    assert_eq!(
        units.fields().unwrap()["units"].as_array().unwrap().len(),
        3
    );
...
fn decode_preserves_nested_subfigure_definitions_and_instances() {
    ...
    assert_eq!(
        parent.fields().unwrap()["members"][0],
    ...
    assert_eq!(
        leaf.fields().unwrap()["neutral_links"][0],
        "iges:model:curve#D1"
    );
    assert_eq!(
...
fn decode_omits_occurrence_with_malformed_placement_and_reports_it() {
    ...
    assert_eq!(
        expansion.fields().unwrap()["issues"][0],
        "malformed_placement"
    );
    let loss = result
        .report()
...
fn decode_does_not_infer_roots_from_malformed_definition_members() {
    ...
    let dangling = native.arenas()["entities"]
        ...
        .unwrap();
    assert_eq!(
        dangling.fields().unwrap()["references"][0]["resolution"],
        "dangling"
    );
...
fn container_only_preserves_raw_occurrence_expansion_without_structure_admission() {
    ...
    assert!(expansion.fields().unwrap()["issues"]
        .as_array()
        .unwrap()
        .is_empty());
    assert!(!result
...
fn decode_preserves_network_definition_and_anisotropic_instance() {
    ...
    assert_eq!(
        definition.fields().unwrap()["declared_connect_point_count"],
        2
    );
    assert_eq!(
...
fn decode_preserves_owned_network_connect_points() {
    ...
    assert_eq!(
        points[0].fields().unwrap()["owner"],
        "iges:entity:directory#3"
    );
    assert_eq!(points[1].fields().unwrap()["position"][2], 3.0);
    assert_eq!(
        points[1].fields().unwrap()["owner"],
        "iges:entity:directory#7"
    );
    assert!(
        result.report().losses.is_empty(),
...
fn decode_reports_occurrence_issue_for_rejected_subfigure_definition() {
    ...
    assert!(native.arenas()["product_occurrences"].is_empty());
    assert_eq!(
        native.arenas()["product_occurrence_expansion"][0]
            .fields()
            .unwrap()["issues"],
        serde_json::json!(["malformed_placement"])
    );
