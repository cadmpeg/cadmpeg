...
fn generated_global_fields(bytes: &[u8]) -> Vec<String> {
    let scan = crate::card::scan(bytes).expect("generated bytes have valid cards");
    let stream = scan
        .lines
        .iter()
        .filter(|line| matches!(line.section, Some(crate::card::Section::Global)))
        .flat_map(|line| line.payload.iter().take(72).copied())
        .collect::<Vec<_>>();
    let global = crate::global::parse(&scan).expect("generated bytes have valid Global data");
    global
        .value_spans
        .iter()
        .map(|span| String::from_utf8(stream[span.clone()].to_vec()).expect("Global is ASCII"))
        .collect()
}
...
#[test]
fn encode_computes_global_metadata_and_defaults_unavailable_identity() {
    let mut ir = CadIr::empty(Units::default());
    ir.model.points.push(Point {
        id: PointId("point#global-metadata".into()),
        source_object: None,
        position: Point3::new(4.0, 5.0, 6.0),
    });
    let plan = IgesEncoder::default()
        ...
        .expect("point can be written");
    let mut written = Vec::new();
    plan.write_to(&mut written).expect("point write succeeds");
    let fields = generated_global_fields(&written);

    assert_eq!(fields.len(), 26);
    assert_eq!(fields[0], "1H,");
    assert_eq!(fields[1], "1H;");
    assert_eq!(fields[2], "7Hcadmpeg");
    assert_eq!(fields[3], "13Hgenerated.igs");
    assert_eq!(fields[4], "7Hcadmpeg");
    assert_eq!(
        fields[5],
        format!(
            "{}H{}",
    ...
    assert_eq!(&fields[6..=10], ["32", "38", "6", "308", "17"]);
    assert_eq!(fields[11], "0H");
    assert_eq!(&fields[12..=16], ["1.0", "2", "2HMM", "1", "1.0"]);
    assert!(fields[17].starts_with("15H"));
    assert_eq!(fields[17].len(), 18);
    assert_eq!(
        fields[18],
        crate::writer::number(cadmpeg_ir::units::COINCIDENCE_TOLERANCE)
    );
    assert_eq!(fields[19], crate::writer::number(6.0));
    assert_eq!(&fields[20..], ["0H", "0H", "11", "0", "0H", "0H"]);
}
