...
fn encode_does_not_attempt_replay_when_the_source_records_no_dialect() {
    ...
    let source = unclassified.source.take().expect("decoded source exists");
    ...
    unclassified.source = Some(
        ...
        .expect("unclassified source deserializes"),
    );
    let plan = IgesCodec
        .plan(
...
fn a_replayed_export_states_the_preserved_dialect_as_its_target() {
    ...
    let report = plan.write_to(&mut written).expect("replay writes");
    assert_eq!(
...
fn a_synthesized_export_states_the_target_it_wrote() {
    for (version, id) in [
        ...
        let plan = IgesCodec
            ...
            .expect("explicit target plans");
        let mut written = Vec::new();
        let report = plan.write_to(&mut written).expect("synthesis writes");
        assert_eq!(
            wire::field_or_default::<Option<cadmpeg_core::dialect::DialectId>>(
                &(report),
                "identity/target"
