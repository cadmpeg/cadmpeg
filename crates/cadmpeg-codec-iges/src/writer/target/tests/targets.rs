...
fn inherit_replays_a_non_default_version_verbatim() {
    ...
    let plan =
        inherit(decoded.ir(), Some(decoded.source_fidelity())).expect("inherited target plans");
    ...
    plan.write_to(&mut written).expect("replay writes");
...
fn inherit_synthesizes_the_source_version_when_the_image_is_gone() {
    let decoded = IgesCodec
        ...
        .expect("point fixture decodes");
    let plan = inherit(decoded.ir(), None).expect("inherited target plans");
...
fn an_explicit_target_writes_a_source_the_catalog_cannot_inherit() {
    let decoded = IgesCodec
        ...
        .expect("point fixture decodes");
    let plan = plan_at(IgesVersion::V5_3, decoded.ir(), None).expect("explicit target plans");
...
fn every_synthesized_target_re_decodes_as_the_dialect_the_report_named() {
    ...
    for version in IgesVersion::ALL {
        ...
        let mut written = Vec::new();
        plan.write_to(&mut written).expect("synthesis writes");
