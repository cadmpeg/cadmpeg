...
fn specified_parameter_end(
    ...
    match (entry.entity_type, entry.form) {
        ...
        (402, 1 | 7 | 14 | 15) => {
            let Some(member_count) = record.count(1) else {
                return ParameterBoundary::Invalid;
            };
            let Some(end) = member_count.checked_add(2) else {
                return ParameterBoundary::Invalid;
            };
            if end <= record.tokens.len() {
                ParameterBoundary::Known(end)
        ...
        (308, 0) => {
            ...
            let Some(end) = member_count.checked_add(4) else {
                return ParameterBoundary::Invalid;
            };
            if end <= record.tokens.len() {
                ParameterBoundary::Known(end)
        ...
        (406, 5) => {
            ...
            if end <= record.tokens.len() {
                ParameterBoundary::Known(end)
            } else if record.tokens.len() >= 2 {
                ParameterBoundary::Known(record.tokens.len())
            } else {
                ParameterBoundary::Invalid
            }
        }
