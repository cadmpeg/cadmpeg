...
fn dimension_transfer_rejects_duplicate_owner_feature_ids() {
    ...
    assert_eq!(
        ...
        BTreeSet::from([ParameterId::mint("creo:featdefs:parameter#917:3".to_string()).expect("identity grammar")])
    );

    let mut ir = CadIr::empty();
    for ordinal in 0..2 {
        ir.model.features.push(Feature::new(
            cadmpeg_ir::features::FeatureId::mint("creo:model:feature#40").expect("identity grammar"),
            ordinal,
