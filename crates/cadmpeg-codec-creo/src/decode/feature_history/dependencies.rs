...
pub(in super::super) fn feature_dependencies(
    ...
    .filter_map(|dependency| {
        let id = IrFeatureId::mint(format!("creo:model:feature#{dependency}")).expect("identity grammar");
        ir.model
            .features
            .iter()
...
pub(in super::super) fn reconcile_feature_links(
    ...
    for feature in &mut ir.model.features {
        ...
        .into_iter()
        .map(|dependency| IrFeatureId::mint(format!("creo:model:feature#{dependency}")).expect("identity grammar"))
        ...
        let parent = current_feature_recipe_parent(&scan.features.operations, feature_id)
            .map(|parent| IrFeatureId::mint(format!("creo:model:feature#{parent}")).expect("identity grammar"))
