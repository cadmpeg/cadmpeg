...
/// Every `id = "step:…"` value in `docs/dialects.toml`.
fn registry_ids() -> BTreeSet<String> {
    ...
    let ids = text
        ...
        .map(str::to_owned)
        .collect::<BTreeSet<_>>();
    assert!(!ids.is_empty(), "the registry declares no step rows");
    ids
}
...
fn each_declaration_classifies_into_the_row_its_discriminants_match() {
    for case in CASES {
        ...
        assert_eq!(
            matched.declared[DECLARED_IMPLEMENTATION_LEVEL], "2;1",
    ...
    for case in CASES {
        ...
        if !admitted {
            assert_eq!(
                ...
                "FILE_SCHEMA {:?}: the nearest row names the strategy actually used",
                case.identifiers
            );
        }
    }
