...
impl BlendBoundState {
    pub(crate) fn new(
        ...
        if first != 1 {
            return Err("header_references: first reference must be one");
        }
        ...
        Ok(Self {
            ...
            surface: surface
                .try_into()
                .map_err(|_| "blend_surface_xmt: must exceed one")?,
        })
    }
    ...
    pub(crate) fn blend_surface(self) -> u32 {
        self.surface.into()
    }
}
...
impl From<BlendBoundState> for StateWire {
    fn from(state: BlendBoundState) -> Self {
        Self {
            xmt: state.xmt(),
            header_references: state.header_references(),
            sense: state.sense(),
            boundary_index: state.boundary_index(),
            blend_surface_xmt: state.blend_surface(),
        }
    }
}
impl TryFrom<StateWire> for BlendBoundState {
    ...
    fn try_from(wire: StateWire) -> Result<Self, Self::Error> {
        Self::new(
            wire.xmt,
            wire.header_references,
            wire.sense,
            wire.boundary_index,
            wire.blend_surface_xmt,
        )
    }
}
...
mod tests {
    ...
    fn bridge_wire_preserves_both_boundaries_and_rejects_invalid_controls() {
        for boundary in [0, 1] {
            let json = format!(
                r#"{{"xmt":2,"header_references":[1,0,3,4,5],"sense":false,"boundary_index":{boundary},"blend_surface_xmt":6}}"#
            );
            let state: BlendBoundState = serde_json::from_str(&json).unwrap();
            assert_eq!(serde_json::to_string(&state).unwrap(), json);
            for (field, invalid) in [
                ("xmt", serde_json::json!(0)),
                ("xmt", serde_json::json!(1)),
                ("header_references", serde_json::json!([0, 0, 3, 4, 5])),
                ("boundary_index", serde_json::json!(2)),
                ("blend_surface_xmt", serde_json::json!(0)),
                ("blend_surface_xmt", serde_json::json!(1)),
            ] {
