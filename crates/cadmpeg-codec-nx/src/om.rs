...
use discriminators::{
    DraftBinary32Branch, OperationStatePairTag,
};
...
fn operation_state_status_row_at<'a>(
    ...
    let status_code =
        OperationStateIndex::read_at(bytes, at, base_offset)?.token()?;
...
fn operation_state_group_row_at(
    ...
    match tag {
        0x4a => {
            ...
            let position = OperationStateIndex::read_at(bytes, position_at, base_offset)?.token()?;
...
fn audit_trail_row_at(
    ...
    AuditTrailRow::new(base_offset, at, AuditRecord {
        ordinal, frame_selector, timestamp, value,
    })
}
