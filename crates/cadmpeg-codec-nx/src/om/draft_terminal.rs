...
const FIXED: [u8; 11] = [0x01, 0x03, 0x02, 0x01, 0x02, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00];
...
impl<O: Copy + Add<Output = O> + From<u16>> DraftTerminalLane<O> {
    ...
    pub(crate) fn indices(&self) -> [LocatedCompactIndex<O, ExtendedCompactIndex>; 2] {
        [LocatedCompactIndex { atom: self.indices[0], offset: self.offset },
         LocatedCompactIndex { atom: self.indices[1], offset: self.offset + O::from(2) }]
    }
    pub(crate) fn tail(&self) -> [u8; 3] { self.tail }
}

macro_rules! checked_origin {
    ($offset:ty) => {
        impl DraftTerminalLane<$offset> {
            pub(crate) fn new(indices: [ExtendedCompactIndex; 2], tail: [u8; 3], offset: $offset) -> Option<Self> {
                offset.checked_add(<$offset>::from(BYTE_LEN))?;
                Some(Self { offset, indices, tail })
            }
        }
    };
}
...
impl DraftTerminalLane<usize> {
    pub(crate) fn into_absolute(self, base: u64) -> Option<DraftTerminalLane<u64>> {
        DraftTerminalLane::<u64>::new(self.indices, self.tail, base.checked_add(self.offset as u64)?)
    }
}

pub(crate) fn scan(record: OperationPayload<'_>) -> Option<DraftTerminalLane> {
    if record.name() != "DRAFT" { return None; }
    ...
    DraftTerminalLane::<usize>::new([first, second], tail, record.payload_offset().checked_add(start)?)
}
...
mod tests {
    ...
    fn terminal_frame_requires_room_for_indices_fixed_bytes_and_tail() {
        let indices = [ExtendedCompactIndex::read(&[0x80, 1]).unwrap(), ExtendedCompactIndex::read(&[0x81, 2]).unwrap()];
        let last_origin = u64::MAX - 19;
        let lane = DraftTerminalLane::<u64>::new(indices, [1, 2, 3], last_origin).unwrap();
        assert_eq!(lane.indices().map(|token| token.offset), [last_origin, last_origin + 2]);
