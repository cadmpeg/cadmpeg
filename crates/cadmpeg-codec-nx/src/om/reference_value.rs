...
impl Tagged28 {
    pub(crate) fn from_word(word: u32) -> Self { Self(word & 0x0fff_ffff) }
}

impl TryFrom<u32> for Tagged28 {
    type Error = &'static str;
    fn try_from(value: u32) -> Result<Self, Self::Error> {
        if value <= 0x0fff_ffff { Ok(Self(value)) }
        else { Err("value exceeds the tagged28 range") }
    }
}

impl From<Tagged28> for u32 {
    fn from(value: Tagged28) -> Self { value.0 }
}
