...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum LeadingBytes { One(u8), Two([u8; 2]), Three([u8; 3]) }
...
impl ControlLeadingValue {
    ...
    pub(crate) fn from_wire(width: u8, value: u32) -> Result<Self, &'static str> {
        ...
        if result.value() != value { return Err("leading_value: exceeds leading_value_width"); }
        Ok(result)
    }

    pub(crate) fn width(self) -> u8 {
        match self.0 { LeadingBytes::One(_) => 1, LeadingBytes::Two(_) => 2, LeadingBytes::Three(_) => 3 }
    }
    ...
    pub(crate) fn value(self) -> u32 {
        match self.0 {
            ...
            LeadingBytes::Three([low, middle, high]) => u32::from(low) | (u32::from(middle) << 8) | (u32::from(high) << 16),
        }
    }
}
...
mod tests {
    ...
    fn leading_values_retain_width_even_when_zero() {
        ...
        for width in [0, 4, 255] { assert!(ControlLeadingValue::from_wire(width, 0).is_err()); }
