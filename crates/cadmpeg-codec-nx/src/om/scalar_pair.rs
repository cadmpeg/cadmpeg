...
impl SketchPairForm {
    ...
    pub(crate) fn discriminator(self) -> &'static [u8] {
        ...
        const EXTENDED: [u8; 17] = [
            0x08, 0x02, 0x03, 0x01, 0xc0, 0x40, 0x02, 0x01, 0xc0, 0x45, 0x04, 0x00, 0x80, 0x86,
            0x02, 0x00, 0x01,
        ];
...
impl DatumPairForm {
    ...
    pub(crate) fn discriminator(self) -> &'static [u8] {
        ...
        match self {
            Self::Initial => DISCRIMINATORS[0],
            Self::Continuation => DISCRIMINATORS[1],
        }
    }
}
...
impl<F: PairForm> PairPosition<F> {
    ...
    pub(crate) fn from_wire(
        ...
        let form = F::ALL_FORMS
            .iter()
            .copied()
            .find(|form| form.prefix() == discriminator)
            .ok_or("discriminator: unsupported scalar-pair framing")?;
        let position =
            Self::new(form, offset).ok_or("payload_offset: scalar-pair offsets overflow")?;
