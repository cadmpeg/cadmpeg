...
impl<S: AsRef<str>> StateMessageText<S> {
    ...
    pub(crate) fn declared_length(&self) -> u8 { self.0.as_str().len() as u8 + 2 }
...
impl<'de> Deserialize<'de> for StateMessageText<String> {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        if wire.declared_length != text.declared_length() {
            return Err(serde::de::Error::custom("declared_length: disagrees with text length plus two"));
        }
        Ok(text)
    }
}
...
mod tests {
    ...
    fn message_text_derives_length_and_preserves_spaces() {
        for text in [" ".to_string(), "x".repeat(253)] {
            ...
            let json = format!(r#"{{"declared_length":{},"text":{}}}"#, text.len() + 2, serde_json::to_string(&text).unwrap());
            assert_eq!(serde_json::to_string(&value).unwrap(), json);
            assert_eq!(serde_json::from_str::<StateMessageText<String>>(&json).unwrap(), value);
        }
    }

    #[test]
    fn message_text_rejects_invalid_bytes_lengths_and_duplicate_count() {
        for text in ["".to_string(), "\n".to_string(), "μ".to_string(), "x".repeat(254)] {
