...
impl CommonFramePrefix {
    ...
    fn index_offsets(self) -> [usize; 3] {
        [
            0,
            self.0[0].raw().len(),
            self.0[0].raw().len() + self.0[1].raw().len(),
        ]
    }
}
...
impl CommonFrameSuffix {
    pub(crate) fn from_wire(
        local_ordinal: u32,
        raw_local_ordinal: &[u8],
        object_index: Option<u32>,
        raw_object_index: &[u8],
    ) -> Result<Self, &'static str> {
        let local_ordinal =
            CanonicalFeatureReferenceToken::from_wire(local_ordinal, raw_local_ordinal)
        ...
        let object = match object_index {
            ...
            Some(value) => Some(
                CanonicalFeatureReferenceToken::from_wire(value, raw_object_index)
    ...
    pub(crate) fn with_target<T>(
        self,
        target: Option<T>,
    ) -> Result<CommonFrameSuffix<Option<T>>, &'static str> {
        if self.object.is_none() && target.is_some() {
            return Err("data_block requires a non-null object_index");
        }
...
impl<T> CommonFrameSuffix<T> {
    pub(crate) fn map_target<U>(self, map: impl FnOnce(u32, T) -> U) -> CommonFrameSuffix<U> {
        CommonFrameSuffix {
            ...
            object: self
                .object
                .map(|(token, target)| (token, map(token.value(), target))),
        }
    }
    pub(crate) fn target(&self) -> Option<&T> {
        self.object.as_ref().map(|(_, target)| target)
    }
    pub(crate) fn local_ordinal(&self) -> u32 {
        self.local_ordinal.value()
    }
    pub(crate) fn raw_local_ordinal(&self) -> &[u8] {
        self.local_ordinal.raw()
    }
    pub(crate) fn object_index(&self) -> Option<u32> {
        self.object.as_ref().map(|(token, _)| token.value())
    }
    pub(crate) fn raw_object_index(&self) -> &[u8] {
        self.object
            .as_ref()
            .map_or(&[0xff], |(token, _)| token.raw())
    }
...
impl<O, T> CommonFrame<O, T> {
    pub(crate) fn prefix(&self) -> CommonFramePrefix {
        self.prefix
    }
    pub(crate) fn state(&self) -> [u8; 8] {
        self.state
    }
    pub(crate) fn suffix(&self) -> &CommonFrameSuffix<T> {
        &self.suffix
    }
    pub(crate) fn byte_len(&self) -> usize {
        self.prefix.byte_len() + 8 + self.suffix.byte_len()
    }
    pub(crate) fn legacy_inactive_modules(&self) -> Option<bool> {
        boolean(self.state[3])
    }
    pub(crate) fn modifies_parasolid_data(&self) -> Option<bool> {
        boolean(self.state[4])
    }
    pub(crate) fn split_tracking_data(&self) -> [u8; 2] {
        [self.state[5], self.state[6]]
    }
    pub(crate) fn group_count(&self) -> u8 {
        self.state[7]
    }
}
...
mod tests {
    ...
    fn prefix_preserves_compact_grammar_and_rejects_marker_width_conflicts() {
        ...
        assert_eq!(
            prefix.raw_indices(),
            [vec![0], vec![0x90, 1], vec![0x80, 0]]
        );
        assert!(
            CommonFramePrefix::from_wire([0, 4097, 0], &prefix.raw_indices(), [1, 1, 1]).is_err()
        );
        assert!(CommonFramePrefix::read(&[0, 0xff, 1, 0x80, 0, 1, 3, 2], [1, 3, 2]).is_none());
        assert!(
            CommonFramePrefix::from_wire([0; 3], &std::array::from_fn(|_| vec![0]), [1, 3, 2])
                .is_err()
        );
    }
    ...
    fn operation_common_frame_types_the_parasolid_modification_field() {
        let mut state = [0; 8];
        assert_eq!(
            common_frame_with_state(state).modifies_parasolid_data(),
            Some(false)
        );
        state[4] = 1;
        assert_eq!(
    ...
    fn operation_common_frame_types_the_legacy_inactive_modules_field() {
        let mut state = [0; 8];
        assert_eq!(
            common_frame_with_state(state).legacy_inactive_modules(),
            Some(false)
        );
        state[3] = 1;
        assert_eq!(
    ...
    fn common_frame_with_state(state: [u8; 8]) -> crate::om::common_frame::CommonFrame<u64> {
        crate::om::common_frame::CommonFrame::<u64>::new(
            ...
            .unwrap(),
            state,
            crate::om::common_frame::CommonFrameSuffix::from_wire(0, &[0], None, &[0xff]).unwrap(),
            0,
        )
        .unwrap()
    }
