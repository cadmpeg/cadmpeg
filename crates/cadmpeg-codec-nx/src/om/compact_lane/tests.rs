    ...
    for anchor_raw in [&[1][..], &[128, 1][..]] {
        for member_raw in [&[2][..], &[128, 2][..]] {
            for count in [3_u8, 255] {
                ...
                assert_eq!(lane.members().map(|index| index.offset).collect::<Vec<_>>(), offsets);
                ...
                assert_eq!(absolute.members().map(|index| index.offset).collect::<Vec<_>>(), offsets.iter().map(|offset| base + *offset as u64).collect::<Vec<_>>());
                ...
                assert!(lane.try_resolve(|atom| (atom.value() != 2).then_some(())).is_none());
            }
        }
    }
}
...
fn abr_lane_positions_include_null_and_extended_widths() {
    for raw in [&[0xff][..], &[2][..], &[128, 2][..]] {
        ...
        let base = u64::MAX - bytes.len() as u64;
        let absolute = lane.clone().into_absolute(base).unwrap();
        assert_eq!(absolute.slots().map(|slot| slot.offset), offsets.map(|offset| base + offset as u64));
        assert!(lane.clone().into_absolute(base + 1).is_none());
        let resolved = lane.clone().try_resolve(|atom| Some(atom.value())).unwrap();
        assert!(resolved.slots().iter().all(|slot| slot.atom.map(|index| index.target) == if raw == [0xff] { None } else { Some(2) }));
