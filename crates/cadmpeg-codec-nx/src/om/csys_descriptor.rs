...
impl From<CsysIdentity> for String {
    fn from(value: CsysIdentity) -> Self { value.0 }
}
...
fn is_identity_byte(byte: u8) -> bool { byte.is_ascii_digit() || (b'a'..=b'f').contains(&byte) }
...
impl CsysDescriptor {
    pub(crate) fn read(bytes: &[u8]) -> Option<Self> {
        ...
        let mut at = 0;
        while at < bytes.len() {
            if !is_identity_byte(bytes[at]) { at += 1; continue; }
            let start = at;
            while at < bytes.len() && is_identity_byte(bytes[at]) { at += 1; }
    ...
    pub(crate) fn from_wire(prefix: Vec<u8>, identity: CsysIdentity, suffix: Vec<u8>) -> Result<Self, &'static str> {
        let mut bytes = prefix.clone();
        bytes.extend_from_slice(identity.as_str().as_bytes());
        bytes.extend_from_slice(&suffix);
        let parsed = Self::read(&bytes).ok_or("prefix/identity/suffix must contain one maximal descriptor identity")?;
    ...
    pub(crate) fn identity(&self) -> &CsysIdentity { &self.identity }
    pub(crate) fn suffix(&self) -> &[u8] { &self.suffix }
}
...
impl LocatedCsysDescriptor {
    pub(crate) fn new(descriptor: CsysDescriptor, source_offset: u64) -> Result<Self, &'static str> {
        source_offset.checked_add(descriptor.prefix.len() as u64)
            .ok_or("source_offset overflows identity_source_offset")?;
        Ok(Self { descriptor, source_offset })
    }
    pub(crate) fn descriptor(&self) -> &CsysDescriptor { &self.descriptor }
    pub(crate) fn source_offset(&self) -> u64 { self.source_offset }
    pub(crate) fn identity_source_offset(&self) -> u64 { self.source_offset + self.descriptor.prefix.len() as u64 }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(try_from = "u8", into = "u8")]
#[repr(u8)]
pub(crate) enum CsysDescriptorSlot { Five = 5, Six = 6, Seven = 7 }
...
impl From<CsysDescriptorSlot> for u8 {
    fn from(value: CsysDescriptorSlot) -> Self { value as Self }
}
...
mod tests {
    ...
    fn identity_lengths_and_maximal_runs_are_checked_at_construction() {
        for len in 29..=33 {
            ...
            assert_eq!(CsysIdentity::try_from(value).is_ok(), (30..=32).contains(&len));
        }
        ...
        assert!(CsysDescriptor::from_wire(vec![0], identity.clone(), b"?bbbbbbbbbbbbbbbbbbbbbbbbbbbbbb".to_vec()).is_err());
        let descriptor = CsysDescriptor::from_wire(vec![2, 1], identity, vec![b'?']).unwrap();
        assert_eq!(LocatedCsysDescriptor::new(descriptor.clone(), u64::MAX - 2).unwrap().identity_source_offset(), u64::MAX);
        assert!(LocatedCsysDescriptor::new(descriptor, u64::MAX - 1).is_err());
        for slot in 0..=u8::MAX {
            assert_eq!(CsysDescriptorSlot::try_from(slot).is_ok(), (5..=7).contains(&slot));
        }
    }
}
