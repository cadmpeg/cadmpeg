...
impl PlaneDescriptor {
    ...
    pub(crate) fn from_wire(identity: String, suffix: &[u8], schema_index: u32, label: &str) -> Result<Self, &'static str> {
        let mut bytes = identity.as_bytes().to_vec();
        bytes.extend_from_slice(suffix);
        let descriptor = Self::read(&bytes).ok_or("identity/suffix must form a forty-byte datum-plane descriptor")?;
        if descriptor.identity() != identity { return Err("identity disagrees with descriptor delimiter"); }
        if descriptor.schema_index() != schema_index { return Err("schema_index disagrees with suffix"); }
        if descriptor.label() != label { return Err("label disagrees with suffix"); }
        Ok(descriptor)
    }

    pub(crate) fn identity(&self) -> &str { &self.identity }
    pub(crate) fn schema_index(&self) -> u32 { self.schema.value() }
...
mod tests {
    ...
    fn descriptor_retains_exact_schema_token_width() {
        for (schema, label) in [(&[0][..], "abcd"), (&[0x80, 0][..], "abc")] {
            ...
            assert!(PlaneDescriptor::from_wire(descriptor.identity().into(), &descriptor.suffix(), 1, label).is_err());
            assert!(PlaneDescriptor::from_wire(descriptor.identity().into(), &descriptor.suffix(), 0, "other").is_err());
            bytes.pop();
