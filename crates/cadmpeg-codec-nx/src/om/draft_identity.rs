...
enum Prefix {
    ...
    Tagged { index: Option<CompactIndexAtom> },
}

impl Prefix {
    fn read(bytes: &[u8]) -> Option<Self> {
        if bytes.first() != Some(&0x41) { return None; }
        if bytes.get(1) == Some(&0xf0) {
            ...
            if bytes.get(at) != Some(&0xf0) { return None; }
            ...
            (bytes.get(at + 1) == Some(&0x01))
                .then_some(Self::IndexedBranch { first, second: second.atom, branch })
        }
    }

    fn raw(&self) -> Vec<u8> {
        fn nullable(atom: &Option<CompactIndexAtom>) -> &[u8] { atom.as_ref().map_or(&[0xff], CompactIndexAtom::raw) }
        match self {
            Self::IndexedBranch { first, second, branch } => {
    ...
    fn byte_len(&self) -> usize {
        let nullable_len = |atom: &Option<CompactIndexAtom>| atom.as_ref().map_or(1, |atom| atom.raw().len());
        match self {
            Self::IndexedBranch { first, second, .. } => 4 + first.raw().len() + nullable_len(second),
    ...
    fn form(&self) -> DraftIdentityForm {
        match self {
            Self::IndexedBranch { first, second, branch } => DraftIdentityForm::IndexedBranch {
                first_index: first.value(), second_index: second.map(CompactIndexAtom::value), branch: *branch,
            },
            Self::Tagged { index } => DraftIdentityForm::Tagged { index: index.map(CompactIndexAtom::value) },
        }
    }
}
...
impl DraftIdentityFrame {
    pub(crate) fn read(bytes: &[u8], offset: usize) -> Option<Self> {
        ...
        let len = tail.iter().take_while(|byte| byte.is_ascii_digit() || (b'a'..=b'f').contains(byte)).count();
        if len == 0 || tail.get(len) != Some(&b'?') { return None; }
        let identity = tail[..len].iter().copied().map(char::from).collect();
        Some(Self { prefix, identity, offset: offset as u64 })
    }

    pub(crate) fn from_wire(prefix: &[u8], form: DraftIdentityForm, identity: String, offset: u64) -> Result<Self, &'static str> {
        let parsed = Prefix::read(prefix).filter(|parsed| parsed.byte_len() == prefix.len())
            .ok_or("prefix is not a complete draft identity prefix")?;
        if parsed.form() != form { return Err("form disagrees with prefix"); }
        if identity.is_empty() || !identity.bytes().all(|byte| byte.is_ascii_digit() || (b'a'..=b'f').contains(&byte)) {
            return Err("identity must contain nonempty lowercase hexadecimal digits");
        }
        offset.checked_add(parsed.byte_len() as u64).ok_or("payload_offset overflows identity_payload_offset")?;
        Ok(Self { prefix: parsed, identity, offset })
    }

    pub(crate) fn offset(&self) -> u64 { self.offset }
    pub(crate) fn prefix(&self) -> Vec<u8> { self.prefix.raw() }
    pub(crate) fn form(&self) -> DraftIdentityForm { self.prefix.form() }
    pub(crate) fn identity(&self) -> &str { &self.identity }
    pub(crate) fn identity_offset(&self) -> u64 { self.offset + self.prefix.byte_len() as u64 }
}
...
mod tests {
    ...
    fn tagged_prefix_retains_alternate_compact_and_null_encodings() {
        for prefix in [&b"A\xf0\x00\xff\x02\x01"[..], &b"A\xf0\x80\x00\xff\x02\x01"[..], &b"A\xf0\xff\xff\x02\x01"[..]] {
            let index = if prefix[2] == 0xff { None } else { Some(0) };
            let frame = DraftIdentityFrame::from_wire(prefix, DraftIdentityForm::Tagged { index }, "0af".into(), 10).unwrap();
            ...
            assert_eq!(DraftIdentityFrame::from_wire(prefix, frame.form(), "0".into(), limit).unwrap().identity_offset(), u64::MAX);
            assert!(DraftIdentityFrame::from_wire(prefix, frame.form(), "0".into(), limit + 1).is_err());
        }
    }
}
