...
impl<O> DatumIndexLane<O> {
    ...
    pub(crate) fn trailer(&self) -> u32 { self.trailer }
    fn byte_len(&self) -> u16 {
        7 + self.indices.as_slice().iter().map(|atom| atom.raw().len() as u16).sum::<u16>()
    }
}
...
impl<O: Copy + Add<Output = O> + From<u16>> DatumIndexLane<O> {
    ...
    pub(crate) fn indices(&self) -> impl Iterator<Item = LocatedCompactIndex<O>> + '_ {
        ...
        self.indices.as_slice().iter().map(move |atom| {
            let token = LocatedCompactIndex { atom: *atom, offset };
...
macro_rules! checked_origin {
    ($offset:ty) => {
        impl DatumIndexLane<$offset> {
            pub(crate) fn new(indices: CountedIndexMembers<CompactIndexAtom, 1>, trailer: u32, offset: $offset) -> Option<Self> {
                let lane = Self { offset, indices, trailer };
...
impl DatumIndexLane<usize> {
    pub(crate) fn into_u64(self) -> DatumIndexLane<u64> {
        DatumIndexLane { offset: self.offset as u64, indices: self.indices, trailer: self.trailer }
    }
}
    ...
    fn datum_terminal_positions_follow_mixed_token_widths_and_checked_extent() {
        for count in [1, 254] {
            ...
            for slot in 0..count {
                ...
                if slot % 2 == 0 { bytes.extend_from_slice(&[0x80, 1]); }
                else { bytes.push(2); }
            }
            bytes.extend_from_slice(&[0, 0x12, 0x34, 0x56, 0x78]);
            let lane = scan(&bytes).into_iter().find(|lane| lane.offset() == 1).unwrap();
            assert_eq!(lane.indices().map(|token| token.offset).collect::<Vec<_>>(), offsets);
            assert_eq!(usize::from(lane.declared_count()), count + 1);
            assert_eq!(lane.trailer(), 0x1234_5678);
            let origin = u64::MAX - (bytes.len() - 1) as u64;
            let last = DatumIndexLane::<u64>::new(lane.indices.clone(), lane.trailer, origin).unwrap();
            assert_eq!(last.indices().map(|token| token.offset).collect::<Vec<_>>(), offsets.iter().map(|offset| origin + (*offset - 1) as u64).collect::<Vec<_>>());
