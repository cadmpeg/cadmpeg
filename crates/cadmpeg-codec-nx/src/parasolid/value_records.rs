...
impl ValueRecordFrame<'_> {
 ...
 fn retained<T>(self, value: T) -> ValueRecord<T> {
  ValueRecord { offset: self.offset, byte_len: self.end - self.offset, xmt: self.xmt, value }
 }
}
fn value_record_frame_at(bytes: &[u8], offset: usize) -> Option<ValueRecordFrame<'_>> {
 let tag = *bytes.get(offset.checked_add(1)?)?;
 match tag {
  0x52 => frame_at(bytes, offset, tag, 4, |raw| CountedLane::new(raw).map(ValuePayload::Integers)),
  0x53 => frame_at(bytes, offset, tag, 8, |raw| CountedLane::new(raw).map(ValuePayload::Doubles)),
  0x54 => frame_at(bytes, offset, tag, 1, |raw| PrintableString::new(std::str::from_utf8(raw).ok()?).ok().map(ValuePayload::String)),
  0x55 => frame_at(bytes, offset, tag, 24, |raw| CountedLane::new(raw).map(ValuePayload::Points)),
  0x56 => frame_at(bytes, offset, tag, 24, |raw| CountedLane::new(raw).map(ValuePayload::Vectors)),
  0x57 => frame_at(bytes, offset, tag, 24, |raw| CountedLane::new(raw).map(ValuePayload::Axes)),
  0x58 => frame_at(bytes, offset, tag, 4, |raw| CountedLane::new(raw).map(ValuePayload::Tags)),
  0x59 => frame_at(bytes, offset, tag, 24, |raw| CountedLane::new(raw).map(ValuePayload::Directions)),
  0x62 => frame_at(bytes, offset, tag, 2, |raw| UnicodeLane::new(raw).map(ValuePayload::Unicode)),
  _ => None,
 }
}
fn frame_at<'a>(bytes: &'a [u8], offset: usize, tag: u8, width: usize,
 ...
 Some(ValueRecordFrame { offset, end, xmt, payload })
}
fn append_value_record<'a>(frame: ValueRecordFrame<'a>, records: &mut EntityValueRecords<'a>) -> Option<()> {
 match frame.payload {
 ValuePayload::Integers(value) => records.integers.push(frame.retained(value.materialize()?)),
 ...
 ValuePayload::Directions(value) => records.directions.push(frame.retained(value.materialize()?)),
 ValuePayload::Unicode(value) => records.unicode.push(frame.retained(value.materialize()?)),
 }
 Some(())
}
#[cfg(test)]
pub(crate) fn entity_52_integer_record_at(bytes: &[u8], offset: usize) -> Option<ValueRecord<CountedValues<u32>>> {
 let frame = value_record_frame_at(bytes, offset)?;
 let ValuePayload::Integers(value) = frame.payload else { return None; };
 Some(frame.retained(value.materialize()?))
}
#[cfg(test)]
pub(crate) fn entity_53_double_record_at(bytes: &[u8], offset: usize) -> Option<ValueRecord<CountedValues<f64>>> {
 let frame = value_record_frame_at(bytes, offset)?;
 let ValuePayload::Doubles(value) = frame.payload else { return None; };
 Some(frame.retained(value.materialize()?))
}
#[cfg(test)]
pub(crate) fn entity_54_string_record_at(bytes: &[u8], offset: usize) -> Option<ValueRecord<PrintableString<&'_ str>>> {
 let frame = value_record_frame_at(bytes, offset)?;
