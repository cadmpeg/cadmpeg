...
impl FieldPosition {
    pub(crate) fn reference_ordinal(self) -> u32 {
        self.0
    }
    pub(crate) fn field_ordinal(self) -> u32 {
        self.0 - LEADING_REFERENCE_COUNT
    }
}

impl From<FieldPosition> for u32 {
    fn from(position: FieldPosition) -> Self {
        position.0
    }
}
...
mod tests {
    ...
    fn field_positions_cover_only_the_bounded_trailing_lane() {
        for reference in [5, 36] {
            ...
            assert_eq!(
                serde_json::from_str::<FieldPosition>(&wire).unwrap(),
                position
            );
        }
        ...
        let positions = references
            .fields()
            .map(|(position, _)| position.reference_ordinal());
