...
impl Type70State {
    pub(crate) fn new(xmt: u32, node_id: u32, references: [u32; 4], count: u16, trailing_reference: u32) -> Result<Self, &'static str> {
        Ok(Self {
            xmt: NonNullXmt::try_from(xmt)?, node_id, references,
            count: NonZeroU16::new(count).ok_or("count: must be positive")?,
            trailing_reference: NonNullXmt::try_from(trailing_reference).map_err(|_| "trailing_reference: must be non-null")?,
        })
    }
...
impl TryFrom<StateWire> for Type70State {
    ...
    fn try_from(wire: StateWire) -> Result<Self, Self::Error> { Self::new(wire.xmt, wire.node_id, wire.references, wire.count, wire.trailing_reference) }
}
#[cfg(test)]
mod tests {
    ...
    fn wire_preserves_open_body_references_and_rejects_null_controls() {
        let json = r#"{"xmt":6,"node_id":0,"references":[3,1,1,0],"count":13,"trailing_reference":45}"#;
        ...
        for (field, value) in [("xmt",1),("count",0),("trailing_reference",1)] {
            ...
            assert!(serde_json::from_value::<Type70State>(wire).unwrap_err().to_string().contains(field));
        }
    }
}
