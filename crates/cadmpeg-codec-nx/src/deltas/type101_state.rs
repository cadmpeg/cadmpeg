...
impl Type101State {
    pub(crate) fn new(
        ...
        let anchor_reference = anchor_reference
            .map(NonNullXmt::try_from)
            .transpose()
            .map_err(|_| "anchor_reference: require a non-null reference")?;
        let form = match (state_words[0], state_words[1]) {
            (19, 9) => Form::Populated,
            (0, 0) => Form::Empty,
            _ => return Err("state_words: require leading [19, 9] or [0, 0]"),
        };
        if terminal_value > 0xff_ffff_ffff {
            return Err("terminal_value: exceeds unsigned 40-bit range");
        }
        Ok(Self {
            references,
            anchor_reference,
            form,
            last_word: state_words[2],
            terminal_value,
        })
    }
...
impl From<Type101State> for StateWire {
    fn from(state: Type101State) -> Self {
        Self {
            ...
            anchor_reference: state.anchor_reference.map(u32::from),
...
mod tests {
    ...
    fn wire_preserves_both_forms_and_rejects_invalid_state() {
        for json in [
            ...
            for (field, value) in [
                ("anchor_reference", serde_json::json!(1)),
                ("state_words", serde_json::json!([19, 0, 27])),
                ("terminal_value", serde_json::json!(1099511627776u64)),
            ] {
                let mut wire: serde_json::Value = serde_json::from_str(json).unwrap();
                wire[field] = value;
                assert!(serde_json::from_value::<Type101State>(wire)
                    .unwrap_err()
                    .to_string()
                    .contains(field));
            }
