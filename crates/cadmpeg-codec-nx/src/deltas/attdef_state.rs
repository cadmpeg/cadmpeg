...
impl AttdefState {
    ...
    pub(crate) fn active_count(&self) -> u32 { self.slots.active_count() }
    pub(crate) fn slot_count(&self) -> u32 { self.slots.slot_count() }
    pub(crate) fn references(&self) -> impl Iterator<Item = u32> + '_ { self.slots.references() }
    pub(crate) fn into_slots(self) -> AttdefSlots { self.slots }
}
...
impl AttdefSlots {
    fn new(slot_count: u32, active_count: u32, references: Vec<u32>) -> Result<Self, &'static str> {
        ...
        Ok(Self { active, null_count: slot_count - active_count })
    }
    pub(crate) fn from_delta_references(references: Vec<u32>) -> Result<Self, &'static str> {
        let mut references = references.into_iter();
        if references.next() != Some(1) { return Err("references: ATTDEF_LIST must start with null"); }
        let references: Vec<_> = references.collect();
        let slot_count = u32::try_from(references.len()).map_err(|_| "references: too many slots")?;
        let active_count = references.iter().take_while(|reference| **reference != 1).count() as u32;
        Self::new(slot_count, active_count, references)
    }
    fn active_count(&self) -> u32 { self.active.len() as u32 }
    fn slot_count(&self) -> u32 { self.active_count() + self.null_count }
