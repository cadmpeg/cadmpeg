...
impl TryFrom<u8> for IntersectionMarker {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        match value {
            0x2b => Ok(Self::Type2b),
            0x2d => Ok(Self::Type2d),
            _ => Err("marker: require 0x2b or 0x2d"),
        }
    }
}
impl From<IntersectionMarker> for u8 {
    fn from(value: IntersectionMarker) -> Self {
        match value {
            IntersectionMarker::Type2b => 0x2b,
            IntersectionMarker::Type2d => 0x2d,
        }
    }
}
#[derive(Clone, Copy)]
...
impl Type38State {
    pub(crate) fn new(
        ...
        let lanes = match linked_references.as_slice() {
            &[left, right] => {
                ...
                identity
                    .checked_add(3)
                    .ok_or("xmt: state successors overflow")?;
                let prior = leading_references
                    .into_iter()
                    .chain([u32::from(left), u32::from(right)])
                    .fold(0, u32::max);
                prior
                    .checked_add(3)
                    .ok_or("leading_references: prior-anchor successors overflow")?;
                let is_sequence = |sequence: [u32; 3]| {
                    state_references.iter().copied().map(u32::from).eq(sequence)
                };
                if leading_statuses[4] == 0 {
                    if numeric_values.is_some() {
                        return Err(
                            "numeric_values: status-zero anchor cannot carry term-use state",
                        );
                    }
                    let anchor = leading_references[4];
                    if anchor <= 1 {
                        return Err("leading_references: status-zero anchor must be non-null");
                    }
                    if !is_sequence([anchor + 1, anchor + 2, anchor + 3]) {
                        return Err("state_references: require ascending anchor successors");
                    }
                    Lanes::Anchor {
                        linked: [left, right],
                    }
                    ...
                    if numeric_values.is_some() {
                        return Err("numeric_values: require descending identity successors");
                    }
                    if !is_sequence([prior + 1, prior + 2, prior + 3]) {
                        return Err("state_references: require descending identity or ascending prior successors");
                    }
                    Lanes::Prior {
                        linked: [left, right],
                    }
            ...
            &[linked] => {
                ...
                if anchor.checked_add(1) != Some(u32::from(second))
                    || anchor.checked_add(2) != Some(u32::from(third))
                {
    ...
    pub(crate) fn linked_references(&self) -> Vec<u32> {
        match self.lanes {
            ...
            | Lanes::Prior { linked }
            | Lanes::Anchor { linked } => linked.into_iter().map(u32::from).collect(),
    ...
    pub(crate) fn state_references(&self) -> Vec<u32> {
        match &self.lanes {
            ...
            Lanes::One { first, start, .. } => {
                let anchor = u32::from(*start);
                vec![u32::from(*first), anchor, anchor + 1, anchor + 2]
            }
        }
    }
    ...
    fn wire_preserves_all_reference_orders_and_rejects_inconsistent_forms() {
        for json in [
            ...
            for (field, value) in [
                ...
                wire[field] = value;
                assert!(serde_json::from_value::<Type38State>(wire)
                    .unwrap_err()
                    .to_string()
                    .contains(field));
            }
        ...
        assert!(serde_json::from_value::<Type38State>(prior)
            .unwrap_err()
            .to_string()
