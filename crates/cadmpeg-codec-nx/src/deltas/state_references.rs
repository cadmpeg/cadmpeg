...
pub(crate) enum StateReferences {
    Leading {
        references: [NonNullXmt; 3],
        last: Option<NonNullXmt>,
    },
...
impl TryFrom<[u32; 4]> for StateReferences {
    ...
    fn try_from([a, b, c, d]: [u32; 4]) -> Result<Self, Self::Error> {
        ...
        if b == 1 && d == 1 {
            return Ok(Self::Interleaved([a, c]));
        }
        let b = NonNullXmt::try_from(b).map_err(|_| error)?;
        let last = if d == 1 {
            None
        } else {
            Some(NonNullXmt::try_from(d).map_err(|_| error)?)
        };
        Ok(Self::Leading {
            references: [a, b, c],
            last,
...
impl From<StateReferences> for [u32; 4] {
    fn from(value: StateReferences) -> Self {
        match value {
            StateReferences::Leading {
                references: [a, b, c],
                last,
            } => [a.into(), b.into(), c.into(), last.map_or(1, u32::from)],
...
mod tests {
    ...
    fn state_reference_forms_preserve_wire_and_reject_invalid_null_positions() {
        ...
        for json in [
            ...
            assert!(serde_json::from_str::<StateReferences>(json)
                .unwrap_err()
                .to_string()
                .contains("references"));
        }
