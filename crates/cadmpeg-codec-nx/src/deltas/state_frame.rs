...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(try_from = "Vec<ReferenceStateFrame>", into = "Vec<ReferenceStateFrame>")]
pub(crate) struct StateFrames(Vec<ReferenceStateFrame>);

impl StateFrames {
    pub(crate) fn new(first: ReferenceStateFrame) -> Self { Self(vec![first]) }
    pub(crate) fn push(&mut self, frame: ReferenceStateFrame) { self.0.push(frame); }
    #[cfg(test)]
    pub(crate) fn as_slice(&self) -> &[ReferenceStateFrame] { &self.0 }
}
...
impl TryFrom<Vec<ReferenceStateFrame>> for StateFrames {
    ...
    fn try_from(frames: Vec<ReferenceStateFrame>) -> Result<Self, Self::Error> {
        if frames.is_empty() { return Err("frames: require at least one frame"); }
        Ok(Self(frames))
    }
}

impl From<StateFrames> for Vec<ReferenceStateFrame> {
    fn from(frames: StateFrames) -> Self { frames.0 }
}
...
mod tests {
    ...
    fn state_frames_preserve_array_wire_and_reject_empty_packets() {
        ...
        assert!(serde_json::from_str::<StateFrames>("[]").unwrap_err().to_string().contains("frames"));
    }
}
