...
impl SurfaceCurveState {
    pub(crate) fn new(
        ...
        Ok(Self {
            surface: surface
                .try_into()
                .map_err(|_| "surface_xmt: must exceed one")?,
            pcurve: pcurve
                .try_into()
                .map_err(|_| "pcurve_xmt: must exceed one")?,
...
impl TryFrom<StateWire> for SurfaceCurveState {
    ...
    fn try_from(wire: StateWire) -> Result<Self, Self::Error> {
        Self::new(
            wire.surface_xmt,
            wire.pcurve_xmt,
            wire.original_curve_xmt,
            wire.tolerance_to_original,
        )
    }
}
...
mod tests {
    ...
    fn surface_curve_wire_preserves_absence_and_independent_tolerance() {
        for (original, expected) in [(1, None), (0, Some(0)), (9, Some(9))] {
            let json = format!(
                r#"{{"surface_xmt":6,"pcurve_xmt":9,"original_curve_xmt":{original},"tolerance_to_original":-0.0}}"#
            );
            ...
            for field in ["surface_xmt", "pcurve_xmt"] {
                for invalid in [0, 1] {
                    ...
                    assert!(serde_json::from_value::<SurfaceCurveState>(wire)
                        .unwrap_err()
                        .to_string()
                        .contains(field));
                }
        ...
        for tolerance in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
            assert!(SurfaceCurveState::new(6, 9, 1, tolerance)
                .unwrap_err()
