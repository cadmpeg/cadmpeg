...
//! Fixed-record Parasolid topology used by NX neutral-binary streams.
//!
//! The records are self-describing only through their type tag and XMT
//! references. This module establishes the unshifted small-index form; callers
//! can use the resulting record offsets to join it to decoded geometry carriers.

use std::collections::BTreeMap;

/// A fixed neutral-binary record with its logical XMT identifier.
#[derive(Debug, Clone)]
pub struct Node {
    /// Parasolid node type.
    pub kind: u8,
...
impl Node {
    /// Read a small big-endian XMT field at its logical record offset.
    pub fn xmt_at(&self, offset: usize) -> Option<u32> {
        read_xmt(&self.bytes, offset + self.shift).map(|(xmt, _)| xmt)
    }

    /// Read adjacent logical XMT slots, carrying each extended encoding's
    /// physical-width expansion into the following slot.
...
/// Decode unshifted, small-index trimmed-curve records. The point witnesses at
/// `+21` and `+45` are intentionally not duplicated in IR: the EDGE's vertex
/// references are the authoritative topological endpoints.
...
impl Graph {
    /// Parse the small-XMT, unescaped fixed-record family.
