...
enum RmColorChoice<'a> {
    Unique { definition: &'a str, source_offset: u64 },
    Conflicting,
}
...
fn resolve_rm_source_color_bindings(
    ...
    for assignment in assignments {
        let Some(source_id) = assignment.target_object_id.as_deref() else { continue; };
        choices.entry(source_id)
            .and_modify(|choice| choice.observe(assignment))
            .or_insert_with(|| RmColorChoice::new(assignment));
    }
    choices.into_iter().filter_map(|(source_id, choice)| {
        let RmColorChoice::Unique { definition, source_offset } = choice else { return None; };
...
fn resolve_rm_face_color_bindings(
    ...
    let definition_ids = definitions.iter().map(|definition| definition.id.as_str()).collect::<BTreeSet<_>>();
    let mut face_records_by_node = BTreeMap::<u32, Vec<_>>::new();
    for record in records {
        let crate::deltas::record_family::RecordFamily::Face { node_id, .. } = &record.family else { continue; };
    ...
    for (object_index, choice) in choices {
        let RmColorChoice::Unique { definition, source_offset } = choice else { continue; };
        if !definition_ids.contains(definition) { continue; }
        let candidates = face_records_by_node
            ...
            .filter_map(|record| {
                ...
                let (Some(partition), None) = (partitions.next(), partitions.next()) else { return None; };
                let face_id = format!("nx:s{partition}:face#{}", record.xmt);
                face_ids.contains(&face_id).then_some(face_id)
            })
            .collect::<BTreeSet<_>>();
        let mut candidates = candidates.into_iter();
        let (Some(face_id), None) = (candidates.next(), candidates.next()) else { continue; };
