...
pub(crate) fn attach(
    ...
    if !model.om.configurations.is_empty() {
        for (ordinal, configuration) in model.om.configurations.iter().enumerate() {
            let id = ConfigurationId::mint(format!("nx:arrangements:configuration#{ordinal}")).expect("identity grammar");
...
fn attach_initial_segment_bodies(
    ...
    let id = FeatureId::mint("nx:feature-history:feature#initial-bodies".to_string()).expect("identity grammar");
...
fn attach_feature_operations(
    ...
    let feature_ids_by_operation = labels
        ...
        .map(|label| {
            ...
            (
                label.id.as_str(),
                FeatureId::mint(format!("nx:feature-history:feature#{key}")).expect("identity grammar"),
            )
        })
        .collect::<BTreeMap<_, _>>();
...
fn text_semantic_annotation(
    ...
    Some(SemanticAnnotation {
        id: SemanticAnnotationId::mint(format!("{native_ref}:semantic-text")).expect("identity grammar"),
