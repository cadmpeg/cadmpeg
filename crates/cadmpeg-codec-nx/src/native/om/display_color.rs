...
use super::{PartColorDefinition, RmFastLoadObjectId, rmfastload_target_object_id};
...
impl RmDisplayColorAssignmentEncoding {
    fn offset(&self) -> u64 {
        match self { Self::Linked(row) => row.offset(), Self::Target(row) => row.offset() }
    }
}
...
impl DisplayColorFrame {
    pub(crate) fn new(encoding: RmDisplayColorAssignmentEncoding, color_index: PaletteIndex) -> Option<Self> {
        encoding.offset().checked_sub(u64::from(color_index.display_byte_len()) + crate::om::column_row::ROW_SUFFIX.len() as u64)?;
        Some(Self { encoding, color_index })
    }
    pub(crate) fn encoding(&self) -> &RmDisplayColorAssignmentEncoding { &self.encoding }
    pub(crate) fn offset(&self) -> u64 { self.encoding.offset() - u64::from(self.color_index.display_byte_len()) }
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(try_from = "wire::RmDisplayColorAssignmentWire", into = "wire::RmDisplayColorAssignmentWire")]
...
pub fn rm_display_color_assignments(
        ...
        .filter(|(entry, _)| entry.name == "/Root/FastLoad/RMFastLoad")
    {
        ...
        for row in crate::om::column_row::scan::linked_rows(record_area) {
            let Some(color) = crate::om::column_row::scan::preceding_color(record_area, row.offset()) else {
                continue;
            };
            ...
            let target_object_id = rmfastload_target_object_id(object_ids, row.target_index().atom.value());
            let Some(row) = row.into_absolute(source_base) else { continue; };
            let Some(frame) = DisplayColorFrame::new(RmDisplayColorAssignmentEncoding::Linked(row), color) else { continue; };
            candidates.push((frame, target_object_id, definition.id.clone(), entry.name.clone()));
        }
        for row in crate::om::column_row::scan::target_rows(record_area) {
            ...
            let target_object_id = rmfastload_target_object_id(object_ids, row.target_index().atom.value());
            let Some(row) = row.into_absolute(source_base) else { continue; };
            let Some(frame) = DisplayColorFrame::new(RmDisplayColorAssignmentEncoding::Target(row), color) else { continue; };
            candidates.push((frame, target_object_id, definition.id.clone(), entry.name.clone()));
        }
    }
    candidates.sort_by_key(|(frame, _, _, _)| frame.offset());
    candidates.into_iter().enumerate().map(|(ordinal, (frame, target_object_id, color_definition, source_entry))| RmDisplayColorAssignment {
