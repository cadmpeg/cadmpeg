...
pub struct ObjectUuidValue {
    ...
    #[serde(serialize_with = "serialize_records", deserialize_with = "deserialize_records")]
...
fn serialize_records<S: Serializer>(records: &NonEmpty<String>, serializer: S) -> Result<S::Ok, S::Error> {
    serializer.collect_seq(records.iter())
}

fn deserialize_records<'de, D: Deserializer<'de>>(deserializer: D) -> Result<NonEmpty<String>, D::Error> {
    NonEmpty::new(Vec::<String>::deserialize(deserializer)?)
        .ok_or_else(|| serde::de::Error::custom("records must contain at least one intersected record"))
}
        ...
        .flat_map(|(section_ordinal, (entry, section))| {
            ...
            crate::om::uuid_string_values(storage, first.offset)
                ...
                .filter_map(|value| {
                    ...
                    let records = NonEmpty::new(records
                        .iter()
                        .enumerate()
                        .filter(|(_, record)| {
...
mod tests {
    ...
    fn uuid_records_preserve_order_and_reject_empty_ownership() {
        ...
        assert!(serde_json::from_value::<ObjectUuidValue>(wire).unwrap_err().to_string().contains("records"));
    }
}
