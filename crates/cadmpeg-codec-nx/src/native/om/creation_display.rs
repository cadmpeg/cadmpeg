...
use super::{RmFastLoadObjectId, rmfastload_target_object_id};
...
pub(crate) enum RmCreationDisplayDataEncoding {
    Index(IndexRow<(), u64>),
    Linked { row: LinkedRow<(), u64>, target_object_id: Option<String> },
    Target { row: TargetRow<(), u64>, target_object_id: Option<String> },
}
...
#[serde(try_from = "wire::RmCreationDisplayDataRelationWire", into = "wire::RmCreationDisplayDataRelationWire")]
...
pub fn rm_creation_display_data_relations(
        ...
        .filter(|(entry, _)| entry.name == "/Root/FastLoad/RMFastLoad")
    {
        ...
        for row in crate::om::column_row::scan::index_rows(record_area) {
            ...
            candidates.push((RmCreationDisplayDataEncoding::Index(row), class_definition.clone(), entry.name.clone()));
        }
        for row in crate::om::column_row::scan::linked_rows(record_area) {
            ...
            let Some(row) = row.into_absolute(source_base) else { continue; };
            candidates.push((RmCreationDisplayDataEncoding::Linked { row, target_object_id }, class_definition.clone(), entry.name.clone()));
        }
        for row in crate::om::column_row::scan::target_rows(record_area) {
            ...
            let target_object_id = rmfastload_target_object_id(object_ids, row.target_index().atom.value());
            let Some(row) = row.into_absolute(source_base) else { continue; };
            candidates.push((RmCreationDisplayDataEncoding::Target { row, target_object_id }, class_definition.clone(), entry.name.clone()));
        }
    }

    candidates.sort_by_key(|(encoding, _, _)| encoding.offset());
    candidates.into_iter().enumerate().map(|(ordinal, (encoding, class_definition, source_entry))| RmCreationDisplayDataRelation {
