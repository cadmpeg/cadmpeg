...
fn row_indices<const N: usize>(values: [u32; N], raw: [Vec<u8>; N]) -> [Result<CompactIndexAtom, String>; N] {
...
impl From<RmCreationDisplayDataRelation> for RmCreationDisplayDataRelationWire {
    fn from(value: RmCreationDisplayDataRelation) -> Self {
        ...
        let (first_index, raw_first_index, first_index_source_offset, encoding, target_object_id) = match value.encoding {
            RmCreationDisplayDataEncoding::Index(row) => (
                Some(row.first_index().atom.value()), Some(row.first_index().atom.raw().to_vec()), Some(row.first_index().offset),
        ...
        Self {
            ...
            first_index, raw_first_index, first_index_source_offset,
            class_name: CLASS_NAME.to_owned(), class_definition: value.class_definition,
            encoding, target_object_id, source_entry: value.source_entry, source_offset,
        }
    }
}
...
impl TryFrom<RmCreationDisplayDataRelationWire> for RmCreationDisplayDataRelation {
    ...
    fn try_from(wire: RmCreationDisplayDataRelationWire) -> Result<Self, Self::Error> {
        if wire.class_name != CLASS_NAME { return Err("class_name must be UGS::RM_creation_display_data".into()); }
        ...
        Ok(Self { id: wire.id, ordinal: wire.ordinal, class_definition: wire.class_definition, encoding, source_entry: wire.source_entry })
    }
}
