    ...
    fn from(value: DataBlockCountedIndexLane) -> Self {
        ...
        Self {
            ...
            member_data_blocks: value
                .frame
                .members()
                .map(|index| index.target.clone())
                .collect(),
            source_offset: value.frame.offset(),
            anchor_source_offset: anchor.offset,
...
impl TryFrom<DataBlockCountedIndexLaneWire> for DataBlockCountedIndexLane {
    ...
    fn try_from(wire: DataBlockCountedIndexLaneWire) -> Result<Self, Self::Error> {
        ...
        let members = wire
            ...
            .map(|((index, raw), target)| {
                Ok(CompactIndexTarget {
                    atom: CompactIndexAtom::from_wire(index, &raw)
                        .map_err(|error| format!("member_indices/raw_member_indices: {error}"))?,
                    target,
                })
            })
        ...
        let frame = CountedLane::<String, u64>::new(anchor, members, wire.source_offset)
            .ok_or("source_offset: counted lane extent overflows")?;
        if frame.anchor().offset != wire.anchor_source_offset {
            return Err("anchor_source_offset: inconsistent with counted lane".into());
        }
        if frame
            .members()
            .map(|index| index.offset)
            .ne(wire.member_source_offsets)
        {
            return Err("member_source_offsets: inconsistent with compact token widths".into());
        }
        Ok(Self {
            id: wire.id,
...
impl TryFrom<DataBlockAbrReferenceLaneWire> for DataBlockAbrReferenceLane {
    ...
    fn try_from(wire: DataBlockAbrReferenceLaneWire) -> Result<Self, Self::Error> {
        ...
        for (slot, ((raw, index), target)) in slots.iter_mut().zip(
            wire.raw_slot_indices
                .into_iter()
        ...
        let frame = AbrLane::<String, u64>::new(slots, wire.source_offset)
            .ok_or("source_offset: ABR lane extent overflows")?;
...
mod tests {
    ...
    fn counted_wire_rejects_each_inconsistent_derived_position() {
        ...
        for member in 0..2 {
            ...
            let error = serde_json::from_value::<DataBlockCountedIndexLane>(invalid).unwrap_err();
            assert!(
                error.to_string().contains("member_source_offsets"),
                "{error}"
            );
        }
    }
    ...
    fn abr_wire_rejects_each_inconsistent_derived_position() {
        let frame = AbrLane::<String, u64>::new(
            std::array::from_fn(|i| {
                (i % 2 == 0).then(|| CompactIndexTarget {
        ...
        .unwrap();
        let lane = DataBlockAbrReferenceLane {
            id: "lane".into(),
            section_ordinal: 0,
            ordinal: 0,
        ...
        assert_eq!(
            json["slot_source_offsets"],
            serde_json::json!([10, 12, 13, 15, 16, 18, 19, 21, 22, 24, 25, 27, 28, 30, 31, 33])
        );
        for slot in 0..16 {
            let mut invalid = json.clone();
    ...
    fn counted_lane_wire_preserves_member_columns() {
        ...
        assert_eq!(
            lane.frame
                .members()
                .map(|index| (index.atom, index.target.as_str(), index.offset))
                .collect::<Vec<_>>(),
            [(
                crate::om::compact::CompactIndexAtom::read(&[2]).unwrap(),
                "member",
                12
            )]
        );
        ...
        for field in [
            ...
            assert!(serde_json::from_value::<
                crate::native::om::compact_lane::DataBlockCountedIndexLane,
            >(malformed)
            .is_err());
        }
    ...
    fn abr_lane_wire_preserves_sixteen_nullable_columns() {
        ...
        assert_eq!(
            (
                lane.frame.slots()[0]
                    .atom
                    .map(|index| (index.atom, index.target.as_str())),
                lane.frame.slots()[0].offset
            ),
            (
                Some((
                    crate::om::compact::CompactIndexAtom::read(&[2]).unwrap(),
