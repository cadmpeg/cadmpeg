    ...
    fn try_from(wire: EncodingWire) -> Result<Self, Self::Error> {
        match wire {
            EncodingWire::Linked {
                ...
                let offset = object_index_source_offset
                    .checked_sub(2)
                    .ok_or("object_index_source_offset precedes row prefix")?;
                let row = LinkedRow::<(), u64>::new(
                    first,
                    discriminator,
                    target,
                    indices,
                ...
                .ok_or("object_index_source_offset: row extent overflows")?;
                if row.target_index().offset != target_index_source_offset {
                    return Err("target_index_source_offset differs from row layout".into());
                }
                if row.indices().map(|index| index.offset) != index_source_offsets {
                    return Err("index_source_offsets differ from row layout".into());
                }
                Ok(Self::Linked(row))
            }
            EncodingWire::Target {
                ...
                .into();
                let offset = target_index_source_offset
                    .checked_sub(5)
                    .ok_or("target_index_source_offset precedes row prefix")?;
                let row = TargetRow::<(), u64>::new(target, indices, mode, offset)
                    .ok_or("target_index_source_offset: row extent overflows")?;
                if row.indices().map(|index| index.offset) != index_source_offsets {
                    return Err("index_source_offsets differ from row layout".into());
                }
                Ok(Self::Target(row))
...
impl TryFrom<RmDisplayColorAssignmentWire> for RmDisplayColorAssignment {
    ...
    fn try_from(wire: RmDisplayColorAssignmentWire) -> Result<Self, Self::Error> {
        ...
        let frame = DisplayColorFrame::new(wire.encoding, color_index)
            .ok_or("source_offset: color token precedes origin")?;
        if frame.offset() != wire.source_offset {
            return Err("source_offset differs from the color-token position".into());
        }
        if frame.encoding.offset() != wire.row_source_offset {
            return Err("row_source_offset differs from the encoding origin".into());
        }
        Ok(Self {
            id: wire.id,
...
mod tests {
    ...
    fn color_assignment_derives_both_offsets_from_its_row_and_token() {
        ...
        for field in ["source_offset", "row_source_offset"] {
            ...
            wire[field] = 99.into();
            assert!(serde_json::from_value::<RmDisplayColorAssignment>(wire)
                .unwrap_err()
                .to_string()
                .contains(field));
        }
