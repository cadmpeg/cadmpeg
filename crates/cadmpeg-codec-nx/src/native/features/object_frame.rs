...
impl From<DataBlockObjectFrame> for ObjectFrameWire {
    fn from(value: DataBlockObjectFrame) -> Self {
        Self {
            ...
            object_id: value.object.atom.value(), raw_object_id: value.object.atom.raw().to_vec(),
...
impl TryFrom<ObjectFrameWire> for DataBlockObjectFrame {
    ...
    fn try_from(wire: ObjectFrameWire) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            id: wire.id, data_block: wire.data_block, ordinal: wire.ordinal,
            object: LocatedCompactIndex { atom, offset: wire.source_offset },
        })
    }
}
...
mod tests {
    ...
    fn object_frames_derive_identity_from_the_exact_non_null_token() {
        for (value, raw) in [(0, "[0]"), (115, "[115]"), (370, "[129,114]"), (0, "[128,0]"), (32511, "[254,255]")] {
            let wire = format!(r#"{{"id":"frame","data_block":"block","ordinal":1,"object_id":{value},"raw_object_id":{raw},"source_offset":100}}"#);
            ...
            assert!(serde_json::from_value::<DataBlockObjectFrame>(invalid).unwrap_err().to_string().contains("object_id/raw_object_id"));
        }
        for raw in ["[]", "[255]", "[128]", "[0,0]"] {
            let wire = format!(r#"{{"id":"frame","data_block":"block","ordinal":1,"object_id":0,"raw_object_id":{raw},"source_offset":100}}"#);
            assert!(serde_json::from_str::<DataBlockObjectFrame>(&wire).unwrap_err().to_string().contains("object_id/raw_object_id"));
        }
    }
}
