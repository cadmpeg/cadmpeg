...
use crate::om::{Swp104PayloadLeadingBranch, branch_items::BranchItems, reference_index::PayloadIndexToken, scalar::ShiftedBinary64, swp104_state::Swp104StateLane};
...
impl FeatureSwp104LeadingBranch {
    ...
    fn byte_len(&self) -> u64 {
        self.members_offset()
            + self.members.as_slice().iter().map(|item| item.token.raw().len() as u64).sum::<u64>()
            + self.state_len() + 3 + self.terminal.token.raw().len() as u64 + 1
    }
}
...
impl From<FeatureSwp104LeadingBranch> for FeatureSwp104LeadingBranchWire {
    fn from(value: FeatureSwp104LeadingBranch) -> Self {
        ...
        let members = value.members.map_indexed(|ordinal, reference| {
            ...
            ReferenceWire { ordinal: ordinal as u32, token: reference.token, data_block: reference.data_block, source_offset }
        });
        ...
        Self {
            ...
            members, terminal, byte_len, source_offset: value.source_offset,
        }
    }
}
...
impl TryFrom<FeatureSwp104LeadingBranchWire> for FeatureSwp104LeadingBranch {
    ...
    fn try_from(wire: FeatureSwp104LeadingBranchWire) -> Result<Self, Self::Error> {
        ...
        let mut at = wire.source_offset.checked_add(40 + u64::from(wire.leading_zero)).ok_or("source_offset overflow")?;
        for (ordinal, item) in wire.members.as_slice().iter().enumerate() {
            if item.ordinal != ordinal as u32 { return Err("members ordinal does not match serialized order".to_owned()); }
        ...
        if wire.byte_len != end - wire.source_offset { return Err("byte_len does not match serialized frame length".to_owned()); }
        let reference = |item: ReferenceWire| Reference { token: item.token, data_block: item.data_block };
        Ok(Self {
            id: wire.id, operation_label: wire.operation_label,
            discriminator: wire.discriminator, scalars,
            leading_zero: wire.leading_zero, mode: wire.mode, state_lane,
            members: wire.members.map_indexed(|_, item| reference(item)),
            terminal: reference(wire.terminal), source_offset: wire.source_offset,
        })
    }
}
        ...
        for leading_zero in [false, true] {
            for witnessed_count in [None, Some(2_u8), Some(255)] {
                for member_count in [1_u8, 254] {
                    ...
                    for _ in 0..4 { payload.extend([47, 164, 122, 225, 71, 174, 20, 123]); }
                    if leading_zero { payload.push(0); }
                    payload.extend([35, 1, member_count + 1]);
                    for _ in 0..member_count { payload.extend([240, 1]); }
                    if let Some(count) = witnessed_count {
                        payload.extend([1, count]);
                        payload.extend(std::iter::repeat_n(0, usize::from(count) + 3));
                    } else { payload.extend([0; 5]); }
                    ...
                    let branch = FeatureSwp104LeadingBranch::from_source("branch".to_owned(), "operation".to_owned(), 1200, source.clone(), |token| Some(format!("block#{}", token.value()))).unwrap();
                    let wire = serde_json::to_value(&branch).unwrap();
                    assert_eq!(wire["byte_len"], payload.len());
                    assert_eq!(wire["members"][0]["source_offset"], 1240 + u64::from(leading_zero));
                    ...
                    assert_eq!(serde_json::from_value::<FeatureSwp104LeadingBranch>(wire.clone()).unwrap(), branch);
                    ...
                    assert!(FeatureSwp104LeadingBranch::from_source("branch".to_owned(), "operation".to_owned(), u64::MAX, source, |_| None).is_none());
                }
            }
        }
    }
