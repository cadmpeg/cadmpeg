...
impl BlockReferencePosition {
    pub(crate) fn new(ordinal: u32) -> Result<Self, &'static str> {
        if ordinal > 18 { return Err("ordinal: block reference position must be within 0..=18"); }
        Ok(Self(ordinal as u8))
    }
    pub(crate) fn ordinal(self) -> u32 { u32::from(self.0) }
    fn terminal(self) -> bool { self.0 == 18 }
    pub(crate) fn enumerate<T>(references: [T; 19]) -> impl Iterator<Item = (Self, T)> {
        references.into_iter().enumerate().map(|(ordinal, reference)| (Self(ordinal as u8), reference))
    }
}
...
impl From<BlockReferencePosition> for PositionWire {
    fn from(position: BlockReferencePosition) -> Self {
        Self { ordinal: position.ordinal(), terminal: position.terminal() }
    }
}
...
impl TryFrom<PositionWire> for BlockReferencePosition {
    ...
    fn try_from(wire: PositionWire) -> Result<Self, Self::Error> {
        ...
        if wire.terminal != position.terminal() { return Err("terminal: only ordinal 18 is the terminal block reference"); }
        Ok(position)
    }
}
...
mod tests {
    ...
    fn block_positions_follow_the_fixed_field_and_preserve_native_wire() {
        for (ordinal, (position, value)) in BlockReferencePosition::enumerate([42; 19]).enumerate() {
            ...
            assert_eq!(BlockReferencePosition::new(ordinal as u32).unwrap(), position);
            let terminal = ordinal == 18;
            for target in ["", r#","data_block":"block""#] {
                let wire = format!(r#"{{"id":"reference","operation_label":"block-op","control":38,"ordinal":{ordinal},"terminal":{terminal},"object_index":66,"raw_object_index":[240,66]{target},"source_offset":100}}"#);
                let reference: FeatureBlockConstructionReference = serde_json::from_str(&wire).unwrap();
                ...
                assert!(serde_json::from_value::<FeatureBlockConstructionReference>(invalid).unwrap_err().to_string().contains("terminal"));
            }
        }
        for ordinal in [19, u32::MAX] {
            let wire = serde_json::json!({"ordinal":ordinal,"terminal":false});
            assert!(serde_json::from_value::<BlockReferencePosition>(wire).unwrap_err().to_string().contains("ordinal"));
        }
    }
}
