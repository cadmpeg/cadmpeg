...
impl FeatureUnlabeledOperationRecord {
    pub(crate) fn from_source(id: String, ordinal: u32, entry_offset: u64, record: UnlabeledOperationRecord<'_>) -> Option<Self> {
        let header = OperationHeader::<u64>::new(entry_offset.checked_add(record.header().offset() as u64)?, record.header().objects())?;
    ...
    pub(crate) fn source_offset(&self) -> u64 { self.header.offset() }
}
...
impl TryFrom<UnlabeledRecordWire> for FeatureUnlabeledOperationRecord {
    ...
    fn try_from(wire: UnlabeledRecordWire) -> Result<Self, Self::Error> {
        ...
        for (slot, token) in tokens.iter_mut().enumerate() {
            let end = wire.object_index_source_offsets.get(slot + 1).copied().unwrap_or(wire.payload_source_offset);
            let width = end.checked_sub(wire.object_index_source_offsets[slot])
                .ok_or_else(|| format!("object_index_source_offsets[{slot}]: reversed token extent"))?;
        ...
        header.end_offset().checked_add(wire.payload_byte_len).ok_or("payload_byte_len: record end overflows")?;
        ...
        Ok(Self {
            id: wire.id, ordinal: wire.ordinal, header, sha256: wire.sha256,
            payload_byte_len: wire.payload_byte_len, payload_sha256: wire.payload_sha256,
        })
    }
}
...
mod tests {
    ...
    fn unlabeled_record_wire_retains_header_widths_and_payload_extent() {
        ...
        assert_eq!(record.header.objects().0.map(|token| token.map(|token| token.raw().to_vec())),
            [None, Some(vec![0]), Some(vec![128, 0]), Some(vec![144, 0, 0])]);
        for (field, value) in [
            ...
            ("object_index_source_offsets", serde_json::json!([114,116,117,119])),
            ("object_index_source_offsets", serde_json::json!([115,116,118,117])),
            ("object_index_source_offsets", serde_json::json!([114,115,116,118])),
            ...
            invalid[field] = value;
            assert!(serde_json::from_value::<FeatureUnlabeledOperationRecord>(invalid).is_err(), "{field}");
        }
    }
    ...
    fn unlabeled_record_wire_accepts_the_last_representable_end() {
        ...
        wire["object_index_source_offsets"] = serde_json::json!([start + 15, start + 16, start + 17, start + 19]);
        ...
        assert!(serde_json::from_value::<FeatureUnlabeledOperationRecord>(wire).unwrap_err().to_string().contains("payload_byte_len"));
    }
}
