...
fn binary64_pair_wire_preserves_all_payload_owner_forms() {
    for payload in ["datum_csys_payload", "datum_plane_payload", "construction_payload", "surface_construction_payload"] {
        ...
        let positions = if payload == "datum_plane_payload" { "23,32" } else { "20,29" };
        let json = format!(r#"{{"id":"pair","operation_label":"operation","{payload}":"payload","ordinal":0,"values":[1.0,2.0],"raw_values":[[47,240,0,0,0,0,0,0],[48,0,0,0,0,0,0,0]],"payload_offset":5,"value_payload_offsets":[{positions}],"source_offset":105,"value_source_offsets":[120,129]{discriminator}}}"#);
        let pair: super::FeaturePayloadScalarPair = serde_json::from_str(&json).unwrap();
        assert_eq!(serde_json::to_string(&pair).unwrap(), json);
        let invalid = json.replace("1.0", "3.0");
...
fn binary64_pair_wire_requires_owner_form_and_complete_payload_extent() {
    let forms = [
        ("datum_csys_payload", Some(vec![8,2,3,1,3,1,192,69,4,0,128,134,2,0,3]), 15_u64, 1_u64),
        ("surface_construction_payload", Some(vec![8,2,3,1,129,2,1,192,69,4,0,128,134,2,0,3]), 16, 1),
        ("datum_plane_payload", None, 18, 1),
        ("construction_payload", Some(vec![47,47,65,0,3,1,3,1,192,69,4,0,128,134,2,0,3]), 17, 0),
    ];
    for (owner, discriminator, prefix, separator) in forms {
        ...
        if let Some(discriminator) = discriminator { wire["discriminator"] = serde_json::json!(discriminator); }
        ...
        for slot in 0..2 {
            ...
            assert!(serde_json::from_value::<super::FeaturePayloadScalarPair>(invalid).unwrap_err().to_string().contains("value_payload_offsets"));
        }
        let mut invalid = wire.clone();
        invalid["payload_offset"] = serde_json::json!(origin + 1);
        assert!(serde_json::from_value::<super::FeaturePayloadScalarPair>(invalid).unwrap_err().to_string().contains("payload_offset"));
        if owner != "datum_plane_payload" {
            for discriminator in [vec![], vec![8,2,3,1,3,1], vec![0,0,65,0,3,1,3,1,192,69,4,0,128,134,2,0,3]] {
                let mut invalid = wire.clone();
                invalid["discriminator"] = serde_json::json!(discriminator);
                assert!(serde_json::from_value::<super::FeaturePayloadScalarPair>(invalid).unwrap_err().to_string().contains("discriminator"));
            }
        }
