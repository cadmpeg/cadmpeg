...
fn nx_body_writing_thread_labels_retain_distinct_unresolved_families() {
    ...
    let threads = super::body_writing_unresolved_feature_definition("THREADS", &source_properties);
    let detailed =
        super::body_writing_unresolved_feature_definition("DETAILED_THREAD", &source_properties);
...
fn nx_non_body_writing_thread_labels_remain_unresolved_for_semantic_review() {
    ...
    assert_eq!(
        super::body_writing_unresolved_feature_definition("THREADS", &source_properties),
        None
    );
    assert_eq!(
        super::body_writing_unresolved_feature_definition("DETAILED_THREAD", &source_properties),
