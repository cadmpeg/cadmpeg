...
fn nx_body_writing_mirror_face_retains_unresolved_family() {
    ...
    let definition = body_writing_unresolved_feature_definition("MIRROR_FACE", &source_properties);
...
fn nx_non_body_writing_mirror_face_remains_native_for_semantic_review() {
    assert_eq!(
        body_writing_unresolved_feature_definition("MIRROR_FACE", &BTreeMap::new()),
        None
    );
}
