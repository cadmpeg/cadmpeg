...
impl From<ParasolidChartRecord> for ChartWire {
    fn from(value: ParasolidChartRecord) -> Self {
        Self {
            ...
            points: value.data.points().iter().map(|point| [point.x, point.y, point.z]).collect(),
            ...
            framing: value.framing, inflated_offset: value.inflated_offset,
        }
    }
}
impl TryFrom<ChartWire> for ParasolidChartRecord {
    ...
    fn try_from(wire: ChartWire) -> Result<Self, Self::Error> {
        ...
        let points = wire.points.into_iter().map(|[x, y, z]| Point3::new(x, y, z)).collect();
        ...
        if wire.count != data.count() { return Err("count: does not match points length"); }
        if wire.chart_count != data.count() { return Err("chart_count: does not match points length"); }
        Ok(Self {
            id: wire.id, stream_ordinal: wire.stream_ordinal, xmt: wire.xmt,
            preamble: ChartPreamble::new(wire.base_parameter, wire.base_scale, wire.chordal_error, wire.angular_error)?,
            data, framing: wire.framing, inflated_offset: wire.inflated_offset,
        })
    }
}
