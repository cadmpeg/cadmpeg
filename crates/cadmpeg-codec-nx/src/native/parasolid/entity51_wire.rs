...
impl TryFrom<Entity51Wire> for ParasolidEntity51Record {
    ...
    fn try_from(wire: Entity51Wire) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            sequence: NonZeroU32::new(wire.sequence).ok_or("sequence must be nonzero")?,
            definition_xmt: wire.definition_xmt,
...
mod tests {
    ...
    fn entity51_wire_derives_count_and_rejects_mismatched_or_unbounded_counts() {
        ...
        let mut wire = serde_json::to_value(&record).unwrap();
        wire["flags"] = 1.into();
        assert!(serde_json::from_value::<ParasolidEntity51Record>(wire)
            .unwrap_err()
            .to_string()
            .contains("flags"));
