    ...
    fn from(value: FeatureDatumPlanePayload) -> Self {
        let (
            ...
            Some(lane) => (
                ...
                lane.indices().map(|entry| entry.atom.raw().to_vec()).collect(),
        ...
        let index_lane =
            match (
                ...
                (Some(offset), Some(declared_count), Some(trailer), _) => {
                    ...
                    if !lane.indices().map(|token| token.offset).eq(wire.index_lane_value_offsets) {
    ...
    fn from(lane: FeatureDraftConstructionIndexLane) -> Self {
        let (declared_count, tokens, data_blocks): (_, Vec<_>, _) = match lane.indices {
            FeatureDraftConstructionIndices::Unresolved(frame) =>
                (usize::from(frame.declared_count()), frame.indices().map(|token| LocatedCompactIndex { atom: token.atom, offset: token.offset }).collect(), None),
            FeatureDraftConstructionIndices::Resolved(frame) => {
                let (tokens, blocks) = frame.indices()
                    .map(|token| (LocatedCompactIndex { atom: token.atom, offset: token.offset }, token.target.clone()))
                    .unzip();
...
impl TryFrom<FeatureDraftConstructionIndexLaneWire> for FeatureDraftConstructionIndexLane {
    ...
    fn try_from(wire: FeatureDraftConstructionIndexLaneWire) -> Result<Self, Self::Error> {
        ...
        let origin = wire.source_offsets.first().and_then(|offset| offset.checked_sub(24))
            .ok_or("source_offsets: missing draft frame prefix")?;
        let tokens = wire.indices.into_iter().zip(wire.raw_indices).enumerate()
            .map(|(slot, (value, raw))| CompactIndexAtom::from_wire(value, &raw)
                .map(Into::into).map_err(|error| format!("indices[{slot}]: {error}")))
            .collect::<Result<Vec<_>, _>>()?;
        let tokens = CountedIndexMembers::new(tokens).map_err(|error| format!("indices: {error}"))?;
        let frame = DraftLeadingLane::<(), u64>::new(tokens, origin)
            .ok_or("source_offsets: draft frame extent overflows")?;
        if !frame.indices().map(|token| token.offset).eq(wire.source_offsets) {
        ...
        let indices = match wire.data_blocks {
            ...
            Some(blocks) => {
                ...
                FeatureDraftConstructionIndices::Resolved(frame.try_resolve(|_| blocks.next())
...
impl TryFrom<FeatureDraftConstructionTerminalLaneWire> for FeatureDraftConstructionTerminalLane {
    ...
    fn try_from(wire: FeatureDraftConstructionTerminalLaneWire) -> Result<Self, Self::Error> {
        ...
        let lane = crate::om::draft_terminal::DraftTerminalLane::<u64>::new(
            [first?, second?], wire.tail, wire.source_offset,
        ).ok_or("source_offset overflows the terminal frame")?;
        if lane.indices().map(|token| token.offset) != wire.index_source_offsets {
            return Err("index_source_offsets must follow source_offset in the terminal frame".into());
        }
        Ok(Self { id: wire.id, operation_label: wire.operation_label, lane })
    }
}
...
pub fn feature_draft_construction_index_lanes(
    ...
    visit_feature_history_operation_records(
        ...
        |_section, section_key, entry_offset, operation_ordinal, record| {
            ...
            let Some(frame) = lane.into_absolute(entry_offset) else { return; };
            let indices = match section_ordinal {
                None => FeatureDraftConstructionIndices::Unresolved(frame),
                Some(section_ordinal) => FeatureDraftConstructionIndices::Resolved(
                    frame.resolve(|index| format!("nx:om-data-blocks-{section_ordinal}:block#{index}")),
                ),
            };
...
pub fn feature_draft_construction_terminal_lanes(
    ...
    visit_feature_history_operation_records(
        container,
        |_section, section_key, entry_offset, operation_ordinal, record| {
            let Some(lane) = crate::om::draft_terminal::scan(record.payload_view()).and_then(|lane| lane.into_absolute(entry_offset)) else {
                return;
            };
