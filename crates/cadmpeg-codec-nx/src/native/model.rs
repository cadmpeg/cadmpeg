...
impl NativeModel {
    ...
    /// Mirrors the legacy all-empty early-return guard exactly. The conjunction
    /// is the original guard's operand set (in original order) minus the sole
    /// non-model operand `object_sections`, which the caller still checks
    /// alongside this method.
    ///
    /// The following model fields are intentionally NOT part of the emptiness
    /// test, mirroring the legacy guard which omitted them:
    /// - `configuration_attribute_uses` (`om`)
    /// - `data_block_column_index_tables` (`om`)
    /// - `data_block_index_rows` (`om`)
    /// - `data_block_linked_index_rows` (`om`)
    /// - `data_block_target_index_rows` (`om`)
    /// - `display_jt_base_node_data` (`display_jt`)
    /// - `display_jt_compressed_element_sequences` (`display_jt`)
    /// - `display_jt_compressed_elements` (`display_jt`)
    /// - `display_jt_coordinate_array_headers` (`display_jt`)
    /// - `display_jt_documents` (`display_jt`)
    /// - `display_jt_geometric_transform_attributes` (`display_jt`)
    /// - `display_jt_group_node_data` (`display_jt`)
    /// - `display_jt_initial_face_degree_symbols` (`display_jt`)
    /// - `display_jt_instance_nodes` (`display_jt`)
    /// - `display_jt_partition_nodes` (`display_jt`)
    /// - `display_jt_polygon_meshes` (`display_jt`)
    /// - `display_jt_range_lod_nodes` (`display_jt`)
    /// - `display_jt_segments` (`display_jt`)
    /// - `display_jt_shape_lod_bindings` (`display_jt`)
    /// - `display_jt_shape_lod_elements` (`display_jt`)
    /// - `display_jt_string_property_atoms` (`display_jt`)
    /// - `display_jt_topology_packet_sequences` (`display_jt`)
    /// - `display_jt_tri_strip_lod_headers` (`display_jt`)
    /// - `display_jt_tri_strip_shape_nodes` (`display_jt`)
    /// - `display_jt_vertex_colors` (`display_jt`)
    /// - `display_jt_vertex_coordinates` (`display_jt`)
    /// - `display_jt_vertex_flags` (`display_jt`)
    /// - `display_jt_vertex_normals` (`display_jt`)
    /// - `display_jt_vertex_records_headers` (`display_jt`)
    /// - `display_jt_vertex_texture_coordinates` (`display_jt`)
    /// - `feature_block_dimensions` (`features`)
    /// - `feature_body_reference_occurrences` (`features`)
    /// - `feature_body_segment_uses` (`features`)
    /// - `feature_datum_csys_descriptors` (`features`)
    /// - `feature_datum_csys_payload_fixed_pairs` (`features`)
    /// - `feature_datum_csys_payload_scalar_pairs` (`features`)
    /// - `feature_datum_csys_payload_scalars` (`features`)
    /// - `feature_datum_csys_payloads` (`features`)
    /// - `feature_datum_plane_csys_identity_uses` (`features`)
    /// - `feature_datum_plane_descriptors` (`features`)
    /// - `feature_datum_plane_payload_scalar_pairs` (`features`)
    /// - `feature_input_column_row_uses` (`features`)
    /// - `feature_input_column_targets` (`features`)
    /// - `feature_sketch_payload_coordinate_pairs` (`features`)
    /// - `feature_sketch_preceding_named_point_uses` (`features`)
    /// - `parasolid_topology_attribute_class_uses` (`parasolid`)
    pub(crate) fn is_empty(&self) -> bool {
        self.segments.segment_index_rows.is_empty()
            && self.segments.segment_om_links.is_empty()
            && self.segments.segment_stream_links.is_empty()
            && self.segments.segment_body_bindings.is_empty()
            && self.segments.segment_body_lineage_statuses.is_empty()
            && self.parasolid.parasolid_blend_surface_records.is_empty()
            && self.parasolid.parasolid_blend_bound_records.is_empty()
            && self.parasolid.parasolid_offset_surface_records.is_empty()
            && self.parasolid.parasolid_trimmed_curve_records.is_empty()
            && self.parasolid.parasolid_surface_curve_records.is_empty()
            && self.parasolid.parasolid_intersection_records.is_empty()
            && self.parasolid.parasolid_term_use_records.is_empty()
            && self.parasolid.parasolid_support_uv_records.is_empty()
            && self.parasolid.parasolid_chart_records.is_empty()
            && self.parasolid.parasolid_attribute_definitions.is_empty()
            && self.parasolid.parasolid_entity_51_records.is_empty()
            && self
                .parasolid
                .parasolid_entity_52_integer_records
                .is_empty()
            && self.parasolid.parasolid_entity_53_double_records.is_empty()
            && self.parasolid.parasolid_entity_54_string_records.is_empty()
            && self.parasolid.parasolid_entity_51_numeric_uses.is_empty()
            && self.parasolid.parasolid_entity_51_string_uses.is_empty()
            && self.parasolid.parasolid_attribute_class_uses.is_empty()
            && self
                .parasolid
                .parasolid_topology_attribute_list_references
                .is_empty()
            && self.om.om_record_areas.is_empty()
            && self.features.feature_operation_labels.is_empty()
            && self.features.feature_operation_records.is_empty()
            && self.features.feature_payload_strings.is_empty()
            && self.features.feature_simple_hole_templates.is_empty()
            && self
                .features
                .feature_simple_hole_repeated_scalar_lanes
                .is_empty()
            ...
            && self
                ...
                .feature_simple_hole_construction_groups
                .is_empty()
            && self.features.feature_body_references.is_empty()
            && self.features.feature_input_blocks.is_empty()
            && self.features.feature_input_block_identity_groups.is_empty()
            && self.features.feature_datum_csys_constructions.is_empty()
            && self.features.feature_datum_plane_headers.is_empty()
            && self.features.feature_datum_plane_block_uses.is_empty()
            && self.features.feature_datum_plane_payloads.is_empty()
            && self.features.feature_datum_csys_block_uses.is_empty()
            && self.features.feature_sketch_references.is_empty()
            && self.features.feature_projected_curve_references.is_empty()
            && self
                .features
                .feature_projected_curve_construction_payloads
                .is_empty()
            && self
                ...
                .feature_projected_curve_construction_strings
                .is_empty()
            && self.features.feature_pattern_references.is_empty()
            && self
                .features
                .feature_pattern_construction_payloads
                .is_empty()
            ...
            && self
                ...
                .feature_pattern_construction_fixed_lanes
                .is_empty()
            && self.features.feature_pattern_transform_lanes.is_empty()
            && self.features.feature_point_construction_headers.is_empty()
            && self
                .features
                .feature_point_construction_scalar_lanes
                .is_empty()
            ...
            && self
                ...
                .feature_draft_construction_index_lanes
                .is_empty()
            && self.features.feature_draft_construction_payloads.is_empty()
            && self
                .features
                .feature_draft_construction_graph_payloads
                .is_empty()
            ...
            && self
                ...
                .feature_surface_construction_branches
                .is_empty()
            && self.features.feature_extrude_profile_references.is_empty()
            && self.features.feature_extrude_payload_headers.is_empty()
            && self.features.feature_extrude_payload_footers.is_empty()
            && self
                .features
                .feature_operation_body_scalar_triples
                .is_empty()
            && self.features.feature_operation_body_members.is_empty()
            && self.features.feature_operation_body_operands.is_empty()
            && self
                .features
                .feature_operation_body_11_continuations
                .is_empty()
            ...
            && self
                ...
                .feature_extrude_construction_profiles
                .is_empty()
            && self.features.feature_extrude_payload_32_branches.is_empty()
            && self.features.feature_extrude_32_constructions.is_empty()
            && self
                .features
                .feature_block_construction_references
                .is_empty()
            && self.features.feature_block_constructions.is_empty()
            && self.features.feature_block_construction_payloads.is_empty()
            && self.features.feature_block_payload_scalars.is_empty()
            && self.features.feature_block_payload_names.is_empty()
            && self.features.feature_block_payload_named_records.is_empty()
            && self.features.feature_block_payload_points.is_empty()
            && self.features.feature_block_payload_point_groups.is_empty()
            && self.features.feature_sketch_records.is_empty()
            && self.features.feature_sketch_construction_inputs.is_empty()
            && self
                .features
                .feature_sketch_construction_payloads
                .is_empty()
            && self.features.feature_sketch_payload_scalars.is_empty()
            && self.features.feature_sketch_payload_names.is_empty()
            && self.features.feature_sketch_payload_fixed_pairs.is_empty()
            && self
                .features
                .feature_sketch_payload_named_records
                .is_empty()
            && self.features.feature_sketch_points.is_empty()
            && self.features.feature_sketch_fixed_points.is_empty()
            && self.features.feature_sketch_point_groups.is_empty()
            && self.features.offset_store_named_points.is_empty()
            && self
                .features
                .feature_sketch_named_point_block_uses
                .is_empty()
            && self.features.feature_sketch_point_uses.is_empty()
            && self
                .features
                .feature_sketch_datum_csys_dependencies
                .is_empty()
            && self.features.feature_boolean_operations.is_empty()
            && self.om.expression_declarations.is_empty()
            && self.features.data_block_object_frames.is_empty()
            && self.om.expressions.is_empty()
            && self.om.classes.is_empty()
            && self.om.fields.is_empty()
            && self.om.object_records.is_empty()
            && self.om.rmfastload_object_id_tables.is_empty()
            && self.om.rmfastload_object_ids.is_empty()
            && self.om.data_blocks.is_empty()
            && self.om.data_block_control_values.is_empty()
            && self.om.data_block_control_class_references.is_empty()
            && self.om.data_block_control_index_values.is_empty()
            && self.om.data_block_control_references.is_empty()
            && self.om.data_block_control_handle_pairs.is_empty()
            && self.om.data_block_references.is_empty()
            && self.om.data_block_counted_index_lanes.is_empty()
            && self.om.data_block_abr_reference_lanes.is_empty()
            && self.features.feature_parameter_bindings.is_empty()
            && self.features.feature_parameter_uses.is_empty()
            && self.om.store_headers.is_empty()
            && self.om.string_values.is_empty()
            && self.om.object_references.is_empty()
            && self.om.persistent_handles.is_empty()
            && self.om.configurations.is_empty()
            && self.om.part_attributes.is_empty()
            && self.om.external_references.is_empty()
            && self.om.external_reference_indexed_records.is_empty()
            && self.om.external_reference_records.is_empty()
            && self.om.external_reference_empty_records.is_empty()
            && self.om.external_reference_tail_reference_pairs.is_empty()
            && self.om.external_reference_record_string_uses.is_empty()
            && self.om.external_reference_record_children.is_empty()
            && self.om.material_texture_assets.is_empty()
            && self.om.material_texture_catalog_entries.is_empty()
            && self.display_jt.display_jt_indices.is_empty()
    }
}
