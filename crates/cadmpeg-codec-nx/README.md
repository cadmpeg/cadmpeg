...
transfer. Validated embedded TIFF material textures transfer as exact document
assets. Complete design history, assembly occurrence placement, material
assignment, appearances, entity-owned attributes, tessellation, and native
`.prt` writing are not supported. See the [format support matrix][support] for
current coverage and the [format notes][spec] for byte semantics.
