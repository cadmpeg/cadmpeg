...
fn merged_archive_keeps_each_component_unknown_record_image_and_owner() {
    ...
    for unknown in unknowns {
        ...
        let id = format!("f3d:xref/{XREF_ROLE}/occurrence-0/{suffix}");
        ...
        let owner = format!(
            "f3d:xref/{XREF_ROLE}%2Foccurrence-0/{}",
    ...
    let source_image = format!("f3d:xref/{XREF_ROLE}/occurrence-0/file:source-image#0");
    assert_eq!(
        decoded
            .source_fidelity()
