            ...
            (section.referenced_profile(), scope.sweep_profile())
        {
            ...
            if group_matches {
                ...
                if let (Some(placement), None) = (candidates.next(), candidates.next()) {
                    let sketch = match neutral_sketch_id(placement) {
                        Some(id) => id,
                        None => continue,
                    };
...
pub(crate) fn bind_loft_and_revolve_sketch_selections(
    ...
    for group in groups.iter().filter(|group| {
        ...
        let spatial_sketch_id = match neutral_spatial_sketch_id(placement) {
            Some(id) => id,
            None => continue,
        };
        ...
        .filter(|group| group.role() == DesignOperandRole::ROLE_0X5 && group.members.len() == 1)
    {
        ...
        let entity = match neutral_spatial_sketch_curve_id(
            &spatial_sketch_id,
            curve.primary_id,
            curve.secondary_id,
        ) {
