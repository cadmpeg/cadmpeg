    ...
    let (codec, confidence) = match forced {
        ...
        None => {
            let (codec, confidence) = registry.detect(&prefix).ok_or_else(|| {
                anyhow!("no codec recognized {}; inspect supports container inputs only, not .cadir.json IR documents; supported: f3d, sldprt, CATPart, NX/Creo prt; use --input-format to override detection", path.display())
            })?;
