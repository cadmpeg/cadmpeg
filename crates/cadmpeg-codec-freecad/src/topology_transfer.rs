...
+1 0 *";
        ...
        assert_eq!(
            result
                ...
                .iter()
                .find(|appearance| appearance.schema.as_deref()
                    == Some("FCStd ViewProvider point style"))
        ...
        assert!(cadmpeg_ir::validate_neutral(&corrupted, Vec::new())
            ...
            .iter()
            .any(
                |finding| finding.message == "invalid view presentation reference, order, or size"
            ));
        assert!(result
            .ir()
            .model
...
+1 0 *";
        ...
        let surface = result
            ...
            .expect("required invariant");
        let cadmpeg_ir::geometry::SurfaceGeometry::Transformed { basis, transform } =
            &surface.geometry
        else {
        ...
        let origin = cadmpeg_ir::eval::surface_point(&surface.geometry, 0.0, 0.0)
            .expect("required invariant");
        ...
        for edge in &result.ir().model.edges {
            ...
            let start = cadmpeg_ir::eval::curve_point(&curve.geometry, range[0])
                .expect("required invariant");
            let end = cadmpeg_ir::eval::curve_point(&curve.geometry, range[1])
                .expect("required invariant");
