...
<Constrain Type="99" First="-1" FirstPos="0" Second="-2" SecondPos="0"/>
    ...
    let cadmpeg_ir::sketches::SketchConstraintDefinition::Native { entities, .. } =
    ...
    for (entity, suffix, direction) in [
        ...
        assert!(entity.0.ends_with(suffix));
        let resolved = result
            .ir()
            .model
            .sketch_entities
        ...
        assert!(matches!(&resolved.geometry,
            cadmpeg_ir::sketches::SketchGeometry::ReferenceLine { origin, direction: actual }
