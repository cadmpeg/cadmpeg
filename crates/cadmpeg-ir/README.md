...
- required IR version 4 schema, including the `subds` arena;
...
canonical arena order. `CadIr::from_json` parses only `ir_version: "4"`, while
`CadIr::migrate_json` explicitly migrates version 3. The version-4 schema
