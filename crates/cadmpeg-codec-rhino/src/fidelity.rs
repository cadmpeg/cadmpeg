...
//! L2 source-fidelity sidecar for the Rhino 3DM container.
//!
//! [`ledger`] projects the single [`partition`](crate::accounting::partition) of
//! a completed [`Scan`] into a v2 source-fidelity sidecar
//! ([`SourceFidelity`]) at [`LedgerLevel::L2`]: complete *refined* tiling at
//! record granularity. The root `source` space is tiled so every physical byte
//! is classified — the 32-byte archive header `Typed`, the leading comment and
//! every direct table record one `Opaque` span, and all container framing
...
/// Build the L2 source-fidelity sidecar for a scanned Rhino container.
///
/// The returned sidecar is canonicalized and has passed
/// [`SourceFidelity::validate`]; its single `source` space tiles `[0, length)`
/// exactly at record granularity. The tiling is total by construction, so a
/// validation failure would be a builder defect and panics rather than being
/// serialized.
pub(crate) fn ledger(scan: &Scan<'_>) -> SourceFidelity {
    let data = scan.data;
    let spans = partition(scan)
        .into_iter()
        .map(|tile| LedgerSpan {
    ...
    let sidecar = SourceFidelity::new(LedgerLevel::L2, LedgerCapability::Accounted, vec![source]);
    sidecar
        .validate()
