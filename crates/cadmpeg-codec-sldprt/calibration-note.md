...
| spline curve poles                                                                     | `brep/spline.rs:521`                  | guarded `control.len()==control_count*dimension`                                                                        | low                                   |
| spline surface poles                                                                   | `brep/spline.rs:695`                  | stored `u_count*v_count` uses `checked_mul`; control values must equal the product times stored vertex dimension | none (descriptor-bounded, control-checked) |
