...
use super::endpoints::{
    ...
    extended_identity_inline_profile_line, extended_tagged_indexed_curve_endpoint_indices,
...
#[cfg(test)]
use super::assembly::legacy_feature_input_section;
#[cfg(test)]
use super::axes::{
...
#[cfg(test)]
use super::bindings::{bind_resolved_curve_vertices, normalize_indexed_curve_entities};
#[cfg(test)]
...
use super::component_paths::{
    compact_edge_path_value, compact_edge_selection_set_value, component_path_feature,
    component_path_features, component_path_input_features, component_path_terminal_feature,
    surface_selection_producer_features, ComponentPathEnd,
};
#[cfg(test)]
use super::curves::{
    compact_bounded_curve_tangent, compact_line_chain_addresses, compact_line_region_addresses,
    ...
    ordered_compact_line_profile, ordered_rectangle_corners, resolve_connected_marker_arcs,
    resolve_slot_marker_arcs, resolve_tangent_bridge_marker_arcs,
    resolve_two_center_semicircle_profile, sketch_plane_frames, tangent_bounded_curve,
    tangent_bridge_arc_geometry, unique_dimensioned_rectangle_markers,
};
#[cfg(test)]
...
use super::endpoints::{
    ...
    compact_legacy_profile_full_circle, compact_legacy_selected_axis_endpoint_indices,
    compact_legacy_short_role_one_curve_endpoint_indices,
    compact_legacy_short_role_two_curve_endpoint_indices, compact_profile_full_circle,
    ...
    extended_compact_indexed_curve_endpoint_indices, extended_declared_inline_line_endpoints,
    ...
    legacy_long_profile_line_endpoint_indices, legacy_marker104_arc_center,
    legacy_profile_radial_circle, legacy_profile_roster_selected_axis_endpoint_indices,
...
#[cfg(test)]
use super::parameters::native_scalar_matches_discrete_parameter;
#[cfg(test)]
...
use super::reference_geometry::{
    ...
    minimal_reference_plane_frame, offset_plane_reference_frame_matches,
    ...
    reconcile_reference_plane_frame, reference_plane_frame_key,
...
#[cfg(test)]
use super::relation_loci::unique_locus;
#[cfg(test)]
...
use super::selections::{
    ...
    inline_surface_reference_at, mirror_pattern_component_path_at,
...
use super::terminations::{
    ...
    compact_single_face_reference_record_at, consensus_termination_vote,
    legacy_single_face_reference_path_at, CompactPointReferenceKind, TerminationVote,
};
#[cfg(test)]
use super::typed_relations::{
    compact_legacy_object_line_endpoints, consecutive_legacy_profile_line_endpoints,
    coordinate_centered_line_endpoints, current_coordinate_linked_line_endpoints,
    current_wide_undetailed_line, extended_direct_object_line_endpoints,
    legacy_marker104_arc_endpoints, legacy_terminal_indexed_profile_line,
    marker_curve_endpoint_markers, one_based_point_roster_line_endpoint_markers,
};
#[cfg(test)]
use super::write_prepare::{
    append_spatial_vertex, arc_angle_relation_kind, patch_spatial_vertex, solved_tangent,
};
#[cfg(test)]
use super::{
    CLASS_MARKER, COMPACT_SCALAR_HEADER, NAME_MARKER, SCALAR_HEADER, VALUE_ONLY_SCALAR_HEADER,
};
#[cfg(test)]
use crate::records::FeatureInputScalarRole;
#[cfg(test)]
use cadmpeg_ir::features::{Angle, BooleanOp, Length};
...
pub(super) fn linked_profile_point(payload: &[u8], offset: usize) -> Option<LinkedProfilePoint> {
    ...
    if !matches!(
        ...
        || (!profile_layout && !legacy_geometry_long_layout)
        || marker_profile_curve_role(payload, offset) != Some(1)
        || payload.get(offset + 29..offset + 31) != Some(&[0; 2])
        || payload.get(offset + 31..offset + 39)
            != Some(&[0x00, 0x00, 0x80, 0xbf, 0x00, 0x00, 0x04, 0x00])
    ...
    let valid_tail = long_tail || (profile_layout && standard_tail);
    if !valid_tail {
        return None;
    }
