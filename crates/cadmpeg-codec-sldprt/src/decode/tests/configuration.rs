...
#[test]
fn unique_partition_binds_the_only_native_configuration_without_name_metadata() {
    let mut ir = CadIr::empty(Units::default());
    ...
    ir.model.configurations.push(DesignConfiguration {
        id: ConfigurationId("native".into()),
        ordinal: 0,
        active: false.into(),
        source_index: None,
        name: "Default".into(),
        material: None,
        properties: BTreeMap::from([("id".into(), "0".into())]),
        bodies: cadmpeg_ir::ConfigurationBodies::Unresolved,
        parameter_values: BTreeMap::new(),
        suppressed_features: Vec::new(),
        parameter_overrides: BTreeMap::new(),
    ...
    assign_configuration_bodies(&mut ir, &[(0, vec![body.clone()])]);
    mark_active_configuration(&mut ir);

    assert_eq!(ir.model.configurations.len(), 1);
    assert!(ir.model.configurations[0].active.is_active());
    assert_eq!(ir.model.configurations[0].source_index, Some(0));
    assert_eq!(ir.model.configurations[0].bodies, vec![body]);
    assert!(ir.model.configurations[0].native_ref.is_some());
}

#[test]
fn unique_partition_without_native_slot_remains_inferred() {
    let mut ir = CadIr::empty(Units::default());
    ir.source = Some(cadmpeg_ir::document::SourceMeta {
        attributes: BTreeMap::from([(
            "active_parasolid_block".into(),
    ...
    assert_eq!(ir.model.configurations.len(), 2);
    assert!(ir.model.configurations[0].bodies.is_unresolved());
    assert_eq!(ir.model.configurations[1].source_index, Some(0));
    assert_eq!(ir.model.configurations[1].bodies, vec![body]);
    assert!(ir.model.configurations[1].native_ref.is_none());
}
