use super::super::disc1e_disc12_disc1a_disc18_disc10_direct_face_root_body;
...
fn direct_keyed_face_use_lattice_owns_selected_faces_and_ignores_unselected_faces() {
    let bodies =
        disc1e_disc12_disc1a_disc18_disc10_direct_face_root_body(&index_records(&lattice()));
    let [body] = bodies.as_slice() else {
        panic!("one disc1e-disc12-disc1a-disc18-disc10 direct face-use body");
    };
...
fn direct_keyed_face_use_lattice_rejects_a_missing_use_back_reference() {
    ...
    assert!(
        disc1e_disc12_disc1a_disc18_disc10_direct_face_root_body(&index_records(&records))
            .is_empty()
    );
}

#[test]
fn direct_keyed_face_use_lattice_rejects_a_mismatched_use_key() {
    let mut records = lattice();
    records
        .iter_mut()
        .find(|record| record.attr == 31)
