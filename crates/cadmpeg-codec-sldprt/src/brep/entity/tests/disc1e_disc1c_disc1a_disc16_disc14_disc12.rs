use super::super::{
    ...
    disc1d_disc15_disc1b_disc19_disc13_face_root_body,
    ...
    disc22_disc1c_disc1a_disc18_disc16_disc14_disc12_disc0e_face_root_body,
    disc26_disc1c_disc24_disc22_disc16_face_root_body,
...
fn disc10_disc14_disc16_terminal_lattice() -> Vec<super::super::EntityRecord> {
    vec![
        flo2(10, 0x14, [7, 1, 11, 1, 1, 1]),
        flo2(11, 0x16, [7, 10, 12, 1, 1, 1]),
        flo2(12, 0x1e, [7, 11, 13, 1, 1, 1]),
        flo2(13, 0x12, [7, 12, 14, 1, 1, 1]),
        record(14, 0x10, [7, 13, 1, 1, 1, 1]),
        record(20, 0x18, [100, 30, 1, 1, 1, 1]),
        record(21, 0x18, [101, 31, 1, 1, 1, 1]),
        record(22, 0x18, [102, 50, 1, 1, 1, 1]),
        record(30, 0x1c, [100, 40, 20, 1, 1, 1]),
        record(31, 0x1c, [101, 41, 1, 1, 1, 1]),
        record(32, 0x1c, [102, 42, 22, 1, 1, 1]),
        record(33, 0x1c, [103, 1, 1, 1, 1, 1]),
...
#[test]
fn disc10_disc14_disc16_terminal_lattice_owns_forward_and_keyed_faces_with_extra_records() {
    let records = disc10_disc14_disc16_terminal_lattice();
    let bodies = disc14_disc16_disc1e_disc12_disc10_face_root_body(&index_records(&records));
    let [body] = bodies.as_slice() else {
        panic!("one disc14-disc16-disc1e-disc12-disc10 body");
    };
    assert_eq!(body.attr, 10);
    assert_eq!(body.regions[0].shells[0].attr, 14);
    assert!(body.refs.contains(&20) && body.refs.contains(&21) && body.refs.contains(&22));
...
#[test]
fn disc10_disc14_disc16_terminal_lattice_accepts_stale_reverse_links_by_key() {
    let mut records = disc10_disc14_disc16_terminal_lattice();
    records
        .iter_mut()
        .find(|record| record.attr == 20)
        .expect("first canonical face")
...
#[test]
fn disc10_disc14_disc16_terminal_lattice_rejects_a_companion_with_a_different_key() {
    let mut records = disc10_disc14_disc16_terminal_lattice();
    records
        .iter_mut()
        .find(|record| record.attr == 31)
    ...
    assert!(disc14_disc16_disc1e_disc12_disc10_face_root_body(&index_records(&records)).is_empty());
}

#[test]
fn disc10_disc14_disc16_terminal_lattice_rejects_ambiguous_same_key_companions() {
    let mut records = disc10_disc14_disc16_terminal_lattice();
    records
        .iter_mut()
        .find(|record| record.attr == 20)
...
fn disc16_disc26_terminal_lattice() -> Vec<super::super::EntityRecord> {
    vec![
        flo2(10, 0x26, [7, 1, 11, 1, 1, 1]),
        ...
        record(32, 0x1e, [102, 42, 22, 1, 1, 1]),
        record(33, 0x1e, [103, 1, 1, 1, 1, 1]),
        flo4(40, 0x28, [100, 1, 30, 1, 1, 1]),
        flo4(41, 0x28, [101, 1, 1, 1, 1, 1]),
        flo4(42, 0x28, [102, 1, 32, 1, 1, 1]),
        record(50, 0x02, [1, 1, 1, 1, 1, 1]),
...
#[test]
fn disc16_disc26_terminal_lattice_owns_forward_and_keyed_faces_with_extra_companions() {
    let records = disc16_disc26_terminal_lattice();
    let bodies = disc26_disc1c_disc24_disc22_disc16_face_root_body(&index_records(&records));
    let [body] = bodies.as_slice() else {
        panic!("one disc26-disc1c-disc24-disc22-disc16 body");
    };
    assert_eq!(body.attr, 10);
    assert_eq!(body.regions[0].shells[0].attr, 14);
    assert!(body.refs.contains(&20) && body.refs.contains(&21) && body.refs.contains(&22));
...
#[test]
fn disc16_disc26_terminal_lattice_accepts_stale_reverse_links_by_key() {
    let mut records = disc16_disc26_terminal_lattice();
    records
        .iter_mut()
        .find(|record| record.attr == 20)
        .expect("first canonical face")
...
#[test]
fn disc16_disc26_terminal_lattice_rejects_a_companion_with_a_different_key() {
    let mut records = disc16_disc26_terminal_lattice();
    records
        .iter_mut()
        .find(|record| record.attr == 31)
...
#[test]
fn disc16_disc26_terminal_lattice_rejects_ambiguous_same_key_companions() {
    let mut records = disc16_disc26_terminal_lattice();
    records
        .iter_mut()
        .find(|record| record.attr == 20)
...
fn disc13_odd_terminal_lattice() -> Vec<super::super::EntityRecord> {
    vec![
        flo2(10, 0x1d, [7, 1, 11, 1, 1, 1]),
        flo2(11, 0x15, [7, 10, 12, 1, 1, 1]),
        flo2(12, 0x1b, [7, 11, 13, 1, 1, 1]),
        flo2(13, 0x19, [7, 12, 14, 1, 1, 1]),
        record(14, 0x13, [7, 13, 1, 1, 1, 1]),
        record(20, 0x0f, [100, 30, 1, 1, 1, 1]),
        record(21, 0x0f, [101, 31, 1, 1, 1, 1]),
        record(22, 0x0f, [102, 50, 1, 1, 1, 1]),
        record(30, 0x17, [100, 40, 20, 1, 1, 1]),
        record(31, 0x17, [101, 41, 1, 1, 1, 1]),
        record(32, 0x17, [102, 42, 22, 1, 1, 1]),
        record(33, 0x17, [103, 1, 1, 1, 1, 1]),
        flo4(40, 0x1f, [100, 1, 30, 1, 1, 1]),
        flo4(41, 0x1f, [101, 1, 1, 1, 1, 1]),
        flo4(42, 0x1f, [102, 1, 32, 1, 1, 1]),
        record(50, 0x02, [1, 1, 1, 1, 1, 1]),
    ]
}

#[test]
fn disc13_odd_terminal_lattice_owns_forward_and_keyed_faces_with_extra_companions() {
    let records = disc13_odd_terminal_lattice();
    let bodies = disc1d_disc15_disc1b_disc19_disc13_face_root_body(&index_records(&records));
    let [body] = bodies.as_slice() else {
        panic!("one disc1d-disc15-disc1b-disc19-disc13 body");
    };
    assert_eq!(body.attr, 10);
    assert_eq!(body.regions[0].shells[0].attr, 14);
    assert!(body.refs.contains(&20) && body.refs.contains(&21) && body.refs.contains(&22));
...
#[test]
fn disc13_odd_terminal_lattice_accepts_stale_reverse_links_by_key() {
    let mut records = disc13_odd_terminal_lattice();
    records
        .iter_mut()
        .find(|record| record.attr == 20)
        .expect("first canonical face")
...
#[test]
fn disc13_odd_terminal_lattice_rejects_a_companion_with_a_different_key() {
    let mut records = disc13_odd_terminal_lattice();
    records
        .iter_mut()
        .find(|record| record.attr == 31)
    ...
    assert!(disc1d_disc15_disc1b_disc19_disc13_face_root_body(&index_records(&records)).is_empty());
}

#[test]
fn disc13_odd_terminal_lattice_rejects_ambiguous_same_key_companions() {
    let mut records = disc13_odd_terminal_lattice();
    records
        .iter_mut()
        .find(|record| record.attr == 20)
...
fn disc0e_long_keyed_lattice() -> Vec<super::super::EntityRecord> {
    vec![
        flo2(10, 0x22, [7, 1, 11, 1, 1, 1]),
        flo2(11, 0x1c, [7, 10, 12, 1, 1, 1]),
        flo2(12, 0x1a, [7, 11, 13, 1, 1, 1]),
        flo2(13, 0x18, [7, 12, 14, 1, 1, 1]),
        record(14, 0x16, [7, 13, 15, 1, 1, 1]),
        flo2(15, 0x14, [7, 14, 16, 1, 1, 1]),
        flo2(16, 0x12, [7, 15, 17, 1, 1, 1]),
        flo2(17, 0x0e, [7, 16, 1, 1, 1, 1]),
        record(20, 0x04, [100, 30, 1, 1, 1, 1]),
        record(21, 0x04, [101, 31, 1, 1, 1, 1]),
        record(30, 0x20, [100, 40, 20, 1, 1, 1]),
        record(31, 0x20, [101, 41, 21, 1, 1, 1]),
        flo4(40, 0x24, [100, 1, 30, 1, 1, 1]),
        flo4(41, 0x24, [101, 1, 31, 1, 1, 1]),
        flo4(50, 0x24, [102, 1, 1, 1, 1, 1]),
    ]
}

#[test]
fn disc0e_long_keyed_lattice_owns_faces_and_ignores_extra_use_nodes() {
    let records = disc0e_long_keyed_lattice();
    let bodies = disc22_disc1c_disc1a_disc18_disc16_disc14_disc12_disc0e_face_root_body(
        &index_records(&records),
    );
    let [body] = bodies.as_slice() else {
        panic!("one disc22-disc1c-disc1a-disc18-disc16-disc14-disc12-disc0e body");
    };
    assert_eq!(body.attr, 10);
    assert_eq!(body.regions[0].shells[0].attr, 14);
    assert!(body.refs.contains(&20) && body.refs.contains(&21));
...
#[test]
fn disc0e_long_keyed_lattice_accepts_stale_reverse_links_by_key() {
    let mut records = disc0e_long_keyed_lattice();
    records
        .iter_mut()
        .find(|record| record.attr == 20)
        .expect("first canonical face")
...
#[test]
fn disc0e_long_keyed_lattice_rejects_a_companion_with_a_different_key() {
    let mut records = disc0e_long_keyed_lattice();
    records
        .iter_mut()
        .find(|record| record.attr == 31)
    ...
    assert!(
        disc22_disc1c_disc1a_disc18_disc16_disc14_disc12_disc0e_face_root_body(&index_records(
            &records
        ))
        .is_empty()
    );
...
#[test]
fn disc0e_long_keyed_lattice_rejects_an_incomplete_chain() {
    let mut records = disc0e_long_keyed_lattice();
    records
        .iter_mut()
