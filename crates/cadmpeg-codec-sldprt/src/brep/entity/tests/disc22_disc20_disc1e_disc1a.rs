use super::super::disc22_disc20_disc1e_disc1a_disc18_disc12_disc10_disc04_face_root_body;
...
fn keyed_lattice_owns_directly_linked_faces() {
    let records = lattice();
    let bodies = disc22_disc20_disc1e_disc1a_disc18_disc12_disc10_disc04_face_root_body(
        &index_records(&records),
    );
    let [body] = bodies.as_slice() else {
        panic!("one disc22-disc20-disc1e-disc1a face-root body");
    };
...
#[test]
fn keyed_lattice_uses_the_face_key_when_direct_companion_link_is_stale() {
...
#[test]
fn keyed_lattice_rejects_ambiguous_face_key_fallback() {
    let mut records = lattice();
    records
        .iter_mut()
        .find(|record| record.attr == 20)
        .expect("first canonical face")
        .refs[1] = 999;
    records.push(record(32, 0x1c, [100, 42, 1, 1, 1, 1]));

    assert!(
        disc22_disc20_disc1e_disc1a_disc18_disc12_disc10_disc04_face_root_body(&index_records(
            &records
        ))
        .is_empty()
    );
