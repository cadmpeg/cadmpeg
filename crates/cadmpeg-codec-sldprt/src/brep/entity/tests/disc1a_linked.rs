...
use super::super::{
    ...
    disc1a_disc16_disc14_disc12_disc0e_disc04_face_root_body,
    ...
    disc1e_disc1a_disc18_disc14_disc04_face_root_body,
    disc1e_disc1a_disc18_disc14_disc12_disc04_face_root_body,
    disc1e_disc1c_disc14_disc0e_face_root_body, disc1e_disc1c_disc16_disc14_disc0e_face_root_body,
...
#[test]
fn disc1a_disc16_disc14_disc12_disc0e_disc04_chain_owns_disc0c_faces() {
    let records = lattice_with_use(
        &[
            (0x1a, 2),
            (0x16, 2),
    ...
    let bodies = disc1a_disc16_disc14_disc12_disc0e_disc04_face_root_body(&index_records(&records));
    let [body] = bodies.as_slice() else {
        panic!("one disc1a-disc16-disc14-disc12-disc0e-disc04 body");
    };
    assert_eq!(body.attr, 10);
    assert_eq!(body.regions[0].shells[0].attr, 11);
    assert!(body.refs.contains(&20) && body.refs.contains(&21));
...
#[test]
fn disc1e_disc1a_disc18_disc14_disc12_disc04_chain_owns_disc0e_faces() {
    let mut records = lattice_with_use(
        &[
            (0x1e, 2),
            (0x1a, 2),
    ...
    records[6].refs[1] = 60;
    records[8].refs[2] = 60;
    records.push(record(60, 0x10, [1, 30, 20, 1, 1, 1]));
    records.push(flo4(50, 0x1c, [1, 1, 1, 1, 1, 1]));
    let bodies = disc1e_disc1a_disc18_disc14_disc12_disc04_face_root_body(&index_records(&records));
    let [body] = bodies.as_slice() else {
        panic!("one disc1e-disc1a-disc18-disc14-disc12-disc04 body");
    };
    assert_eq!(body.attr, 10);
    assert_eq!(body.regions[0].shells[0].attr, 12);
    assert!(
        body.refs.contains(&20)
            && body.refs.contains(&21)
            && body.refs.contains(&50)
            && body.refs.contains(&60)
    );

    let mut broken_bridge = records;
    broken_bridge[12].refs[2] = 21;
    assert!(
        disc1e_disc1a_disc18_disc14_disc12_disc04_face_root_body(&index_records(&broken_bridge))
            .is_empty()
    );
}
