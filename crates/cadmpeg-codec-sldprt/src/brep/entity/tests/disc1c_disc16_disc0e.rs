use super::super::{
    disc1c_disc16_disc0e_face_root_body, disc1c_disc16_disc14_disc04_face_root_body,
};
...
#[test]
fn disc14_disc04_terminal_lattice_owns_disc0c_faces() {
    let records = [
        flo2(10, 0x1c, [3, 1, 11, 1, 1, 1]),
        flo2(11, 0x18, [3, 10, 12, 1, 1, 1]),
        flo2(12, 0x16, [3, 11, 13, 1, 1, 1]),
        ...
        record(20, 0x0c, [1, 30, 1, 1, 1, 1]),
        record(21, 0x0c, [1, 31, 1, 1, 1, 1]),
        record(30, 0x12, [1, 40, 20, 1, 1, 1]),
        record(31, 0x12, [1, 41, 21, 1, 1, 1]),
        flo4(40, 0x1a, [1, 1, 30, 1, 1, 1]),
        flo4(41, 0x1a, [1, 1, 31, 1, 1, 1]),
    ];
    let by_attr = index_records(&records);

    let bodies = disc1c_disc16_disc14_disc04_face_root_body(&by_attr);
    let [body] = bodies.as_slice() else {
        panic!("one disc1c-disc16-disc14-disc04-face-root body");
    };
    assert_eq!(body.attr, 10);
    assert_eq!(body.regions[0].shells[0].attr, 12);
    assert!(body.refs.contains(&20) && body.refs.contains(&21));
    ...
    let mut broken_chain = records;
    broken_chain[5].refs[2] = 0;
    assert!(disc1c_disc16_disc14_disc04_face_root_body(&index_records(&broken_chain)).is_empty());
}
