...
fn bodies(entities: &[EntityRecord]) -> (Vec<BodyRecord>, usize) {
    ...
    if out.is_empty() {
        out.extend(disc1e_disc1c_disc1a_disc14_disc12_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(disc1e_disc1c_disc1a_disc16_disc12_face_root_body(&by_attr));
    }
    if out.is_empty() {
    ...
    if out.is_empty() {
        out.extend(disc16_disc0e_disc20_disc14_disc04_face_root_body(&by_attr));
    }
    if out.is_empty() {
    ...
    if out.is_empty() {
        out.extend(disc20_disc1c_disc1a_disc14_disc12_disc10_disc0e_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(disc20_disc12_disc1e_disc1c_disc18_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(disc20_disc12_disc1e_disc1c_disc10_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(disc20_disc12_disc1e_disc1c_disc04_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(disc20_disc12_disc1e_disc1c_disc14_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(disc20_disc10_disc1e_disc1c_disc18_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(disc16_disc12_disc1a_disc14_disc10_face_root_body(&by_attr));
    }
    if out.is_empty() {
    ...
    if out.is_empty() {
        out.extend(disc1e_disc1a_disc18_disc14_disc10_disc0e_disc04_face_root_body(&by_attr));
    }
    if out.is_empty() {
    ...
    if out.is_empty() {
        out.extend(disc22_disc18_disc20_disc1e_disc04_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(disc22_disc16_disc20_disc1e_disc04_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(disc22_disc12_disc20_disc1e_disc16_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(disc22_disc16_disc1e_disc1c_disc18_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(disc22_disc1a_disc20_disc1e_disc16_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(disc18_disc1a_disc20_disc16_disc14_face_root_body(&by_attr));
    }
    if out.is_empty() {
    ...
    if out.is_empty() {
        out.extend(disc22_disc20_disc1e_disc1a_disc18_disc16_face_root_body(
            &by_attr,
        ));
    }
    if out.is_empty() {
        out.extend(disc22_disc20_disc1a_disc18_disc10_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(
    ...
    if out.is_empty() {
        out.extend(
            disc28_disc1e_disc1c_disc18_disc16_disc14_keyed_population_face_root_body(&by_attr),
        );
    }
    if out.is_empty() {
        out.extend(
            disc2a_disc26_disc24_disc1e_disc1c_disc18_keyed_population_face_root_body(&by_attr),
        );
    }
    if out.is_empty() {
        out.extend(
    ...
    if out.is_empty() {
        out.extend(
            disc1c_disc1a_disc16_disc14_disc12_disc10_tail_reciprocal_face_root_body(&by_attr),
        );
    }
    if out.is_empty() {
        out.extend(
    ...
    if out.is_empty() {
        out.extend(disc22_disc20_disc1e_disc16_disc14_disc10_disc0e_face_root_body(&by_attr));
    }
    if out.is_empty() {
    ...
    if out.is_empty() {
        out.extend(
            disc22_disc1c_disc1a_disc18_disc16_disc14_disc12_disc0e_face_root_body(&by_attr),
        );
    }
    if out.is_empty() {
    ...
    if out.is_empty() {
        out.extend(
            disc1b_disc1d_disc19_disc15_disc13_disc11_disc0f_direct_face_root_body(&by_attr),
        );
    }
    if out.is_empty() {
        out.extend(
    ...
    if out.is_empty() {
        out.extend(disc1c_disc16_disc24_disc1a_disc18_face_root_body(&by_attr));
    }
    if out.is_empty() {
    ...
    if out.is_empty() {
        out.extend(disc20_disc1a_disc18_disc16_disc14_disc12_disc04_keyed_face_root_body(&by_attr));
    }
    if out.is_empty() {
    ...
    if out.is_empty() {
        out.extend(disc20_disc1e_disc1c_disc18_disc14_disc10_disc0e_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(
    ...
    if out.is_empty() {
        out.extend(disc1e_disc1c_disc1a_disc16_disc10_disc0e_disc04_face_root_body(&by_attr));
    }
    if out.is_empty() {
    ...
    if out.is_empty() {
        out.extend(disc26_disc1c_disc24_disc22_disc16_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(disc1e_disc1c_disc14_disc0e_face_root_body(&by_attr));
    }
    if out.is_empty() {
        out.extend(disc1e_disc1a_disc16_disc14_disc12_disc10_disc0c_face_root_body(&by_attr));
    }
    if out.is_empty() {
    ...
    if out.is_empty() {
        out.extend(disc1e_disc18_disc16_disc14_disc0e_disc04_face_root_body(
            &by_attr,
        ));
    }
    if out.is_empty() {
...
fn disc20_disc1a_disc18_disc16_disc14_disc12_disc04_keyed_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body(
        by_attr,
...
fn disc20_disc1e_disc1c_disc18_disc14_disc10_disc0e_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
...
fn disc1e_disc1c_disc1a_disc14_disc12_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
...
fn disc1e_disc1c_disc1a_disc16_disc12_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_reciprocal_face_links_with_unselected_companions(
        by_attr,
...
fn disc20_disc1c_disc1a_disc14_disc12_disc10_disc0e_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_population_policy(
        by_attr,
...
fn disc20_disc12_disc1e_disc1c_disc10_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc20_disc12_disc1e_disc1c_disc04_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc20_disc12_disc1e_disc1c_disc14_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc20_disc10_disc1e_disc1c_disc18_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc16_disc12_disc1a_disc14_disc10_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc16_disc0e_disc20_disc14_disc04_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
...
fn disc14_disc16_disc1e_disc12_disc10_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc22_disc1a_disc20_disc1e_disc04_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc22_disc18_disc20_disc1e_disc04_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc22_disc16_disc20_disc1e_disc04_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc22_disc12_disc20_disc1e_disc16_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_unselected_canonical_faces_with_options(
        by_attr,
        &[
            (0x0022, 2),
            (0x0012, 2),
...
fn disc22_disc16_disc1e_disc1c_disc18_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc22_disc1c_disc1a_disc18_disc16_disc14_disc12_disc0e_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc1a_disc16_disc1e_disc18_disc14_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc1d_disc15_disc1b_disc19_disc13_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc1b_disc1d_disc19_disc15_disc13_disc11_disc0f_direct_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
...
fn disc1c_disc14_disc1a_disc18_disc10_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc1c_disc16_disc24_disc1a_disc18_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc22_disc1e_disc1c_disc1a_disc18_disc10_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc22_disc20_disc1e_disc1a_disc18_disc16_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc22_disc20_disc1a_disc18_disc10_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_unselected_canonical_faces_with_options(
        by_attr,
...
fn disc22_disc20_disc1e_disc18_disc16_disc0e_disc04_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_unselected_canonical_faces_with_options(
        by_attr,
...
fn disc28_disc1e_disc1c_disc18_disc16_disc14_keyed_population_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_population_face_root_body(
        by_attr,
...
fn disc2a_disc26_disc24_disc1e_disc1c_disc18_keyed_population_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_population_face_root_body(
        by_attr,
...
fn disc20_disc1c_disc1a_disc16_disc14_disc12_disc10_disc0e_disc10_tail_direct_face_use_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_direct_face_use_root_body(
        by_attr,
...
fn disc1c_disc1a_disc16_disc14_disc12_disc10_tail_reciprocal_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_reciprocal_face_links_with_unselected_companions(
        by_attr,
...
fn disc22_disc20_disc1e_disc16_disc14_disc10_disc0e_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_unselected_canonical_faces_with_options(
        by_attr,
...
fn disc18_disc1a_disc20_disc16_disc14_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc26_disc1c_disc24_disc22_disc16_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links_with_unselected_companions(
        by_attr,
...
fn disc1e_disc18_disc16_disc14_disc0e_disc04_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_keyed_face_links(
        by_attr,
...
fn disc1e_disc1a_disc18_disc14_disc10_disc0e_disc04_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_unselected_canonical_faces_with_options(
        by_attr,
...
fn disc1e_disc1c_disc1a_disc16_disc10_disc0e_disc04_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body_with_unselected_canonical_faces_with_options(
        by_attr,
...
fn disc1e_disc1a_disc16_disc14_disc12_disc10_disc0c_face_root_body(
    by_attr: &HashMap<u16, &EntityRecord>,
) -> Vec<BodyRecord> {
    keyed_face_root_body(
        by_attr,
...
fn keyed_use_by_key<'a>(
    ...
    let candidates = by_attr
        ...
        .collect::<Vec<_>>();
    let [use_node] = candidates.as_slice() else {
        return None;
    };
    Some(*use_node)
}
...
fn keyed_companion_use<'a>(
    ...
    let keyed =
        if direct.is_none() && bridged.is_none() && forward_keyed.is_none() && allow_keyed_fallback
        {
            keyed_use_by_key(by_attr, companion, use_disc).map(|use_node| (use_node, None))
        } else {
            None
        };
    let mut selected = None;
...
fn keyed_face_root_body_with_unselected_canonical_faces_with_options(
    by_attr: &HashMap<u16, &EntityRecord>,
    chain_shape: &[(u16, u8)],
    canonical_disc: u16,
    companion_disc: u16,
    ...
    for face in canonical_faces {
        ...
        let Some((companion, companion_bridge)) = keyed_face_companion(
            by_attr,
            face,
            companion_disc,
            canonical_face_bridge,
            KeyedLinkPolicy::ForwardKeyed,
            true,
        ) else {
            unselected_faces.insert(face.attr);
        ...
        let bridged_selected = companion_bridge.is_some();
    ...
    let mut bodies = keyed_face_root_body_with_keyed_face_links_with_population_policy(
        &filtered,
        chain_shape,
        canonical_disc,
        companion_disc,
        ...
        .all(|face| {
            ...
            let use_link_matches = match use_bridge {
                ...
                None => {
                    ...
                    let keyed_fallback_selected = allow_keyed_use_fallback
                        && !forward_keyed_selected
                        && keyed_use_by_key(by_attr, companion, use_disc)
                            .is_some_and(|record| record.attr == use_node.attr);
                    use_node.refs.get(2) == Some(&companion.attr)
                        || forward_keyed_selected
                        || keyed_fallback_selected
                }
            };
...
mod tests {
    ...
    mod disc1a_disc18_disc16_disc12_disc10_disc04_auxiliary;
    mod disc1a_linked;
    ...
    mod disc1e_disc1a_disc18_disc14_disc10_disc0e_disc04;
    mod disc1e_disc1a_disc18_disc14_disc12_disc10_direct_face_use;
    mod disc1e_disc1a_disc18_disc16_disc14_disc12_disc04;
    mod disc1e_disc1c_disc1a_disc12_face;
    mod disc1e_disc1c_disc1a_disc16_disc10_disc0e_disc04;
    mod disc1e_disc1c_disc1a_disc16_disc14_disc12;
    mod disc1f_disc1d_disc1b_disc17_disc15_disc13_disc11_disc0f;
    mod disc20_disc12_disc1e_disc1c_disc18;
    mod disc20_disc18;
    mod disc20_disc1a_disc18;
    mod disc20_disc1a_disc18_disc16_disc14_disc12_disc04_keyed;
    ...
    mod disc22_disc12_disc20_disc1e_disc16;
    ...
    mod disc24_disc20_disc1e_disc1a_disc18_disc16_disc14_disc12_disc10_dangling_terminal_reciprocal_face_use;
