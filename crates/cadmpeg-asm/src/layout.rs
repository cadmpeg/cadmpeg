...
mod token {
    ...
    const ENTITY_REF: u8 = 12;
    /// `IDENT` (`0x0D`). Spec §2.1.
    const IDENT: u8 = 13;
    /// `SUBIDENT` (`0x0E`). Spec §2.1.
    const SUBIDENT: u8 = 14;
    /// `TERMINATOR` (`0x11`). Spec §2.1.
    const TERMINATOR: u8 = 17;
    /// `POSITION` (`0x13`). Spec §2.1.
    const POSITION: u8 = 19;
    /// `VECTOR_3D` (`0x14`). Spec §2.1.
    const VECTOR_3D: u8 = 20;
    /// `ENUM_VALUE` (`0x15`). Spec §2.1.
    const ENUM_VALUE: u8 = 21;
    /// `VECTOR_2D` (`0x16`). Spec §2.1.
    const VECTOR_2D: u8 = 22;
    /// `INT64` (`0x17`). Spec §2.1.
    const INT64: u8 = 23;
}
...
mod body {
    ...
    const CHUNK1_HISTORY_BODY_FLAGS: usize = 16;
    /// Offset of `chunk3_first_lump` (`sab_ref8`). Spec §5.2.
    const CHUNK3_FIRST_LUMP: usize = 34;
    /// Offset of `chunk4_first_wire` (`sab_ref8`). Spec §5.2.
    const CHUNK4_FIRST_WIRE: usize = 43;
    /// Offset of `chunk5_transform` (`sab_ref8`). Spec §5.2.
    const CHUNK5_TRANSFORM: usize = 52;
}
...
mod lump {
    ...
    const RESERVED_SLOT: usize = 27;
    /// Offset of `chunk4_first_shell` (`sab_ref8`). Spec §5.2.
    const CHUNK4_FIRST_SHELL: usize = 43;
    /// Offset of `chunk5_owner_body` (`sab_ref8`). Spec §5.2.
    const CHUNK5_OWNER_BODY: usize = 52;
}
...
mod shell {
    ...
    const CHUNK5_FIRST_FACE: usize = 53;
    /// Offset of `chunk6_wire` (`sab_ref8`). Spec §5.2.
    const CHUNK6_WIRE: usize = 62;
    /// Offset of `chunk7_owner` (`sab_ref8`). Spec §5.2.
    const CHUNK7_OWNER: usize = 71;
}
...
mod vertex {
    ...
    const CHUNK3_OWNING_EDGE: usize = 36;
    /// Offset of `chunk4_index_flag` (`sab_ref8`). Spec §5.2.
    const CHUNK4_INDEX_FLAG: usize = 45;
    /// Offset of `chunk5_point` (`sab_ref8`). Spec §5.2.
    const CHUNK5_POINT: usize = 54;
}
