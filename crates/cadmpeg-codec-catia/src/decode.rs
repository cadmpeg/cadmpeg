...
fn finish_decode(
    ...
    let relation_expression_instance_count = native
        .entity_records
        .iter()
        .filter(|record| record.relation_expression_instance.is_some())
        .count();
    let instanced_relation_expression_count = native
        .entity_records
        .iter()
        .filter_map(|record| record.relation_expression_instance.as_ref())
        .map(|instance| instance.expression.as_str())
        .collect::<HashSet<_>>()
        .len();
    ...
    report.coverage.extend([
        ...
        (
            "decoded_relation_expression_instance_count".to_string(),
