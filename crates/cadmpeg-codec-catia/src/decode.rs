...
fn finish_decode(
    ...
    let atom_entity_suffix_value_count = native
        ...
        .count();
    let fixed_width_atom_entity_suffix_value_count = native
        .entity_records
        .iter()
        .filter(|record| {
            record.suffix_value.as_ref().is_some_and(|value| {
                matches!(
                    value.payload,
                    crate::native::CatiaEntitySuffixPayload::FixedWidthAtom { .. }
                )
            })
        })
        .count();
    ...
    report.coverage.extend([
        ...
        (
            "decoded_fixed_width_atom_entity_suffix_value_count".to_string(),
            fixed_width_atom_entity_suffix_value_count,
        ),
        (
