...
fn decode_reports_structurally_typed_unresolved_b5_faces() {
    ...
    let graph =
        crate::families::b5::graph::parse(&stream, &mut None).expect("typed unresolved face graph");
...
fn decode_reports_typed_distinct_surface_b5_faces() {
    ...
    let graph =
        crate::families::b5::graph::parse(&stream, &mut None).expect("typed multi-surface graph");
    assert_eq!(graph.face_records.len(), 2);
    assert_eq!(graph.faces.len(), 1);
