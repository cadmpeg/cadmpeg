...
| --- | --- | --- | --- | --- |
| `container` | `work`, `alloc_bytes` (migrated) | `PER_EXTENT_GRAPH_BYTES` (256) per registered extent + Concat copy (both `alloc_bytes`) | whole-image bytes charged once by `scan_view` before the directory walk | migrated: directory scan charged as work up front; extent-graph footprint charged in `register_extent_spaces`; Concat copy ~= sum of MainDataStream + SurfacicReps extents charged to `alloc_bytes` (reason `derived_concat`, doc §4.5) by `begin_derived_space`; directory accumulators now capped `Vec::new`+push, no `with_capacity` |
