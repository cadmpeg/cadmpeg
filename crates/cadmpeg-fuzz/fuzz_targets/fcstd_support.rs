...
pub(crate) const MAX_MUTATION_BYTES: usize = 1 << 20;
...
pub(crate) fn bounded(data: &[u8]) -> &[u8] {
    &data[..data.len().min(MAX_MUTATION_BYTES)]
}

pub(crate) const EMPTY_DOCUMENT: &[u8] = br#"<Document SchemaVersion="4" FileVersion="1"><Objects Count="0"/><ObjectData Count="0"/></Document>"#;

pub(crate) fn shape_document(entry: &str) -> Vec<u8> {
...
pub(crate) fn decode(bytes: Vec<u8>) {
