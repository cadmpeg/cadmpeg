...
`source` is absent when the document has no source-container metadata. A present source block contains `format`, `attributes`, and `dialect`. `dialect` is always serialized. It is `null` for an unclassified source and otherwise contains the primary source `DialectMatch`, including its dialect id, declarations, instance, and admission. Its dialect namespace equals `source.format`. Nested and carried format layers remain in the container and decode reports; the CADIR source block retains only the primary layer.
