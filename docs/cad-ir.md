...
`native.f3d` and `native.sldprt`, when present, each contain `version: 1`. `native.nx` contains `version: 2`. Fusion native data includes ACT, Design, persistent-reference, sketch-link, construction-recipe, and ASM-history records. SOLIDWORKS native data includes feature histories and feature-input lanes. NX native data includes OM class and field declarations, externally bounded object records, framed strings, ordered tagged-reference occurrences, grouped persistent handles, arrangements, and expressions bound to their owning object records.
