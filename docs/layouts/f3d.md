...
Covers the two ASM stream headers (§3), the SAB tag inventory (§4.1), the
fixed-size Fusion-ASM topology records (§6.2, §6.3), the analytic geometry
carriers (§7.2, §7.3) as ordered token slots, and the two fixed Design-segment
headers (§8.1). Procedural spline carriers are variable-length token graphs and
...
| --- | ---- | ------: | ------- | ---- |
| `0x02` | CHAR | 1 B | unsigned 8-bit | §4.1 |
| `0x03` | SHORT | 2 B | signed 16-bit | §4.1 |
| `0x04` | LONG | variable | signed int, 32 or 64-bit per the header width | §4.1 |
| `0x05` | FLOAT | 4 B | IEEE float32 | §4.1 |
| `0x06` | DOUBLE | 8 B | IEEE float64 | §4.1 |
| `0x0A` | TRUE | 0 B | logical true; a data token, not a terminator | §4.1 |
| `0x0B` | FALSE | 0 B | logical false / sentinel | §4.1 |
| `0x0C` | ENTITY_REF | variable | RecordTable index; `ref_size` wide | §4.1 |
| `0x0D` | IDENT | variable | record/class name token (leaf); one length byte plus N name bytes | §4.1 |
| `0x0E` | SUBIDENT | variable | base-class name token; one length byte plus N name bytes | §4.1 |
| `0x11` | TERMINATOR | 0 B | end of the current top-level record | §4.1 |
| `0x13` | POSITION | 24 B | 3D point, three f64; centimetres | §4.1 |
| `0x14` | VECTOR_3D | 24 B | 3D vector, three f64 | §4.1 |
| `0x15` | ENUM_VALUE | variable | enumeration or secondary integer; `ref_size` wide | §4.1 |
| `0x16` | VECTOR_2D | 16 B | 2D (u,v) | §4.1 |
| `0x17` | INT64 | 8 B | AutoCAD int64 attribute value; eight bytes in either header width | §4.1 |

## `asmheader_binaryfile8`

Spec §3 · layout: byte offsets · size: 47 B
...
- `docs/formats/f3d.md` — The declared 47-byte size is the spec's own stated start of the string region.

## `asmheader_binaryfile4`

Spec §3 · layout: byte offsets · size: 31 B
...
- `docs/formats/f3d.md` — The declared 31-byte size is the spec's own stated start of the string region.

## `body`

Spec §6.2 · layout: byte offsets · size: 61 B
...
Spec §6.2 · layout: byte offsets · size: 80 B
...
Spec §6.2 · layout: byte offsets · size: 81 B
...
Spec §6.2 · layout: byte offsets · size: 100 B
...
Spec §6.2 · layout: byte offsets · size: 98 B
...
Spec §6.2 · layout: byte offsets · size: 63 B
...
Spec §6.3 · layout: byte offsets · size: 60 B
...
Spec §7.2 · layout: ordered slots (no stated byte offsets) · size: not stated
...
Spec §7.2 · layout: ordered slots (no stated byte offsets) · size: 161 B
...
Spec §7.2 · layout: ordered slots (no stated byte offsets) · size: 134 B
...
Spec §7.2 · layout: ordered slots (no stated byte offsets) · size: 142 B
...
Spec §7.3 · layout: ordered slots (no stated byte offsets) · size: 115 B
...
Spec §7.3 · layout: ordered slots (no stated byte offsets) · size: 130 B
...
Spec §8.1 · layout: byte offsets · size: 11 B

The 11-byte size is the spec's own "eleven-byte indexed header". §8.1 states the segment's integers are little-endian ("a nonempty contiguous sequence of little-endian i32 values").
...
Spec §8.1 · layout: byte offsets · size: 58 B
...
| Area | Spec | Reason |
| ---- | ---- | ------ |
| Procedural intcurves and spline surfaces (§7.3-§7.6) | §7.3 | Variable-length token graphs with recursive subtypes, revision gates, and conditional members. The spec states slot order in prose but no widths, offsets, or totals, so no arithmetic can close. |
| ASM header trailing metadata sequence (§3) | §3 | The spec states outright that the remaining header is a sequence rather than a fixed-offset structure. |
| Transform record (§6.2) | §6.2 | The spec writes the matrix extent as `13×f64 (@+18..117)`, which reads either as thirteen 9-byte DOUBLE chunks starting at +18 or as a byte range +18..117. Neither reading plus the three trailing enum bytes reaches the declared 142 B, so no offset row can be stated without choosing between them. |
| Container, manifest, and history-partition layers (§1, §2) | §1.3 | ZIP entries, `Manifest.dat` text grammar, and length-delimited delta records; no fixed byte-offset structure is stated. |
