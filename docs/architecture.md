...
CADIR input bypasses codec detection and parses directly into `CadIr`. The parser accepts exactly IR version 5, including its required `subds` arena and `byte_ledger`. Geometry exports are refused when a source decode transferred no geometry unless `--allow-empty` is set.
...
| `cadmpeg-ir`           | Layered version 5 IR model, canonical JSON, free-carrier source associations, source-byte accounting, sparse provenance and exactness, native namespaces, structural diff, validation, codec traits, and report types. |
...
| `cadmpeg-codec-creo`   | Creo `#UGC:2`/PSB section and token decode, prototype and loop structure, opaque `VisibGeom` preservation, and derived `ActDatums` reference-plane carriers; no model B-rep transfer.                          |
