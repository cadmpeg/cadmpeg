...
- **Tessellation: None.**
- **Design intent: Partial.** A closed formula graph with `LENGTH`, `ANGLE`, `Real`/`R`, or `Integer`/`I` scalar types transfers its resolved outputs and every declared, uniquely bound scalar root as document parameters. Formula outputs retain their defining expression when consumed downstream, including typed results without a saved evaluation. The source formula text and first-occurrence dependency order remain exact; evaluated lengths use millimetres, angles use radians, real values are dimensionless, and integer values require an exact signed 64-bit integral scalar. Generated root literals are marked derived. Cyclic components, unresolved output identities, ambiguous or incomplete bindings, string values, features, sketches, constraints, configurations, and incomplete relation graphs remain native.
