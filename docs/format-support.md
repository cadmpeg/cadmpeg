...
- **Design intent: Partial.** Native revolution, sum-surface, polycurve, and extrusion constructions transfer with exact solved carriers. Built-in history records transfer as ordered native operations with command identity, command version, scalar and transform values, object antecedents and descendants, and unambiguous producer dependencies. Object-reference, embedded-geometry, polyedge, and SubD-chain history values remain bounded native parameters. Dimensions, hatches, annotations, and application-specific userdata remain opaque.
