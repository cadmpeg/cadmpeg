...
- **Topology: Partial.** The body, shell, face, loop, fin, edge, and vertex graph attaches when framing and references resolve. Non-null shell BODY and REGION references supply ownership identity when either record is omitted; shell layouts carry either a null or terminal-FACE cache in their final field. Exact-key BODY, SHELL, FACE, LOOP, FIN, EDGE, VERTEX, REGION, POINT, LINE, CIRCLE, ELLIPSE, PLANE, CYLINDER, CONE, SPHERE, TORUS, B_SURFACE, and B_CURVE deltas replacements and tombstones use the first current snapshot for each key in adjacent equal-schema pairs. Unmatched tombstone relations remain unresolved.
- **Tessellation: None.**
