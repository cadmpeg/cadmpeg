...
- **Topology: Partial.** Standard-nested and FBB-only files emit a connected body, shell, face, loop, coedge, edge, and vertex graph when trim, support, and endpoint assignment resolve. Reference-closed E5 graphs emit the same connected common-IR topology. Zero-entity topology remains native because its support-occurrence, vertex-incidence, body, and shell registries are unresolved. Incomplete endpoint quotients and owner graphs decline atomically to disconnected geometry.
- **Tessellation: None.**
