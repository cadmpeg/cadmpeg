...
- **Geometry: Partial for archive 50/60/70/80.** Points, point clouds, lines, arcs, polylines, polycurves, NURBS curves, NURBS surfaces, plane surfaces, clipping-plane carriers, revolution surfaces, sum surfaces, meshes, extrusions, and SubD control cages transfer into typed IR. Lengths and length-valued tolerances convert to millimeters; angles, unit vectors, knot values, UV values, and relative tolerances remain unscaled. Unsupported classes and future payload versions remain native unknown records with bounded bytes or a complete-record length and digest. V3/V4 geometry remains unknown.
