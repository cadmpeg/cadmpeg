...
- **Tessellation: None.**
- **Design intent: Partial.** A closed formula with `LENGTH`, `ANGLE`, `Real`/`R`, or `Integer`/`I` scalar types transfers its resolved output and every declared, uniquely bound input as document parameters. The source formula text and first-occurrence dependency order remain exact; evaluated lengths use millimetres, angles use radians, real values are dimensionless, and integer values require an exact signed 64-bit integral scalar. Generated input literal text is marked derived. Unresolved output identities, ambiguous or incomplete bindings, string values, features, sketches, constraints, configurations, and incomplete relation graphs remain native.
