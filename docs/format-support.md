...
- **Product structure: Inspect.** End-anchored EXTREFSTREAM child-part names and paths transfer as ordered typed native records with exact byte offsets and also appear in source dependency metadata. Assembly graph instances, placements, and constraints remain open.
