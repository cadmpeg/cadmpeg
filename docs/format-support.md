...
- **Tessellation: Partial.** Embedded JT 9 shape-LOD segments decode lossless and uniformly quantized coordinates, lossless and Deering normals, reconstruct topological dual-mesh connectivity and corner attribute indices, resolve logical shape-node to late-loaded payload ownership, and transfer nonnegative-group triangles with seam-preserving normals in document units. Body association, texture coordinates, colors, and transformed scene instances remain unresolved.
