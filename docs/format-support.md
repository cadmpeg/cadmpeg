...
- **Tessellation: None.**
- **Design intent: Partial.** A closed single-input formula with `LENGTH`, `ANGLE`, `Real`/`R`, or `Integer`/`I` scalar types transfers its resolved output and uniquely bound input as document parameters. The source formula text and parameter dependency remain exact; evaluated lengths use millimetres, angles use radians, real values are dimensionless, and integer values require an exact signed 64-bit integral scalar. Generated input literal text is marked derived. Unresolved output identities, ambiguous bindings, multi-input signatures, string values, features, sketches, constraints, configurations, and incomplete relation graphs remain native.
