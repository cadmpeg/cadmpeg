...
- **Tessellation: None.**
- **Design intent: Partial.** A formula with `LENGTH`, `ANGLE`, `Real`/`R`, or `Integer`/`I` scalar types transfers every declared, uniquely bound scalar input independently of whether its result identity resolves. A closed formula graph also transfers its resolved outputs. Formula outputs retain their defining expression when consumed downstream, including typed results without a saved evaluation. The source formula text and first-occurrence dependency order remain exact; evaluated lengths use millimetres, angles use radians, real values are dimensionless, and integer values require an exact signed 64-bit integral scalar. Generated root literals are marked derived. Cyclic components, ambiguous or incomplete input bindings, string values, features, sketches, constraints, configurations, unresolved outputs, and incomplete relation graphs remain native.
