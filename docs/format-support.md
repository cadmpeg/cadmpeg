...
- **Tessellation: None.**
- **Design intent: Partial.** A formula with `LENGTH`, `ANGLE`, `Real`/`R`, or `Integer`/`I` scalar types transfers each uniquely bound scalar input independently of its sibling bindings and result closure. A closed formula graph also transfers its resolved outputs when its arithmetic expression parses completely, evaluates with valid dimensions, and agrees exactly with any stored result. Formula outputs retain their defining expression when consumed downstream, including typed results without a saved evaluation. The source formula text and first-occurrence dependency order remain exact; evaluated lengths use millimetres, angles use radians, real values are dimensionless, and integer values require an exact signed 64-bit integral scalar. Generated root literals are marked derived. Cyclic components, ambiguous input bindings, string values, sketches, features, constraints, configurations, unresolved outputs, and incomplete relation outputs remain native.
