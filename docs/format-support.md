...
- **Presentation and metadata: Inspect.** Length-bounded Parasolid attribute-class declarations retain stream-local class and field-record identities, exact printable class name, declared field count, ordered catalog references, variant header words, and inflated-stream offset. Attribute field payloads, instances, ownership bindings, colors, materials, and other presentation values remain unresolved.
