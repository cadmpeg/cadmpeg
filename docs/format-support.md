...
- **Design intent: Partial.** Native revolution, sum-surface, polycurve, extrusion, hatch, and NURBS-cage constructions transfer with exact solved carriers. Built-in history records transfer as ordered native operations with command identity, command version, scalar and transform values, object antecedents and descendants, and unambiguous producer dependencies. Linear, angular, radial, and diameter dimensions transfer with defining plane-space geometry, display state, text, style identity, detail identity, distance scale, and evaluated length or angle parameters. Object-reference, embedded-geometry, polyedge, and SubD-chain history values remain bounded native parameters. General annotations and application-specific userdata remain opaque.
