...
- **Tessellation: None.**
- **Design intent: Partial.** Feature-history record areas project ordered neutral features and retain their byte-exact operation labels and four object-index slots. Unite, subtract, and intersect operations project as typed body-combine features with ordered native target/tool selections. Other named operations project as native features until their inputs are complete. Typed numeric expressions retain object identity, owning OM record, name, exact expression text, declared millimeter or degree unit, ordered exact-name parameter dependencies, and evaluated values for context-free and acyclic same-unit dependency arithmetic. Named arrangements retain ordered configuration names and default state. Complete feature boundaries and inputs, object-index-to-body binding, sketch geometry, constraints, and other operation semantics remain open.
