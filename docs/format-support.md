...
- **Tessellation: None.**
- **Design intent: Partial.** A formula with a typed `LENGTH` input and result transfers its resolved output and every uniquely bound scalar input as document parameters when the complete relation graph closes. The source formula text and ordered parameter dependencies remain exact; evaluated values use millimetres. Generated input literal text is marked derived. Unresolved output identities, ambiguous bindings, other type signatures, features, sketches, constraints, configurations, and incomplete relation graphs remain native.
