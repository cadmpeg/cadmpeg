...
- **Procedural write invariant:** One solved curve has at most one procedural construction. Exact-cache, compound, helix, vector-offset, surface-offset, non-null spring, subset, ranged projection, silhouette/taper-silhouette, two- and three-surface intersection, two-sided offset, and prefix-only blend/surface/parametric/skin definitions with paired analytic/NURBS surface and NURBS UV supports serialize with their native construction fields, child carriers, role fields, and solved caches. Null-support spring layouts, early-close projections, scalar offset, and blend-spine definitions remain rejected source-less until their external or conditional fields can be emitted without semantic loss. No typed construction is reduced silently to a cache-only curve.
