...
| Rhino `.3dm` (archive 50/60/70/80)         | **L3 tested**  | SubD, native constructions and history records, evaluated dimensions                                  |
...
- **Geometry: Partial for archive 50/60/70/80.** Points, point clouds, lines, arcs, polylines, polycurves, curves on surfaces with parameter/support carriers, NURBS curves, NURBS surfaces, NURBS cages and morph controls, plane surfaces, clipping-plane carriers, revolution surfaces, sum surfaces, hatches with placed boundary loops, detail-view boundaries, meshes, extrusions, and SubD control cages transfer into typed IR. Registered legacy identities for polycurve, NURBS curve, NURBS surface, revolution surface, and Brep use the same typed readers as their current identities. Lengths and length-valued tolerances convert to millimeters; angles, unit vectors, knot values, UV values, relative tolerances, and hatch pattern scale remain unscaled. Unsupported classes and future payload versions remain native unknown records with bounded bytes or a complete-record length and digest. V3/V4 geometry remains unknown.
...
- **Design intent: Partial.** Native revolution, sum-surface, polycurve, persistent polyedge-reference, curve-on-surface, extrusion, hatch, NURBS-cage, and curve/surface/cage morph constructions transfer with exact solved carriers or complete persistent-reference semantics. Morph operations retain captive identities, localizers, tolerances, preview mode, and structure-preservation mode. Built-in history records transfer as ordered native operations with command identity, command version, scalar and transform values, complete object selections, polyedge constructions, SubD edge chains, object antecedents and descendants, and unambiguous producer dependencies. Modern and legacy V5 linear, angular, radial, diameter, and ordinate dimensions transfer with defining plane-space geometry, display state, text, style identity, forced arrow state, detail identity, distance scale, and evaluated length or angle parameters. Center marks retain their common dimension state and mark radius without fabricating a measured value. Embedded point, point-cloud, curve, surface, Brep, mesh, SubD, extrusion, NURBS-cage, morph-control, hatch, detail-view, dimension, center-mark, and polyedge-reference history values transfer as unit-normalized geometry, complete topology, and exact construction semantics. General annotations and application-specific userdata remain opaque.
...
- **Presentation and metadata: Partial.** Document units and tolerances, application properties, notes, current selectors, layers, object identity, names, effective color and visibility, dimension text and display state, hatch pattern references and transforms, and source associations transfer. Rendering attributes and nested material references are structurally framed but not transferred as typed appearance data. Materials, line styles, general annotations, plugin classes, and plugin userdata remain retained rather than typed.
