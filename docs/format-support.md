...
- **Geometry: Partial.** Analytic surfaces and curves, cached NURBS carriers, parameterizations, signed radii, point-degenerate curves, exact-cache, compound, helix, vector-offset, subset, projection, two- and three-surface intersection, two-sided offset, and prefix-only blend/surface/parametric/skin constructions with null, analytic, or NURBS surface/UV support pairs transfer. Law, taper, loft, net, sweep, variable-blend, and related families remain incomplete when no solved cache resolves.
...
- **Procedural write invariant:** One solved curve has at most one procedural construction. Exact-cache, compound, helix, vector-offset, subset, ranged projection, two- and three-surface intersection, two-sided offset, and prefix-only blend/surface/parametric/skin definitions with null or paired analytic/NURBS surface and NURBS UV supports serialize with their native construction fields, child carriers, role fields, and solved caches. Early-close projections require an external solved carrier and are rejected source-less; scalar offset and blend-spine definitions remain rejected until their fields can be emitted without semantic loss. No typed construction is reduced silently to a cache-only curve.
