...
- **Design intent: Partial.** Document user parameters, sketch dimensions, and construction inputs transfer with source expressions, canonical evaluated values, dependency identities, local order, and full-fidelity native references. Paired, counted, and null-locus dimension frames retain their ordered sketch operands and role codes. Indexed parameter scopes retain their ordered record-reference tables and transfer as neutral Sketch nodes, typed blind and two-sided Extrude nodes with resolved sketch operands, signed normal direction, ordered counted construction-operand groups, nested operand-identity chains and fixed persistent identities, and ordered counted selection members resolved against persistent sketch geometry when possible, and typed Fillet and Chamfer nodes with dimensional forms and counted groups of ordered edge-recipe operands. Fillet nodes retain each ordered edge group as a separate neutral radius assignment with its tangency weight. ASM history states, Design assignments, sketch-side records, construction recipes, body persistent-ID histories, variable-width face/edge design-reference tags, persistent references, MetaStream identities, and ACT channels transfer. Remaining dimension companion variants, edge-recipe-to-B-rep identity, unresolved Extrude selection identities and Boolean operands, and non-distance Extrude termination modes remain open, so the Fusion feature history is not yet replayable.
