...
- **Geometry: Partial.** Standard-nested and FBB-only files transfer vertices, bridged planes, signed-axis analytic carriers, supported edge curves, consolidated NURBS carriers, uniquely domain-bound constant-offset constructions, exact two-support extrusion constructions, support-bound result/carrier/support surface closure, and fit-free rolling-ball surface jets. Consolidated class-`0x61` records retain their complete counted reference/tail form or long-form prefix, monotone member lane, five persistent references, and scalar. Consolidated revolution records retain their profile allocation identity, right-handed unit axis frame, full-turn angular chart, and profile interval; their neutral carriers remain unresolved until the profile allocation binds to an exact directrix. E5 one-pcurve boundary supports lift analytic isoparametric lines and circles into exact 3D edge carriers and retain general nonplanar pcurves as exact parametric surface-curve constructions. Zero-entity graphs transfer analytic, NURBS, and typed procedural curve carriers. Freeform face aliases and unbridged planes remain unknown.
