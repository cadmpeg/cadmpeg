...
- **Creo Parametric `.prt` (L1 claimed):** container navigation plus partial placed geometry, connected topology, sketches, constraints, definition-scoped dimension parameters, expressions, and typed feature operations. Decoded dimension rows transfer independently of incomplete table tails; table completeness gates only ordinal relation joins. Curve-equation expressions retain case-insensitive bindings and complete scoped dependency symbols, resolve unambiguous decoded dimension values in relation units, evaluate numeric and string values, and execute nested conditional branches without guessing unresolved conditions. The cumulative L2–L6 gates remain incomplete. Read only. See the [coverage contract](formats/creo_prt-coverage.md).
