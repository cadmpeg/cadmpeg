...
- **Geometry: Partial.** Standard-nested files transfer vertices, planes when their bridge records resolve, curved analytic surfaces, supported edge curves, consolidated NURBS carriers, uniquely domain-bound constant-offset constructions, and fit-free rolling-ball surface jets. E5 one-pcurve boundary supports lift analytic isoparametric lines and circles into exact 3D edge carriers and retain general nonplanar pcurves as exact parametric surface-curve constructions. Other layouts transfer subsets of analytic or freeform carriers.
