...
- **Topology: Partial.** Standard-nested and FBB-only files emit a connected body, shell, face, loop, coedge, edge, and vertex graph when trim, support, and endpoint assignment resolve. Reference-closed E5 graphs emit the same connected common-IR topology. Zero-entity topology retains a complete framed-record identity inventory, exact edge-stride references, positional oriented-use pairs, face-local supports, and counted vertex incidences in separate native registries; their cross-registry binding and body/shell ownership remain unresolved. Incomplete endpoint quotients and owner graphs decline atomically to disconnected geometry.
- **Tessellation: None.**
