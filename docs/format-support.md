...
- **Geometry: Partial.** Analytic surfaces and curves, cached NURBS carriers, parameterizations, signed radii, point-degenerate curves, exact, ruled, sum, revolution, and offset spline surfaces, exact-cache curve, compound, deformable, helix, vector-offset, surface-offset, spring, subset, projection, silhouette/taper-silhouette, two- and three-surface intersection, two-sided offset, and prefix-only blend/surface/parametric/skin constructions with null, analytic, or NURBS surface/UV support pairs transfer under both modern and legacy subtype names. Law, loft, net, sweep, variable-blend, and related families remain incomplete when no solved cache resolves.
