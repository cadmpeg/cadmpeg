...
- **Geometry: Partial for archive 50/60/70/80.** Points, point clouds, lines, arcs, polylines, polycurves, NURBS curves, NURBS surfaces, plane surfaces, clipping-plane carriers, revolution surfaces, sum surfaces, meshes, extrusions, and SubD control cages transfer into typed IR. Registered legacy identities for polycurve, NURBS curve, NURBS surface, revolution surface, and Brep use the same typed readers as their current identities. Lengths and length-valued tolerances convert to millimeters; angles, unit vectors, knot values, UV values, and relative tolerances remain unscaled. Unsupported classes and future payload versions remain native unknown records with bounded bytes or a complete-record length and digest. V3/V4 geometry remains unknown.
...
- **Design intent: Partial.** Native revolution, sum-surface, polycurve, and extrusion constructions transfer with exact solved carriers. Built-in history records transfer as ordered native operations with command identity, command version, scalar and transform values, object antecedents and descendants, and unambiguous producer dependencies. Linear, angular, radial, and diameter dimensions transfer with defining plane-space geometry, display state, text, style identity, detail identity, distance scale, and evaluated length or angle parameters. Object-reference, embedded-geometry, polyedge, and SubD-chain history values remain bounded native parameters. Hatches, general annotations, and application-specific userdata remain opaque.
