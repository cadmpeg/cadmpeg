...
- **Design intent: Partial.** Document user parameters, sketch dimensions, and construction inputs transfer with source expressions, canonical evaluated values, dependency identities, local order, and full-fidelity native references. Paired, counted, and null-locus dimension frames retain their ordered sketch operands and role codes. Indexed parameter scopes retain their ordered record-reference tables and transfer as neutral Sketch nodes, typed blind and two-sided Extrude nodes with resolved sketch operands, signed normal direction, and ordered counted selection members, and typed Fillet and Chamfer nodes with dimensional forms and ordered edge-recipe operands. ASM history states, Design assignments, sketch-side records, construction recipes, body persistent-ID histories, variable-width face/edge design-reference tags, persistent references, MetaStream identities, and ACT channels transfer. Remaining dimension companion variants, edge-recipe-to-B-rep identity, Extrude selection-member topology and Boolean operands, and non-distance Extrude termination modes remain open, so the Fusion feature history is not yet replayable.
