...
- **Design intent: Partial.** Document user parameters, sketch dimensions, and construction inputs transfer with source expressions, canonical evaluated values, dependency identities, local order, and full-fidelity native references. Paired, counted, and null-locus dimension frames retain their ordered sketch operands and role codes. Indexed parameter scopes retain their family-local ordinal, current and preceding ASM delta-state identities, ordered record-reference tables, and transfer as neutral Sketch nodes, typed blind and two-sided Extrude nodes when the profile plane is the start and distance is the termination, with resolved sketch operands, signed normal direction, typed join, cut, intersect, or new-body result operation, native profile-plane, offset-profile-plane, and selected-face starts, native one-sided distance, one-sided to-face, and two-sided distance discriminators, typed body/profile/face operand groups, face operands resolved to exact face-recipe envelopes, counted i32 recipe nodes, and active B-rep candidate sets, ordered counted construction-operand groups, nested operand-identity chains and fixed persistent identities, and ordered counted selection members resolved against persistent sketch geometry when possible, and typed Fillet and Chamfer nodes with dimensional forms and counted groups of ordered edge-recipe operands. Fillet nodes retain each ordered edge group as a separate neutral radius assignment with its tangency weight. ASM history states, Design assignments, sketch-side records, construction recipes, body persistent-ID histories, variable-width face/edge design-reference tags, persistent references, MetaStream identities, and ACT channels transfer. Remaining dimension companion variants, recipe-to-B-rep identity, unresolved Extrude selection identities, neutral offset/from-face start and face-termination projection, and face-operand assignment to Extrude starts and terminations remain open, so the Fusion feature history is not yet replayable.
