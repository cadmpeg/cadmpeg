...
- **Design intent: Partial.** Document user parameters transfer with source expressions, canonical evaluated values, dependency identities, and full-fidelity native references. ASM history states, Design assignments, sketch-side records, construction recipes, body persistent-ID histories, variable-width face/edge design-reference tags, persistent references, MetaStream identities, and ACT channels transfer. Owned dimensions and feature inputs remain native until their indexed owner records resolve to neutral constraints and operations. A complete replayable Fusion feature history remains open.
