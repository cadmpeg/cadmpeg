...
- **Tessellation: None.**
- **Design intent: Partial.** A formula with `LENGTH`, `ANGLE`, `Real`/`R`, or `Integer`/`I` scalar types transfers each uniquely bound scalar input independently of its sibling bindings and result closure. A closed formula graph also transfers its resolved outputs. Formula outputs retain their defining expression when consumed downstream, including typed results without a saved evaluation. The source formula text and first-occurrence dependency order remain exact; evaluated lengths use millimetres, angles use radians, real values are dimensionless, and integer values require an exact signed 64-bit integral scalar. Generated root literals are marked derived. An entity record carrying exactly two finite binary64 coordinates transfers as an isolated planar sketch point when its `2DPoint` field relations select exactly one design object carrying a `PRTSketch` field; the sketch placement remains unresolved. The source design fields remain unresolved because this point transfer does not account for their other semantics. Cyclic components, ambiguous input bindings, string values, features, other sketch geometry, constraints, configurations, unresolved outputs, and incomplete relation outputs remain native.
