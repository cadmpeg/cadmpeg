...
- **Container and versions: Partial.** The codec joins the top-level manifest's asset-folder UUID to the matching per-asset manifest, scopes B-rep selection to that design folder, selects its first `.smbh`, falls back to its first `.smb`, and decodes linked Protein, Design, MetaStream, and ACT records. In-memory decoding accepts archives through 1 GiB, individual inflated entries through 512 MiB, and at most 1 GiB total declared inflated content. Entry declarations above these limits are rejected before payload allocation. The authoritative tie-break among multiple B-rep entries of the same class within one active asset folder remains unresolved.
