...
- **Tessellation: None.**
- **Design intent: Partial.** Typed numeric expressions retain object identity, owning OM record, name, exact expression text, declared millimeter or degree unit, ordered exact-name parameter dependencies, and evaluated values for context-free and acyclic same-unit dependency arithmetic. Named arrangements retain ordered configuration names and default state. Feature, sketch, constraint, and history operation semantics remain open.
