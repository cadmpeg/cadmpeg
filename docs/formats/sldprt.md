...
A referenced line-or-circle handle with exactly two incident coordinate-bearing point handles carries a construction line from the first point in marker order to the second. Point incidence may be stored by a link in either direction. Every admissible sketch placement must produce the same two distinct endpoints. The handle retains its native identity on the construction line. Geometry-handle reachability is the undirected transitive closure of dimensional operands and marker links.
...
A sketch placement whose normal is parallel to one world axis supersedes profile-derived coordinate transforms. The marker coordinate pair supplies the two ascending world-coordinate components other than the normal axis. The normal-axis component equals the sketch-origin component. Projection of that model-space point onto the stored sketch u-axis and `normal × u` axis supplies sketch coordinates independently of profile anchors.
