...
The instance operand list is the first scalar record's complete ordered operand-cell list. For every operand tag except `fe 83`, a resolved operand marker is the unique marker that belongs to the scalar's owning sketch feature, has a local identifier equal to the operand-cell address, and belongs to the marker family selected by the operand-cell tag. For `fe 83`, the address is the zero-based ordinal among line-or-circle markers in marker-offset order within the owning feature object.

Operand-cell tags `d6 80`, `7b 83`, and `7c bc` address point or constrained-point handles. Tags `e1 80`, `86 83`, and `87 bc` address line-or-circle handles. Tag `fe 83` addresses a line-or-circle handle used as a circular dimension operand. For local-identifier operands, the operand family disambiguates an identifier reused by geometry and relation markers.
...
Operand tags `837b` and `bc7c` select point or constrained-point markers. Tags `8386` and `bc87` select line-or-circle markers by local identifier. Tag `83fe` selects the same marker family by ordinal. The tag-qualified marker kind disambiguates equal local identifiers before uniqueness is tested.
