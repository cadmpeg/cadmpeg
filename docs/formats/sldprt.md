...
Constraint incidence is bidirectional. A coordinate-bearing line, circle, or arc marker owns a relation marker when one of its resolved local-link cells targets that relation marker. Native constraints retain these reverse owners separately from the relation marker's forward reference-handle operands.
