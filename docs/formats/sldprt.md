...
Point-distance operands select explicit profile loci. Line-distance and angular operands select the profile entity shared by their linked endpoint markers. For three-operand symmetric and at-intersection relations, the operand roster is the unique loci selected by the relation marker's two forward local links plus coordinate-bearing markers whose local link targets the relation marker. A relation with resolved operands and one driving scalar maps to the corresponding neutral distance, horizontal-distance, vertical-distance, angle, radius, or diameter constraint. Relation-marker coordinates do not identify constraint operands. A relation marker without linked local identifiers does not produce a sketch constraint.
