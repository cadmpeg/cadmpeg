...
`ResolvedFeatures` sketch entities begin with `ff ff 1f 00 03`. A little-endian u32 at marker +17 is the entity type: `0` point, `1` curve, `2` arc, `3` constrained point. Marker +48 stores a finite little-endian f64 state value. The coordinate-bearing variant of handle families `0..5` has the 12-byte prefix `ff ff ff ff ff ff ff ff 00 00 80 bf` at marker +5, `1e 00` at marker +64, and two finite little-endian f64 coordinate fields at marker +66 and +74, in metres. The reference-bearing variant stores two little-endian u16 feature-local object identifiers at marker +64 and +66, a little-endian u16 selector at marker +68, zero at marker +70, and little-endian f64 `-1.0` at marker +72. Both identifiers select sketch entities owned by the same feature object as the referencing marker.
...
Keywords `Feature` elements use the `Type` attribute as their operation-family token. `Helix/Spiral`, `Surface-Sweep`, and `Thicken` denote helix, surface-sweep, and face-thickening operations independently of the localized display name in `Name`.
