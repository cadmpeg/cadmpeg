...
**Known.** `step.md:141` through `step.md:143` state that a REFERENCE entry binds a local resource name to a resource URI and that an out-of-file target is an external dependency.
...
**Known.** `step.md:143` identifies out-of-file URI targets as external dependencies. The clear-text exchange structure does not contain an access procedure.
...
**Known.** `step.md:122` through `step.md:126` define identity and reference resolution inside the DATA sections. `step.md:141` through `step.md:143` define local resource bindings and external dependencies.
...
**Known.** `step.md:141` and `step.md:142` state that each REFERENCE entry contains a resource URI. The specification gives no cache-identity rule.
...
**Known.** `step.md:7` through `step.md:9` identify an AP242 BO-Model XML sidecar as an encoding that is distinct from the Part 21 clear-text exchange structure.
...
**Known.** `step.md:122` through `step.md:126` define identity and reference resolution inside the Part 21 DATA sections. The specification gives no cross-encoding composition rule.
...
**Known.** `step.md:7` through `step.md:9` identify a ZIP container as distinct from an uncompressed Part 21 exchange structure. `step.md:19` defines the uncompressed outer grammar.
...
**Known.** `step.md:141` through `step.md:143` define resource names and URIs in a Part 21 REFERENCE section. The specification gives no container-relative resolution rule.
...
**Known.** `step.md:5` through `step.md:9` define support for Part 21 clear text and identify Part 28 XML as a distinct encoding.
...
**Known.** `step.md:96` through `step.md:126` define Part 21 values, records, identities, and references. The specification gives no Part 28 mapping.
...
**Known.** `step.md:5` through `step.md:9` define support for Part 21 clear text and identify Part 26 binary or HDF5 as a distinct encoding.
...
**Known.** `step.md:96` through `step.md:126` define Part 21 values, records, identities, and references. The specification gives no Part 26 mapping.
...
**Known.** `step.md:54` defines the syntax of a user-defined name. `step.md:124` through `step.md:126` require an unknown entity to retain its name, complete spans, and outgoing references.
...
**Known.** `step.md:54` defines the syntax of a user-defined name. `step.md:98` through `step.md:101` define a typed parameter as a name with one parameter.
...
**Known.** `step.md:143` through `step.md:145` define the complete byte boundary of a SIGNATURE section. The specification gives no field grammar for its content.
...
**Known.** `step.md:22` through `step.md:30` place the optional SIGNATURE section after all DATA sections and before the exchange terminator. `step.md:143` through `step.md:145` define the byte boundary of the SIGNATURE section.
...
**Known.** `step.md:143` through `step.md:145` require retention of the complete SIGNATURE byte range. The specification gives no field grammar for the retained content.
...
**Known.** `step.md:143` through `step.md:145` define structural retention only. The specification gives no cryptographic validation conditions.
