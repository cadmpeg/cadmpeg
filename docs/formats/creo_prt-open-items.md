...
- The scalar token grammar inside the four packed `tab_cyl` control-point
  bodies is unspecified. The prototype-adjacent `tab_cyl` instance rows use a
  separate construction from the repeated cubic replay.
