...
| L2 | Incomplete | Extend graph-only NURBS-base OFFSET_SURF saved-carrier identity to periodic, differing-basis, and regular surfaces whose rational derivative bounds do not certify global orientation within the subdivision limit. Close terminal branches without paired FIN-carried SP_CURVE witnesses and cases whose distinct endpoints map to one procedural parameter. |
