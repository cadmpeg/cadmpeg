...
| L2 | Incomplete | Extend graph-only NURBS-base OFFSET_SURF saved-carrier identity to varying-weight, periodic, differing-basis, and regular surfaces whose global orientation is not certified by their derivative control nets. Close terminal branches without paired FIN-carried SP_CURVE witnesses and cases whose distinct endpoints map to one procedural parameter. |
