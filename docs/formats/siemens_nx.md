...
An offset-store expression-object frame is `object_id:compact_index, 00 72 01 c0 20 02 01 c0 45 04 00 80 86 02 01 02 80 a4`. The compact index is non-null and uses the same direct and extended forms. Its value is the persistent object ID of an expression declaration. A frame resolves only when exactly one expression declaration has that object ID. The frame and discriminator lie within one bounded data block; frame order and the compact-index byte offset are retained.
