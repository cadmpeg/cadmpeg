...
For that dependency, a datum-coordinate-system scalar aliases a sketch-point coordinate when its shifted-binary64 encoding occupies the same absolute source bytes as the named-point coordinate encoding. The alias retains the zero-based sketch-coordinate ordinal, datum-coordinate-system scalar identity, and shared value offset. Equal decoded values at different offsets do not establish an alias.
