...
An Int32 Compressed Data Packet Mk. 2 begins with `value_count:u32 LE`; a zero count ends the packet. A nonempty packet continues with a codec byte. Codec 1 contains `code_bit_count:u32 LE` and `ceil(code_bit_count / 32)` little-endian code words consumed most-significant-bit first and decoded by the bit-length codec. Codec 3 contains the same code-text fields, followed by one probability context and a recursive out-of-band packet when escape symbols occur. The probability context begins with a big-endian entry count; its field widths, minimum value, and entries are a byte-aligned MSB-first bit stream. Codec 4 begins with `chop_bits:u8`. A zero chop width continues with one complete recursively encoded packet having the outer value count. A nonzero chop width continues with `bias:i32 LE, span_bits:u8`, then recursive MSB and LSB packets; each nested packet has the outer value count and each reconstructed value is `(lsb | (msb << (span_bits - chop_bits))) + bias`. Recursive depth does not exceed three. An invalid codec, field width, recursion, count, padding bit, or packet bound rejects the packet.
