...
A datum-coordinate-system fixed pair is `0b 02 03 01 03 01 c0 45 04 00 80 86 02 00 03, 30, first:i56 BE, 00, 30, second:i56 BE`. Each signed atom has value `i56 / 2^55`. The pair retains the datum operation and payload identities, exact discriminator, both decoded dimensionless values, both exact seven-byte two's-complement payloads, payload-relative offsets, and absolute source offsets. The frame does not assign an origin, axis, orientation, sketch coordinate, or constraint role to either value. When a known datum-coordinate-system scalar or pair frame starts in the second leading construction block and is truncated by that block boundary, consecutive blocks in the same offset store extend the reconstructed payload exactly through the byte completing that frame. Later bytes in the terminal block do not belong to the reconstructed payload under this rule. An absent successor or a changed partial frame leaves the payload at its two explicitly referenced blocks.
