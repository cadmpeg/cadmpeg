...
When all three coordinate quantization widths are zero, each XYZ component is encoded as an exponent-and-sign Int32 Compressed Data Packet Mk. 2 followed by a mantissa packet. Both vectors have the unique vertex count and use Lag1 prediction. The low nine bits of each reconstructed exponent-and-sign integer and the low 23 bits of its reconstructed mantissa integer form one IEEE 754 binary32 bit pattern. All reconstructed coordinates are finite. A `coordinate_hash:u32 LE` follows the six packets. The decoded coordinate array retains the owning header, ordered XYZ points in metres, coordinate hash, complete encoded byte length, and source offset. A packet failure, count disagreement, nonfinite coordinate, or missing hash rejects all decoded coordinate arrays atomically.
