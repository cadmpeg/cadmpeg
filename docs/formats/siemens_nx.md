...
A feature input has a column target when exactly one linked or target-index row in a complete composite table addresses the input block in row slot zero. The relation retains the input, operation and header slot, row grammar and identity, composite table, target block, and target-token offset. Zero or multiple complete-table target rows leave the input without a column target. Trailing-lane and ordinary index-row reuse cannot establish a column target.
