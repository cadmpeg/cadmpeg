...
Before its unique `Hole_` template string, a `SIMPLE HOLE` payload may carry exactly four marker-`30` shifted-binary64 scalars. When the first scalar is bitwise equal to the third and the second is bitwise equal to the fourth, the two pairs redundantly witness one ordered two-dimensional placement in model millimeters. Any other scalar count or unequal pair rejects the placement atomically. The placement remains native two-dimensional data until its model-space coordinate frame is resolved.

Each complete placement witness is followed immediately by two tagged object indices. `f0,lo` encodes an ordinal below 256. `f1,hi,lo` encodes a big-endian ordinal of at least 256. Both pairs address blocks by direct ordinal in the offset store that owns the operation-header input blocks. The four indices resolve atomically: the operation inputs must select one store and every addressed ordinal must exist in that store. The first and repeated pairs retain their order independently because they identify the native construction data accompanying each redundant placement witness.
...
A datum-plane descriptor block is exactly 40 bytes: a non-empty lowercase hexadecimal identity followed by a `?` delimiter and the remaining descriptor suffix. Descriptor references resolve within the operation-selected offset store. The typed descriptor retains its owning plane header, descriptor-lane ordinal, resolved block, identity, exact delimiter-prefixed suffix, and absolute block offset. A malformed identity, missing delimiter, or non-40-byte block leaves the descriptor untyped.
