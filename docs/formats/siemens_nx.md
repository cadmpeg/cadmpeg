...
A second self-framed row is `02 0b, first:compact_index, 93 8c, discriminator:u8, target:compact_index, ff ff 90 fe, index[3], 00 47, flag:u8, 04 01 c0 44 04 00`. The discriminator is `16`, `17`, or `18`; the flag is `03` or `07`; and `target` plus all three trailing indices are non-null in-range block ordinals in the same offset-only store. The unresolved leading index, discriminator, ordered four-block lane, flag, and token offsets are retained. A changed marker, other discriminator or flag, null or out-of-range block index, or incomplete row rejects the candidate atomically.
