...
A JT 9 tri-strip shape-LOD element has object-type identifier `ab10dd10-c82a-d111-9b6b-0080c7bb5997`. Its body begins `base_version:u16 LE = 1, vertex_version:u16 LE = 1, vertex_bindings:u64 LE, topological_mesh_version:u16 LE`, where the topological-mesh version is 1 or 2. The remaining bytes are the complete compressed mesh representation. The header retains both fixed versions, the exact binding mask, topological-mesh version, remaining byte length and hash, and source offset. A truncated header or unsupported version rejects all tri-strip headers atomically.
