...
A later `DATUM_CSYS` construction depends on a `SKETCH` operation when exactly one sketch-point identity addresses a named-point span containing one of the coordinate-system construction's resolved blocks. The dependency retains the point-identity witness and exact shared block. The construction and point blocks must belong to the same offset store, and the sketch must precede the coordinate-system operation. No dependency is assigned for zero, multiple, or later sketch candidates.
