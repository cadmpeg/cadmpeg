...
When an operation-header input and an index-row slot resolve to the same bounded data block, their exact reuse relation retains the operation, header slot, row, row slot, shared block, and row-token offset. Repeated row slots remain distinct uses. The relation does not assign a semantic operand or class role.
