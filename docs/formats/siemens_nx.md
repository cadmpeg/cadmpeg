...
A polygon with a nonnegative group is a rendered triangle; negative-group polygons are structural dual-mesh records and do not enter the rendered triangle list. A canonical tessellation requires one exact property-table binding from the owning logical shape node to the type-7 segment and payload object containing the coordinate and topology records. The logical node resolves through its common node element in the binding's type-1 scene segment. Coordinates are converted from serialized metres to document millimetres. When normals are bound, triangle corners are deindexed so each output vertex has the corner's coordinate and mask-selected normal; this preserves multiple normals at one coordinate seam. The canonical tessellation retains the shape-node source identity, ordered coordinates, rendered triangles, and parallel normal vectors; it has no body association until a separate NX body-to-scene-node relation is present. A missing or ambiguous scene binding, non-triangular rendered polygon, missing coordinate array, out-of-range attribute index, incomplete normal lane, or empty rendered triangle set rejects all JT tessellations atomically.
