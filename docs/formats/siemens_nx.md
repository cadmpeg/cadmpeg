...
Each expression table is one equations feature; equations features follow their first physical expression-record offsets and precede operation features. Within an acyclic table, neutral parameter order is a stable dependency order with physical record order as its tie-breaker. A cyclic table retains exact formula text without neutral dependency edges. An operation with one or more resolved parameter uses depends once on each equations feature owning those parameters, in first-use order. Parameter consumer metadata follows the first physical use occurrence.
