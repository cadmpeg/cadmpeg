...
A `DATUM_CSYS` operation supplies the construction dependency of the immediately following `SKETCH` operation when the coordinate-system payload's second block is followed immediately by the sketch record's complete nonempty payload-reference block lane in the same offset store. Every sketch reference must resolve, and its blocks must form one consecutive span in serialized reference order. The dependency retains both operation identities, the typed construction and payload, the bounded sketch record, the complete ordered reference lane, and the exact consecutive boundary blocks. A different intervening operation, missing or ambiguous payload or sketch record, unresolved reference, nonconsecutive lane, or store change does not establish the dependency.
