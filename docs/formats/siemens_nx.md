...
Hvec form depends on the stream: partition streams use **`xyz3`** (`x,y,z` meters); deltas streams use **`ext11`** (`x,y,z, p3,p4,p5,p6, tx,ty,tz, t`), with a unit tangent and strictly increasing native `t`. The two ext11 surface-parameter lanes are `(p3,p5)` and `(p4,p6)`. A non-sentinel lane belongs to a support only when evaluating every lane pair on that support reproduces the corresponding chart point within `chordal_error` and no other support satisfies the same lane. Two assigned lanes must address distinct supports. A sentinel-bearing, multiply matching, non-evaluating, or conflicting lane supplies no support chart. The chart parameter is meter-scale: `t_{k+1} = t_k + chord_k · f_k`, with `t_0 = base_parameter` and chords in meters. `chordal_error` defines the verification tolerance for chart-hosted carriers. Intersection charts use `(base_parameter, base_scale) = (0.0, 1.0)`. Procedural-spine charts have `chart_count == count`, sentinel `parameter_error`, and finite non-zero `base_scale`. When `xyz3` and `ext11` records have the same xmt, count, and point sequence within the larger chordal error, the `ext11` native `t` sequence and surface-parameter lanes govern the shared chart carrier.
...
When an intersection support has no uniquely assigned values-array lane, each CHART_s point maps through the support carrier's inverse parameterization. Analytic planes, cylinders, cones, spheres, and tori use their closed-form inverse. NURBS surfaces solve within their bounded knot domain using exact first parameter derivatives; each point after the first uses the preceding solution as its continuation seed. OFFSET_SURF preserves its support's parameterization and solves on that support domain against the offset surface. The complete derived lane transfers only when forward evaluation reproduces every chart point within `chordal_error`. Consecutive analytic azimuth parameters unwrap by integral turns to preserve chart continuity. A serialized lane that already has a unique support assignment takes precedence.
