...
An offset-store named point object spans two consecutive bounded data blocks. The first block begins with the type-free `Point<positive decimal>` name frame and contains exactly the first framed scalar; the second block contains exactly the second framed scalar. No other complete name or scalar frame occurs across the two-block concatenation. The object retains both block identities, scalar order and values, and exact source offsets. The record assigns no sketch ownership, coordinate unit, or model-space frame.
