...
One or more sketch named-point block uses and one or more reconstructed sketch-point records identify the same solved two-dimensional point when they belong to the same sketch operation and named-point object, their `Point<positive decimal>` names are identical, and every same-name record has two scalar values bit-identical to the named-point values in order. One identity relation retains every reconstructed witness in payload order, the independently framed named point, and every block use and sketch reference in reference order. No identity is assigned when the reconstructed payload contains no same-name point or any same-name point has conflicting coordinates.
