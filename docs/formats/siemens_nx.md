...
An `RMFastLoad` linked-index row has an explicit display-color assignment when a canonical color-index token occurs immediately before its `02 0b` opener and immediately after the preceding row's exact `01 c0 44 04 00` suffix. Indices 1 through 127 use the one-byte token equal to the index. Indices 128 through 216 use `80, index`. The assignment retains the linked row's unresolved leading object identity, discriminator, target index, three post-marker indices, flag, mode, resolved part color definition, exact compact-index and color tokens, and absolute token and row offsets. These row indices retain serialized roles only. Index zero, an index greater than 216, a changed preceding suffix, a missing unique part color definition, or intervening bytes reject the assignment atomically.
