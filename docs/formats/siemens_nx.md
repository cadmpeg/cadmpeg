...
used by feature-history Boolean target and tool lists. The tuple can cross a
12-byte row boundary. The body-image binding is valid only when the wrapper
word resolves to the exact compressed stream position.
...
Within one ordered feature-history area, the last operation carrying a primary-body field is that body object's latest writer. A body object is terminal when no later Boolean operation names it as a tool. When every emitted partition image has a segment body-object binding and a decoded latest writer, the terminal bound images are the final body set; bound nonterminal images are consumed history. This selection is atomic: an unbound image, a bound image without a decoded writer, or operation references outside the ordered area leaves every image retained.
