...
A graph-only OFFSET_SURF produces a FACE-owned B_SURFACE cache when exactly one construction and exactly one cache satisfy the full-domain fit contract. A nonperiodic B_SURFACE support with uniform positive weights has a constant oriented normal when its control net is a separable planar translation grid, every consecutive U increment is a positive multiple of one nonzero U direction, every consecutive V increment is a positive multiple of one nonzero V direction, the two directions are independent, and every derivative knot denominator is positive. A candidate cache with identical degrees, knots, counts, weights, and periodicity is a full-domain fit when every cache pole differs from the corresponding support pole plus the signed normal translation by at most the document linear tolerance. The maximum pole residual is the cache-fit tolerance because the surface residual is a convex combination of the pole residuals. Ambiguous construction-to-cache relations do not transfer.

For a nonplanar support with the same uniform-weight restrictions, the first and second U/V derivative control nets bound the support partials and normal derivatives over the complete active knot rectangle. Let `N = Pu × Pv`. One evaluated unit normal `n` orients the proof; every pairwise cross product of a U-derivative control vector and a V-derivative control vector must have strictly positive projection onto `n`. Their minimum projection `m` is a positive lower bound for `|N|`. With control-net norm bounds `|Pu|≤U`, `|Pv|≤V`, `|Puu|≤UU`, `|Puv|≤UV`, and `|Pvv|≤VV`, the unit-normal derivative bounds are `(UU·V + U·UV)/m` in U and `(UV·V + U·VV)/m` in V. The derivative control nets of the cache-minus-support pole differences plus the signed normal-derivative bounds give global Lipschitz bounds for cache error. Adaptive subdivision begins on every nonempty U/V knot-span product; a rectangle is certified when its evaluated midpoint error plus half-width times each directional Lipschitz bound does not exceed the document linear tolerance. Every rectangle must certify within the subdivision limit. The largest certified rectangle bound is the cache-fit tolerance. A nonpositive normal bound, discontinuous derivative knot denominator, malformed basis, nonfinite evaluation, or subdivision exhaustion rejects the cache relation.
