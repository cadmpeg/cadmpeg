...
A second self-framed row is `02 0b, first:compact_index, 93 8c, discriminator:u8, target:compact_index, ff ff 90 fe, index[3], 00 47, flag:u8, 04 01 c0 44 04 00`. The discriminator is `16`, `17`, or `18`; the flag is `03` or `07`; and `target` plus all three trailing indices are non-null in-range block ordinals in the same offset-only store. The unresolved leading index, discriminator, ordered four-block lane, flag, token offsets, block containing its opening byte, and opening offset within that block are retained. The opening block does not own the logical row because the row may cross physical block boundaries. A changed marker, other discriminator or flag, null or out-of-range block index, or incomplete row rejects the candidate atomically.
...
A target-index row is `02 01 01 01 16, target:compact_index, ff ff 90 fe, index[3], 00 47 03 07 01 c0 44 04 00`. The target and all three trailing indices are non-null in-range block ordinals in the same offset-only store. The row retains its ordered four-block lane, token offsets, block containing its opening byte, and opening offset within that block. Physical block boundaries do not delimit the row or assign ownership. A changed marker or suffix, null or out-of-range block index, other lane arity, or incomplete row rejects the candidate atomically.

When an operation-header input and a slot in any of the three row grammars resolve to the same bounded data block, one common exact reuse relation retains the operation, header slot, row grammar and identity, row slot, shared block, and row-token offset. In the linked and target grammars the target is row slot zero and the three post-marker indices are slots one through three. Repeated row slots remain distinct uses. The relation does not assign a semantic operand or class role.
...
The `FACE_BLEND` operation label projects as a face blend. Its first and second support-face selections and radius law remain unresolved until corresponding construction fields are assigned.
...
The owning `BLOCK` feature links the complete typed dimension set and construction independently. Dimension order is native parameter order. Exactly one transferred solid output body establishes the block placement when its complete ownership graph contains exactly one region and one shell and that shell's surfaces reduce to exactly three mutually orthogonal plane-normal bands, every band has two distinct extrema, and the three extrema separations have a tolerance-bounded bijection with the ordered dimensions. Faces outside that output body do not participate. Each normal is initially oriented so its first non-zero model-space component is positive, and bands are ordered by descending model-space x, y, and z components before dimension matching; this order breaks equal-dimension permutation ties. After dimension ordering, the third axis and its offsets are reversed when required for a right-handed frame. The three resulting minimum plane offsets define the origin. These ordered axes and origin form the local-to-model transform. Missing, multiple, or non-solid output bodies, multiple regions or shells, an incomplete ownership graph, additional owned plane directions, non-orthogonal bands, missing extrema, or a separation outside the document linear tolerance leave placement absent.
