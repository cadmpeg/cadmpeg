...
A polygon with a nonnegative group is a rendered triangle; negative-group polygons are structural dual-mesh records and do not enter the rendered triangle list. A canonical tessellation requires one exact property-table binding from the owning logical shape node to the type-7 segment and payload object containing the coordinate and topology records. The logical node resolves through its common node element in the binding's type-1 scene segment. Coordinates are converted from serialized metres to document millimetres. When normals, colors, or texture coordinates are bound, triangle corners are deindexed so each output vertex has the corner's coordinate and mask-selected attribute record; this preserves attribute seams at a shared coordinate. Normals become the parallel canonical normal vector. RGBA binary32 values use codec-owned channel kind `0x4e580001`, item size 16, and the two-bit color binding as flags. Texture channel ordinal `n` uses codec-owned kind `0x4e580100 + n`, item size `4 * component_count`, and flags `n | (binding_nibble << 8)`. Channel payloads are attribute-record binary32 components in little-endian order and their counts equal the deindexed vertex count. The canonical tessellation retains the shape-node source identity, ordered coordinates, rendered triangles, normals, and channels; it has no body association until a separate NX body-to-scene-node relation is present. A missing or ambiguous scene binding, non-triangular rendered polygon, missing coordinate array, out-of-range attribute index, incomplete bound attribute lane, inconsistent texture component count, or empty rendered triangle set rejects all JT tessellations atomically.
...
Compressed vertex arrays occur in binding order: coordinates, normals, colors, texture-coordinate channels zero through seven, then vertex flags. Each present array starts at the exact end of its predecessor.
