...
An Int32 Compressed Data Packet Mk. 2 begins with `value_count:u32 LE`; a zero count ends the packet. A nonempty packet continues with a codec byte. Codec 1 contains `code_bit_count:u32 LE` and `ceil(code_bit_count / 32)` little-endian code words consumed most-significant-bit first and decoded by the bit-length codec. Codec 3 contains the same code-text fields, followed by one probability context and a recursive out-of-band packet when escape symbols occur. The probability context begins with a big-endian entry count; its field widths, minimum value, and entries are a byte-aligned MSB-first bit stream. Recursive out-of-band depth does not exceed three. An invalid codec, field width, recursion, count, padding bit, or packet bound rejects the packet.
