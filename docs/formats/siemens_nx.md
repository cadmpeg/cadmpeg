...
When an intersection support is an analytic plane, cylinder, cone, sphere, or torus and has no uniquely assigned values-array lane, each CHART_s point maps through the analytic carrier's inverse parameterization. The complete derived lane transfers only when forward evaluation reproduces every chart point within `chordal_error`. Consecutive azimuth parameters unwrap by integral turns to preserve chart continuity. A serialized lane that already has a unique support assignment takes precedence.
