...
**Need.** The direction rule fixes the sign of the choice but not the magnitude. A constant that is too narrow turns a file that omits or malforms field 9, 11, or 19 into a file that decodes with many per-entity rejections, which is an improvement over the earlier whole-file refusal but not a transfer. The IEEE 32-bit and 64-bit significance pair `6` and `15` from IGES 5.3 §2.2.4.3.11 is the alternative, and it is what most producers declare.
...
### GL-11. IGES 4.0 defines no version-flag clamp and no flag above six

**Question.** Which normative source authorizes the version-flag clamp for a file whose declared flag is outside the IGES 4.0 value table?

**Known.** `iges.md` states that a version flag below `1` is assigned `3` and a version flag above `11` is assigned `11`, and cites IGES 5.3 §2.2.4.3.23, which states "Postprocessors finding an unrecognized value less than 1 shall assign 3; postprocessors finding an unrecognized value greater than 11 shall assign 11". `global.rs::ResolvedGlobal::dialect_loss` reports the clamp. NBSIR 88-3813 (IGES 4.0) §2.2.4.2.23, page 16, gives a table with **six** values only: `1` for 1.0, `2` for ANSI Y14.26M-1981, `3` for 2.0, `4` for 3.0, `5` for ANSI Y14.26M-1987, and `6` for 4.0, marked "This document". It states one rule for an out-of-table value: "If no valid integer number is entered in this field, the default value of 3 (corresponding to Version 2.0) should be assumed." IGES 4.0 states no upper clamp. The IGES 5.3 table extends the same six rows with `7` for ASME Y14.26M-1989, `8` for 5.0, `9` for 5.1, `10` for USPRO/IPO100 IGES5.2, and `11` for 5.3, and IGES 5.3 value `5` reads ASME/ANSI Y14.26M-1987 where IGES 4.0 value `5` reads ANSI Y14.26M-1987.

**Need.** The clamp is the rule that turns a declaration into the effective version, and the effective version selects the verified-set test. Under IGES 4.0 a declared `7` is not "greater than 11" and not "less than 1", so IGES 5.3's clamp does not apply; IGES 4.0's own rule reaches only a value that is not a valid integer, and `7` is a valid integer that is not in its table. The two documents therefore disagree about what a declared `7` through `11` means to a 4.0-era postprocessor, and the decoder resolves it by the later rule.

**Conflict.** The clamp is self-referential for this purpose. Choosing which version's clamp to apply needs the effective version, and the effective version is what the clamp produces. The decoder resolves the loop by always applying the IGES 5.3 rule and reporting the clamp, which is defensible because IGES 5.3 is the only version whose §2.2.4.3.23 covers the whole flag range, but it is a choice this document makes and no version states.

**Note.** The discriminating check is a corpus sweep for a declared field-23 value outside `1` through `11` and for a declared value of `7` through `11`, with the count of each recorded. A sweep that finds every file declares a value inside `1` through `11` makes the upper clamp unreachable and leaves only the lower clamp, whose `3` both documents give. The self-reference stays either way, and the fix for it is a stated rule that the newest known clamp applies to an unrecognized declaration, written into `iges.md` as a decoder rule rather than a specification citation.
