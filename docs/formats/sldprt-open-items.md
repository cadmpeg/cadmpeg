...
- The native identities of optional classless feature-manager nodes after the lights-and-cameras container are unresolved. Their source IDs are allocation positions and shift when earlier nodes are absent; source ID alone does not distinguish scene lights, design binder, comments, body folders, materials, equations, notes, sensors, favorites, history, selection sets, markups, and exploded views.
