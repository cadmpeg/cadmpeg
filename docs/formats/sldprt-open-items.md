...
- The u32 endpoint selector of the up-to-vertex `3` edge-endpoint point-reference form has no neutral semantics; the edge-endpoint reference projects with the endpoint retained natively. Codes above `9` have no neutral projection. Second-direction end-spec codes other than `0` and `1`, second-direction end specs whose first-direction code is not `0`, `1`, or `9`, and end-spec-shaped records whose +18 word carries a reference child rather than a termination code are unresolved. Feature-local faces selected by `moSingleFaceRef_w` paths bind to surviving B-rep faces through the `ATOM_ID_2001` face-identity attribute. The disposition of a selected feature-local face id that the attribute lane does not carry is unresolved; consumption by a later operation and an undecoded owning face are not distinguished.
...
- Reconciliation between generated feature-local bodies selected by compact `moCombineBodies_c` target and tool paths and bodies that survive in the final B-rep is unresolved. The `ATOM_ID_2001` attribute lane carries face identities only and binds no body identity.
...
- The identifier space of the `LAST_BODY_MODIFYING_FEATURE_ID` body attribute is unresolved. Its values are not native feature object ids, so it binds no body to a history feature.
- Reconciliation between generated feature-local edges selected by entry-form `moCompEdge_c` paths and edges that survive in the final B-rep is unresolved. The `ATOM_ID_2001` attribute lane carries no edge identity, and the grammar of the `EDGE_PERM_ID_2_2003` attribute family is unresolved. Compact-ID edge vectors remain unresolved.
