...
- The model-space coordinate mapping of `moPlaneInterAxisData_c` line coordinates is unresolved. Plane-intersection axes expose two ordered `moPlaneRef_w` source operands, but the line-coordinate ordering does not agree with the fixed-reference-plane frame ordering.
