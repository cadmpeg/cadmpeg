...
- Neutral invariants and operand roles for constraint codes `29..41` and `43..85` remain incomplete. Code `28` is coradial and code `42` is merge-points coincidence. All native identities and the numeric taxonomy through `85` are defined. Codes above `85` are unresolved native extensions.
