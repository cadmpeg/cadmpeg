...
An expanded model scalar section stores `double_xar\0 f8 <count>` followed by exactly `count` ordered slots. `10` is the literal-one slot, `0b` is the literal-zero slot, `e5` occupies a five-byte dictionary-reference slot, and `e8` occupies a four-byte dictionary-reference slot. Other slots use their defined scalar token widths. The final counted slot is `e0`, an explicit terminal null. Literal slots retain their decoded values; dictionary-reference slots retain their exact unresolved bytes until their selector mapping is defined.
