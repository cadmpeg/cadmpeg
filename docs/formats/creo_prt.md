...
Two coaxial circular cones reduce their intersection to equality between their signed linear radial functions. With the first cone's axial coordinate `t`, the second cone's axis alignment `d ∈ {-1, 1}`, and its origin at first-axis coordinate `c`, the functions are `q1(t) = r1 + k1t` and `q2(t) = r2 + dk2(t - c)`. Each equation `q1(t) = s q2(t)` for `s ∈ {-1, 1}` contributes one axis-normal circle of radius `|q1(t)|` when its linear coefficient is nonzero and the radius is positive. An identity for either sign means the cone surfaces coincide and does not define an intersection curve. The edge's paired solved endpoints select one circle when exactly one candidate contains both endpoints.
