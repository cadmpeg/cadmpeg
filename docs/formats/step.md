...
trailing or consecutive hyphens. A root number is `0`, `1`, or `2`. The root
component states a root number when its text is that number, or when its text
is a registered root identifier. The registered root identifiers are `itu-t`
and `ccitt` for root `0`, `iso` for root `1`, and `joint-iso-itu-t` and
`joint-iso-ccitt` for root `2`. The comparison with a registered root
identifier is exact. An ASN.1 identifier that differs from each registered root
identifier in spelling or in case states no root number. An uppercase spelling
such as `ISO` is not an ASN.1 identifier, so it has no component form. A
component in the parentheses form states the number in its parentheses, so
`iso(1)` states root `1` and `iso(standard)` states no number. When the root
number is `0` or `1`, a numeric second component is in `0..=39`. A root
component that states no root number constrains no second component. CADIR
decision: leading and trailing whitespace around an identifier is ignored for
validation, matching, and uniqueness.
CADIR decision: an object identifier component outside the range that its
position permits is a recoverable defect. A root component that states no root
number is out of range, whether its text is a number or an ASN.1 identifier.
The other out-of-range components are a negative number in any position and a
second component number above `39` under root `0` or `1`. The header parse
continues, and the header uses the schema name as it uses the name of a valid
identifier. The object identifier is not admitted: it stays invalid for a DATA
section schema name, for a `FILE_POPULATION` governing schema, and for the
AP242 edition report. The decode reports one
