...
**Known.** `asm.md` §6.3 "**Construction-cache ownership**" defines ownership relative to the outer non-`ref` construction scope. The token-space `surface_cache` and `curve_cache` readers enter that scope and exclude every marker in a nested subtype before applying the family ordinal. Binary `decode_pcurve_cache` and `decode_curve_cache`, the compound/directrix helpers, and writable patch-layout readers still scan raw markers. Binary readers try `INT_WIDTHS = [8, 4]` and return the first width with a decodable block. `procedural_curve_recursive` changes the ordinal to last for wrapper flags and first for other families. No path counts competing valid blocks or withholds on a tie.
