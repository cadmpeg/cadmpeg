...
**Known.** `asm.md` §6.3 "**Construction-cache ownership**" defines ownership relative to the outer non-`ref` construction scope. The token-space `surface_cache`, `curve_cache`, and procedural-curve record reader enter that scope and exclude every marker in a nested subtype before applying the family ordinal. The procedural-curve reader takes the last directly owned curve for source-wrapper families and the first for other families. The `rule_sur` reader and the legacy `sum_spl_sur`, `rot_spl_sur`, `off_spl_sur`, taper-family, and `comp_spl_sur` readers consume their direct construction fields in grammar order and require the subtype close after the complete family payload. Binary `decode_pcurve_cache` and `decode_curve_cache`, the remaining compound/directrix helpers, and writable patch-layout readers still scan raw markers. Binary readers try `INT_WIDTHS = [8, 4]` and return the first width with a decodable block. No remaining path counts competing valid blocks or withholds on a tie.
