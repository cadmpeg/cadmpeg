...
**Known.** `asm.md` §6.3 "**Construction-cache ownership**" defines ownership relative to the outer non-`ref` construction scope. The token-space `surface_cache`, `curve_cache`, procedural-curve record reader, and procedural-surface family readers consume only directly owned cache markers. The procedural-curve reader takes the last directly owned curve for source-wrapper families and the first for other families. Fixed-layout procedural-surface readers consume their construction fields in grammar order and require the subtype close after the complete family payload. The shared revision surface-tail reader returns the domain of the solved cache it consumes, so a containing construction does not scan the enclosing scope again. Generic binary `decode_pcurve_cache` and `decode_curve_cache` try both integer widths but return a value only when exactly one `(width, marker)` candidate decodes. Compound/directrix helpers outside the token-space readers and writable patch-layout readers still scan raw markers. A generic binary helper can still accept one nested candidate because it has no enclosing-scope grammar. The remaining raw paths do not all carry an owner or stream-width witness.
