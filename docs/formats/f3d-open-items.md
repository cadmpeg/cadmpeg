...
- The per-member roles within a `0x80000000` spline-group relation — which members are the spline's defining entities versus handle entities, and why the member count differs from the spline's control-point count — are unresolved beyond the grouping itself.
...
- In a feature-owned sketch's member-run head record, the roles of the eleven zero bytes between the record index and the placement matrix and of the marked tail after the matrix are unresolved.
- Sheet-metal (`BaseFlange`, `EdgeFlange`, `Hem`), `RemoveBody`, surface `Stitch`, `Base Feature`, and `CopyPasteBodies` parameter-scope kinds have no neutral scope grammar; their owned parameters retain native operation semantics.
