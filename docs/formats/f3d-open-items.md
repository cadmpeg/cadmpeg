...
- The class-specific fields after the fixed `*_recipe_data` null sentinel and integer prologue are unresolved; their feature-operation, profile, extent, and dependency semantics are not assigned. Fillet and Chamfer edge operands resolve to ordered edge recipes, but the recipe fields assigning each operand to the active B-rep edge identity remain unresolved. Extrude scopes resolve their Sketch operand, distance/draft parameters, counted construction-operand and selection groups, nested operand-identity chains, fixed persistent-identity leaves, fixed-width selection members, and member identities that name persistent geometry in the selected Sketch. Selection identities that do not name persistent sketch geometry, non-persistent operand-identity leaf variants, the context UUID's role, the Boolean operation, and non-distance termination operands remain unresolved. The semantic roles of each counted group's role/scalar fields and variant byte and the parameter-scope u32 immediately following its ordered reference table are also unresolved.
