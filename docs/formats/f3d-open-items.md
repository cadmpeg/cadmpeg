...
- The leading two-boolean sense pair of a revision-gated `off_spl_sur` stores the text form's two sense tokens in the same order, and the pair is contiguous with the two-boolean ASM extension prefix. Which position carries the U sense and which the V sense is unresolved — the flags do not perturb the solved cache, so no geometric discriminator distinguishes them — and the meaning of the extension prefix is unresolved. An instance whose extension-prefix flags are both true carries an additional run before the shared tail — a boolean, six integers, a boolean, an embedded cache-first intcurve with optional endpoints, further booleans, a small tolerance scalar, and four `-1` integers; which flag gates the run and the field roles are unresolved, and such records are retained verbatim. The run's leading fields correspond structurally to a surface–surface-intersection payload prefix — presence logical, six-integer header, intersection curve, two pcurve logicals, and tolerance — but the trailing four `-1` integers exceed the three endpoint-term slots of that grammar, so the run grammar remains unresolved.
