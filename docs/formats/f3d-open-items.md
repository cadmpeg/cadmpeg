...
- The Design `MetaStream` Dimension object is a registry with no owned entity IDs. Paired-locus dimension frames resolve two concrete sketch-geometry records, but the remaining dimension companion variants and their locus arities are unresolved.
- The indexed parameter companion has a fixed prefix and an owner backlink. The semantic role of its opaque u64 and the nested-versus-sibling role of the immediately following indexed record are unresolved outside the paired-locus dimension frame.
