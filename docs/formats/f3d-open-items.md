...
- The class-specific fields after the fixed `*_recipe_data` null sentinel and integer prologue are unresolved; their feature-operation, profile, extent, and dependency semantics are not assigned. Fillet and Chamfer edge operands resolve to counted groups of ordered edge recipes, and Fillet groups resolve their radius and tangency-weight inputs, but the recipe fields assigning each operand to the active B-rep edge identity remain unresolved. Extrude face recipes join their persistent Design reference to a deterministic set of active B-rep face candidates; the recipe field selecting one candidate remains unresolved. Extrude scopes resolve their result operation, profile-plane, offset-profile-plane, and selected-face starts, one-sided distance, one-sided to-face, and two-sided distance forms, Sketch operand, distance/draft parameters, body/profile/face construction-operand roles, counted construction-operand and selection groups, nested operand-identity chains, fixed persistent identities, fixed-width selection members, and member identities that name persistent geometry in the selected Sketch. Selection identities that do not name persistent sketch geometry, the context UUID's role, the Boolean byte preceding the start-support enum, and the assignment and ordering of face operands between the profile start and one-sided termination remain unresolved. The construction-group scalar fields and variant byte, and the role field outside Extrude scopes, remain unresolved.
- The semantic role of a parameter record's u64 prefix value is unresolved.
