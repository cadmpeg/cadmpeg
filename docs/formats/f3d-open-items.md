...
**Known.** `f3d.md` §3.1 "A mesh body's geometry container" gives the three decoded classes. The `99F6967E-ED35-4222-B906-5CCF0AC70B53` class is the `Base Mesh Feature` scope and owns its ordered mesh-body references. The `443807AD-8025-41A3-8A50-5157579C3D78` class is one mesh-body collection per feature, the `E5B3F49A-D8D0-4EEF-BC2B-FCDDAEF9745E` class is one wrapper per mesh body, and the `f85f2e62-7627-4922-a16d-53e1275d2aac` class is one fixed Scene-state record per mesh body. The collection owns one `6FC173DC-C7E3-402C-A8C0-891A26DADF8D` record. That record is empty unless the mesh feature has a texture resource. The `EA90DA22-556C-4C61-89BB-20C2681B7A9D` record stores two equal affine matrices. Equality is a record invariant; unequal matrices invalidate the body join. The complete nonsingular affine map applies to container coordinates in model centimetres. It changes vertex coordinates but not triangle tuples or attribute selectors. A negative determinant reverses geometric winding in model space. Separate mesh bodies retain separate containers, identities, and matrices even when their geometry bytes are equal.

**Need.** We must settle the complete byte framing of the five payloads and the optional texture-resource entry before writing a mesh body from a neutral model.
