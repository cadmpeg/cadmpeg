...
- The basic surface record names `offset` and `sur-sur-int` do not occur in the ASM-232 record vocabulary. They correspond to the `off_spl_sur` construction and to the surface–surface-intersection construction graph carried by other records; their standalone payload grammars remain undefined.
...
- The enum opening the shared revision-gated surface tail is a cache-kind discriminator, coherent with the five-value standard-tail enum of `law_spl_sur`. Its zero value selects the full solved-cache tail; nonzero branch grammars remain undefined, and a nonzero value selects a native branch in which the containing record is retained verbatim.
...
- The revision-gated `cl_loft_spl_sur` tail kind is a bounded strategy selector of which only the zero value has a defined grammar; the nonzero vocabulary and the condition selecting the optional trailing values and BS3 curve of the kind-zero payload remain unresolved.
