...
- The `tedge` trailing LONG (`chunk[13]`), following the per-entity serializer revision stamp, and the `tvertex` trailing LONG (`chunk[9]`), following the tolerance slots, are version-gated with the same gate, absent in older streams and taking values `0` and `1` when present. Both are retained verbatim; their roles are unresolved.
