...
An edge operand has two exact candidate proofs. The incidence proof intersects every selector's complete boundary-count-and-coedge-ordinal candidate set with every nonempty changed shared-edge set supplied by the prefix references. The cardinality proof intersects every selector's boundary-count candidate set with those same changed shared-edge sets. Either proof resolves the operand when its intersection contains exactly one stable edge slot. When both proofs resolve, they must name the same slot. The proven slot is retained on the native operand independently of whether every member in its containing group resolves. An edge group resolves from its complete ordered member run when every member resolves individually; the projected edge list follows member order and removes repeated edge identities after their first occurrence. A group also resolves as an unordered set when its members have distinct operand identities, every operand carries the same nonempty set of deleted preceding boundary-edge slots, and the number of distinct slots equals the operand count. This cardinality form projects the slots in increasing stable-slot order. Both forms retain the native group identity. A missing operand, duplicate operand identity, contradictory proof, absent complete group proof, or lost edge reference prevents resolution. For every prefix reference in source order, the operand retains its referenced faces in both the result and preceding topologies. Every referenced face retains its face slot, its loops in face-membership order, and each loop's aligned coedge and edge slots in cyclic membership order. When adjacent loop edges share exactly one vertex and its carrier bindings are unique, the loop also retains aligned stable vertex slots, point slots, and model-space point positions. Each topology side retains the stable edge slots shared by the referenced-face boundaries and the primary candidate-face boundaries; the preceding side additionally retains the subset deleted or updated by the transition. The recipe order follows the scope reference-table order.
