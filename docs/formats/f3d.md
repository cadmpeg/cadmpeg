...
An `Extrude`, `Fillet`, or `Chamfer` scope with null current and preceding history-state identities is suppressed. An active scope in these families carries both identities and its current state is the operation result. A `Sketch` scope does not modify ASM topology and may carry null state identities without being suppressed.
