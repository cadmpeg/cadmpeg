...
**`law_spl_sur`**: legacy layouts begin with the U and V parameter intervals as four doubles; modern layouts begin directly with the primary recursive law formula. A counted sequence of additional law formulas follows. Standard-tail enum selector `0` (`full`) contains a solved NURBS surface, its fit tolerance, and six discontinuity float arrays. The optional interval prefix is structurally distinguished from a formula by its leading double tag. Native generation retains the interval prefix when present.
