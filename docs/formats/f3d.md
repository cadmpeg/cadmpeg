...
The standard edge-recipe program tail is a `-1`-delimited sequence of nine nonempty runs. Run 0 contains one root word. Runs 1–4 form the first side clause with lengths 2, 1, 1, and `2 + 8n`. Runs 5–8 form the second side clause with the same lengths. Each final run contains a two-word prefix followed by `n` ordered eight-word entries. An entry is an i32 side-local selector, a positive i32 count of edges in the referenced face loop, and two topology triplets. In each triplet the first and third words are the same positive i32; the middle word equals that integer or is one less. The root, each two-word header, both scalar words, both payload prefixes, and every entry field are retained. A tail containing nested `-1` delimiters outside this nine-run grammar remains an exact unstructured program.
