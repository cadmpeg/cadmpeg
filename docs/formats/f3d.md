...
Within a `Fillet` scope, an owned parameter whose `source_kind` is `Radius` carries a fillet radius. One positive radius selects the constant-radius form. A grouped Fillet assigns every owned `Radius` and `TangencyWeight` parameter exactly once; an ungrouped Fillet has at most one `Radius` and no `TangencyWeight`. Other parameter families retain the native feature. Within a `Chamfer` scope, `Distance` selects an equal-distance chamfer, `Distance 1` and `Distance 2` select the two-distance form, and `Distance` together with `Angle` selects the distance-angle form. These three forms are mutually exclusive, and every owned parameter belongs to the selected form. An incomplete, excess, mixed, or unclassified parameter lane retains the native feature. The distance scalars use the parameter record's centimetre storage and the angle scalar uses its radian storage.
