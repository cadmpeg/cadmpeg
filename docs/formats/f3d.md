...
The standard edge-recipe program tail is a `-1`-delimited sequence of nine nonempty runs. Run 0 contains one root word. Runs 1–4 form the first side clause with lengths 2, 1, 1, and `2 + 8n`. Runs 5–8 form the second side clause with the same lengths. Each final run contains a two-word prefix followed by `n` ordered eight-word entries. The root, each two-word header, both scalar words, both payload prefixes, and every payload entry are retained as typed fields. A tail containing nested `-1` delimiters outside this nine-run grammar remains an exact unstructured program.
