...
An `Extrude` scope stores its result-operation u32 at primary-header offset 28. When an indexed reference occupies offsets 25 through 35 as `01 + u32 record_index + six zero bytes`, the operation follows at offset 38. Operation values are `1 = join`, `2 = cut`, `3 = intersect`, and `4 = new body`. The two immediately following u32 values select the extent form: `(1, 1) = one-sided to face`, `(1, 2) = one-sided distance`, and `(2, 0) = two-sided distance`. A one-sided distance has exactly one `AlongDistance` parameter and no `AgainstDistance`; a one-sided to-face extent has neither distance parameter and has at least one face operand group; a two-sided distance has exactly one of each distance parameter. Join, cut, and intersect require an existing-body operand group; new body has no existing-body operand group.
