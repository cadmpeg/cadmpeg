...
The indexed companion record begins with a 58-byte common prefix. Its eleven-byte indexed header is followed by 20 zero bytes, `01 + u32 owner_record_index`, six zero bytes, a nonzero opaque u64, and eight zero bytes. Its `record_index` equals the owner frame's `companion_record_index`. The companion-owned payload interval begins after this prefix and ends at the next parameter-owner or parameter-scope primary header in the same Design `BulkStream`. A parameter scope beginning immediately after the prefix denotes an empty companion payload. Another indexed-record header can begin a nested companion record or the following sibling record; byte adjacency alone does not select between those roles.
