...
A counted linear frame with state mask `0x20` stores an offset graph. Its locus list is an equal-length nonzero-role source prefix followed by a zero-role result suffix. Every locus resolves to a line. The return run alternates one source index and one result index, uses every locus exactly once, and supplies the source/result bijection. Every pair is parallel and has the same nonzero signed separation along the source line's stored left normal. The shared signed separation is the graph's offset distance. When its magnitude equals the companion parameter's magnitude after centimetre-to-millimetre conversion, the parameter drives the offset relation. The signed-distance-to-parameter factor is `1` when their signs agree and `-1` otherwise; this factor preserves the offset direction induced by the stored source-curve direction. A null-locus frame governing that companion is the offset dimension's annotation frame and does not form a second constraint.
...
Within a `Fillet` scope, an owned parameter whose `source_kind` is `Radius` carries a fillet radius. One positive finite radius selects the constant-radius form. A grouped Fillet assigns every owned `Radius` and `TangencyWeight` parameter exactly once; every tangency weight is finite. An ungrouped Fillet has exactly one `Radius` and no `TangencyWeight`. Other parameter families or unrepresentable scalar values retain the native feature. Within a `Chamfer` scope, `Distance` selects an equal-distance chamfer, `Distance 1` and `Distance 2` select the two-distance form, and `Distance` together with `Angle` selects the distance-angle form. These three forms are mutually exclusive, and every owned parameter belongs to the selected form. Every distance is positive and finite; a distance-angle form's angle is finite and strictly between zero and pi radians. An incomplete, excess, mixed, unclassified, or unrepresentable parameter lane retains the native feature. The distance scalars use the parameter record's centimetre storage and the angle scalar uses its radian storage.
...
For each selector, an edge is an incidence match when, for every present clause entry, its incident loops contain both encoded boundary-count and coedge-ordinal incidences. Each selector retains all matching changed historical edge slots in stable slot order and separately retains the edge slot when that set is a singleton. Primary operand resolution uses the complete selector/reference intersection. When that intersection is empty, exactly one singleton persistent-reference edge resolves the operand when the edge also belongs to the deleted boundary set and appears in a selector's incidence, boundary-count, or topology-triplet candidates. A construction group with a known preceding history state retains that state and every ordered operand identity even when none of its edge identities resolves.
