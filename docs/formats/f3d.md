...
The standard edge-recipe program tail is a `-1`-delimited sequence of nine nonempty runs. Run 0 contains one root word. Runs 1–4 form the first side clause with lengths 2, 1, 1, and a positive variable length. Runs 5–8 form the second side clause with the same lengths. The root, each two-word header, both scalar words, and both payloads are retained as typed fields. A tail containing nested `-1` delimiters outside this nine-run grammar remains an exact unstructured program.
