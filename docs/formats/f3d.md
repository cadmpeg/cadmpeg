...
Each table variant has a stable neutral identity formed from the complete table-entry name and the variant name as two byte-length-prefixed UTF-8 strings. Table order and variants in other tables do not affect that identity. The table's `active` member selects the active neutral variant. Parameter overrides, suppressed-feature names, and material names transfer as variant properties. A paired rule transfers its `when` condition to the named variant when exactly one variant across all tables has its `activate` name; the property identity includes the complete rule-entry name. Native tables and rules remain the full-fidelity source for unrecognized members.
