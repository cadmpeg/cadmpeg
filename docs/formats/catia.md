...
- `b5 03 62` (loop node): payload `<0x80 + n_refs> (pcurve_ref edge_ref)* surface_ref <tail>`, `n_refs` odd. Member ref tokens use the E5-style compact set (`18`/`38`/`10 <hi>`=hi<<8/`08 <lo>`/`30 <lo><hi>`=(u16le)<<8). Token `28 <lo><hi>` takes bits 16–23 from the containing record's object id and replaces its low 16 bits with `<lo><hi>`. The tail begins with `<0x80 + edge_count>`; its remaining topology metadata does not alter member identity or order.
