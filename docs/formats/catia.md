...
- `b5 03 30` (offset surface): payload `82 result_carrier_ref source_surface_ref <distance:f64le> <carrier_kind:u8> <u0:f64le> <u1:f64le> <v0:f64le> <v1:f64le>`. The bounds are strictly increasing. `carrier_kind` is `15` for a plane result carrier, `05` for a cylinder, `11` for a cone, `09` for a sphere, `0d` for a torus, `19` for an `a8 03 32` rolling-ball result carrier, and `01` for a class-`31` result cache. The first reference supplies the result geometry and parameter chart; the second reference and signed distance define the exact offset construction.
