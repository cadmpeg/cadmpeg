...
**Object-stream topology:** `b5 03 5f` nodes are faces in native ordinal order. Distinct `b5 03 5e` identifiers referenced by loop nodes are physical edges. Each `b5 03 62` node defines one loop and stores its ordered edge occurrences. A paired pcurve lifted through its carrier defines the edge curve, and its endpoints coincide with `05 08 01` vertex loci. Consecutive endpoint loci separated by at most `5e-3` mm identify one logical vertex. The fixed serialized loop sequence has exactly one head-to-tail endpoint-sense assignment after that identification when it represents a closed boundary. This identification is loop-local: it does not merge unrelated nearby loci. Pcurve degree and carrier identify lines, circles, and B-splines. An object-stream file contains one body. The 3D edge geometry uses f32 endpoint coordinates, and native pcurves have degree 1 or 2.
