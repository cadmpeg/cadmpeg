...
Coordinate binding is one joint constraint problem over the complete quotient. Assign one distinct coordinate row to every logical-vertex component, restrict each assignment to that component's intersected domain, and require every edge with an explicit unordered endpoint-pair domain to select one stored pair across its two assigned components. Coordinate rows with equal stored XYZ form one coordinate class whose capacity is its row population. The binding is resolved only when the capacity-constrained class assignment has exactly one solution; permutations among rows of the same class do not change vertex placement. Uniqueness of the independent component domains is insufficient.
...
Every structurally complete `a5/a6/a7` or `b2/b3/b4 03 20` jet transfers to the pcurve arena even when its support reference or edge run remains unbound. The equivalent neutral degree-5 NURBS uses sixfold piece boundaries and retains the native evaluation range. Support identity and leading-extrapolation count remain attached to the carrier provenance.

A `B:06` side-use payload ending in `0x84` or `0x88` stores `0x80+n`, exactly `n` compact persistent references, then that terminal sense byte. The counted vector must consume the complete preceding payload; a non-closing payload has no decoded reference vector. The two terminal values are the two edge-use senses. In a complete five-reference edge run, writing the edge-node identities as `(curve,startVertex,endVertex,startParameter,endParameter)`, the two use-reference vectors are `(endParameter,startParameter)` then `(startParameter,curve)`. They do not identify the owning faces.
...
- **Owner allocation link:** when a `5f` link is immediately followed by a `62` owner packet, the owner's ninth identity equals `5f.target + 1`. Both checked successor identity and framed adjacency are required; an intervening record breaks the link.
...
- `b5 03 27/28/29/2b/2d` analytic surfaces: plane origin+normal, cylinder origin+axis+radius, cone apex+orthonormal frame+half-angle+slant chart, torus center+orthonormal frame+major/minor radii, and revolution profile reference+axis origin+direction+angular gauge. The profile reference uses the same positional-byte token forms as loop members; the axis fields immediately follow its variable-width token.
...
Each nested total-length tree is rooted at `7C 08`; every root is an independent object graph, and each `7C09` object holds a lead-coded head and a `7C0A` tagged-atom payload. It is the **feature/object-ownership layer**, not the expanded face→loop→coedge table or the port→vertex collapse. `7C09` head: `<lead> 01 <owner_ref> <class_ref> [storage_ref]`; its ordinary references use `80..d0 → byte−80` and its paged references use `d1..e4 <byte> → (prefix−d1)·256+byte+1`. `owner_ref` is a one-based ordinal into the containing graph's serialized `7C09` sequence. Class refs are zero-based per-file schema ordinals rather than object ordinals or global type codes. `7C0A` atoms use the same compact and paged forms, plus raw `0x51..0x7F` and escaped `0x80 <value:u32le>`. A fixed-width reference is `0x32 <ordinal:u32le>` and selects a one-based `7C09` record ordinal in the same graph; its four ordinal bytes are not independent atoms. E5 blobs inside `7C0A` are templated descriptor records (≈59 or 46 bytes), not NURBS payloads. The class30 pair records and `76 ac 7f`-delimited handle table are coedge/half-edge sub-tables, not the port→vertex relation.

All `7C09` records in one graph carrying the same `owner_ref` are fields of one serialized design object. Preserve their source order and class-specific storage forms as one owner group. Preserve the distinct resolved field-class names in first field order as the object's field vocabulary. When `owner_ref` is within the graph's record population it also selects that ordinal's exact record; an out-of-range owner remains an unresolved identity and does not invalidate the field group. A schema class name labels its field record and is not by itself a neutral feature operation.

A compact `0x81` reference field, a fixed-width `0x32` reference field, and every reference item in a `0x3b` list select a one-based `7C09` record ordinal in the same graph. Preserve every stored ordinal in payload order and link each in-range ordinal to its exact field record. The selected record's `owner_ref` identifies the referenced design object. Resolve object dependencies in first reference order, collapse repeated references to the same owner, and omit self-references. An out-of-range ordinal remains a reference link without a target. References to records without a materialized owner group remain record links without an object dependency.

The following `7C02` schema catalog stores UTF-8 strings. A nonzero first byte is the inclusive one-byte-header-plus-string length. A zero first byte selects a following `u32le` string-byte length after the five-byte header. It either immediately follows the `7C08` graph or follows the graph's intervening `7C0B` value block. String values may contain line feeds and non-ASCII unit symbols. Its fixed first four entries are `CATCatalogManager`, `catalogManager`, `catalogLinks`, and the empty string. A `7C09` head's `class_ref` is the zero-based ordinal of its class name in this catalog.
...
Plane `0x97` circle pcurves use arc-length parameters, normalized to angle by `t_neutral=t_native/radius`; they transfer as exact piecewise rational quadratic arcs. Plane `0xa0` pcurves store degree-5 position/first-derivative/second-derivative jets at distinct knot sites. Each adjacent site pair defines one exact quintic Bézier span from the two endpoint jets. The neutral NURBS uses sixfold span boundaries, preserving every quintic span and its native parameterization without fitting.
