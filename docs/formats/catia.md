...
A design object whose every field has a positionally paired complete definition-bound value and whose definitions all select the same `CircPattern` catalog entry is one circular-pattern feature. This establishes the operation identity and circular form independently of its unresolved seeds, axis, count, angle, field roles, and outputs. A missing definition value, a distinct selected entry, or a distinct selected name leaves the feature identity unresolved.
