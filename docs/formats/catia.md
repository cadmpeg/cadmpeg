...
A compact `0x81` reference field, a fixed-width `0x32` reference field, and every reference item in a `0x3b` list select a one-based `7C09` record ordinal in the same graph. Preserve each valid target as an exact field-record link in payload order. The selected record's `owner_ref` identifies the referenced design object. Resolve object dependencies in first reference order, collapse repeated references to the same owner, and omit self-references. References to records without a materialized owner group remain record links without an object dependency.
