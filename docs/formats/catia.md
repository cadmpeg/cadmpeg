...
- `b5 03 21` (pcurve): after the support reference, degree, knot, multiplicity, and UV-pole lanes, the exact suffix is `05 05 <zero:f64le> <positive_scalar:f64le> <one:f64le> <zero:f64le> 00 07`. Every scalar is finite, and the suffix exhausts the payload. `catia_support_ref` is the owning surface's `object_id` directly. The 3D edge is the pcurve lifted through the surface (plane / cylinder arc-length `θ=u/r` / surface-of-revolution / bspline), and the clamped end poles land on `05 08 01` vertices to f32 round-trip.
