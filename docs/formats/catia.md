...
Port-domain intersections preserve physical-edge pair correlation. For every edge with distinct endpoint ports and an explicit pair domain, at least one pair must place one member in each of the edge's two current port components. Equal endpoint ports encode a closed edge: both endpoint positions bind the same logical vertex, and its admissible domain consists of diagonal pairs `[v,v]`. Check this constraint after every corner merge and on the complete quotient.
