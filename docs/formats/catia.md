...
A complete entity-record suffix value is `<prefix:bytes[4]> E7 <trailer:bytes[2]>` for an unset value or `<prefix:bytes[4]> E6 <bits:u64le> <trailer:bytes[2]>` for a finite binary64 scalar. The production is assigned only when it consumes the complete suffix. Preserve the exact prefix, evaluation, and trailer independently of their field-specific semantic roles.
