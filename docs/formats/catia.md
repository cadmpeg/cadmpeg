...
For `b5 03 29`, the 185-byte payload is `<lead> <apex:3f64> <direction_x:3f64> <direction_y:3f64> <axis:3f64> <half_angle:f64> <pre_range_scalar:f64> <angular_range:2f64> <slant_range:2f64> <angular_scale:f64> <one:f64> <zero:f64> <angular_domain:2f64>`. The slant interval is increasing and may begin at the apex; `|slant_min|≤1e-12` denotes zero. The active angular range and full-turn domain obey the `b2 03 29` range/domain invariants, and the two fixed scalars are exactly `1` and `0`. Its native chart evaluates with `azimuth=U/angular_scale` and `P(U,V)=apex+V·(cos(half_angle)·axis+sin(half_angle)·(cos(azimuth)·direction_x+sin(azimuth)·direction_y))`. Neutral cone parameters are `(azimuth,(V-slant_min)·cos(half_angle))`; the neutral carrier origin is the axis point at `slant_min`, with radius `slant_min·sin(half_angle)`.
