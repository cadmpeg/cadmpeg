...
The transferable arithmetic expression grammar is finite decimal literals, `mm`, `rad`, and `deg` unit suffixes separated from the literal by zero or more ASCII whitespace bytes, the dimensionless constants `PI` and `E`, resolved relation symbols, parentheses, `sin`, `cos`, `tan`, `asin`, `acos`, `atan`, `sinh`, `cosh`, `tanh`, `asinh`, `acosh`, `atanh`, `log`, `ln`, `exp`, `ceil`, `floor`, `int`, `round`, `abs`, `sqrt`, `min`, and `max` calls, right-associative `**` powers, unary `+` and `-`, products and quotients, then sums and differences in precedence order. Power binds more tightly than unary sign, so `-2**2` is `-(2**2)`; a signed exponent is permitted. Parentheses, function arguments, unary operators, and recursively nested powers share a maximum nesting depth of 128. A `deg` literal converts to radians. `PI` is π and `E` is the base of the natural logarithm. Expression dimensions are signed 32-bit integer exponents of length and angle; multiplication adds exponents and division subtracts them with checked arithmetic. A power exponent must be dimensionless. A dimensionless base accepts any exponent whose result is finite. A dimensioned base requires an integral signed 32-bit exponent and multiplies both dimension exponents by it with checked arithmetic. Addition, subtraction, `min`, and `max` require equal dimensions. `sin`, `cos`, and `tan` accept an angle in radians or a dimensionless radian value and produce a dimensionless result. `asin` and `acos` require a dimensionless argument in `[-1,1]` and produce an angle; `atan` accepts any finite dimensionless argument and produces an angle. Hyperbolic and inverse-hyperbolic functions require a dimensionless argument and produce a dimensionless result; `acosh` requires an argument at least one and `atanh` requires an argument strictly between negative one and one. `log` and `ln` require one positive dimensionless argument; `log` computes the base-10 logarithm and `ln` computes the natural logarithm. `exp` requires a dimensionless argument. `ceil`, `floor`, `int`, and the one-argument `round` require a dimensionless argument and produce a dimensionless integral value. `int` truncates toward zero. `round` resolves a half-integer to the nearest even integer. `abs` retains its argument dimension. `sqrt` requires a nonnegative argument with even length and angle exponents and halves both exponents. Division by zero, exponent overflow, non-finite evaluation, trailing syntax, unsupported units, unsupported function names, invalid function domains, and excess nesting do not form a transferable formula result. The evaluated dimension must equal the declared result type. An `E6` result must equal the evaluated binary64 value; an `E7` result requires the same syntactic and dimensional closure but has no stored value to compare.
