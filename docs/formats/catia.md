...
A typed legacy relation with an exact parameter identity, no input clauses, no output clause, and a non-`VoidType` result is a constant formula for that parameter when the selected identity contains exactly one typed parameter. Its expression uses the arithmetic grammar and dimensional rules below. The evaluated result type and value must equal the selected parameter's declared type and stored binary64 value. Multiple relations selecting the parameter, incomplete or unsupported expressions, mismatched types, and mismatched values leave the generated scalar literal unchanged.
