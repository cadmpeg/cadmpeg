...
The transferable arithmetic expression grammar is finite decimal literals, `mm`, `rad`, and `deg` unit suffixes separated from the literal by zero or more ASCII whitespace bytes, the angle constant `PI`, resolved relation symbols, parentheses, `sin`, `cos`, `log`, `min`, and `max` calls, unary `+` and `-`, products and quotients, then sums and differences in precedence order. A `deg` literal converts to radians. `PI` is the radian angle π. `sin` and `cos` require one angle argument and produce a dimensionless result. `log` requires one positive dimensionless argument and computes its natural logarithm. `min` and `max` require two equal-dimensional arguments and retain that dimension. Addition and subtraction require equal dimensions. Multiplication requires at least one dimensionless operand. Division accepts a dimensionless divisor or equal-dimensional operands, the latter producing a dimensionless result. Division by zero, non-finite evaluation, trailing syntax, unsupported units, unsupported function names, and other dimension combinations do not form a transferable formula result. The evaluated dimension must equal the declared result type. An `E6` result must equal the evaluated binary64 value; an `E7` result requires the same syntactic and dimensional closure but has no stored value to compare.
