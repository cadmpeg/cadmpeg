...
A complete entity-record suffix value begins `<prefix-atom0:one_byte_atom> <prefix-atom1:one_byte_atom> <prefix-atom2:one_byte_atom> <prefix-code:u8>`. Its evaluation is `E7` for an unset value, `E8` for a zero-payload control state, `E6 <bits:u64le>` for a direct finite binary64 scalar, or `E6 00 00 00 E6 <bits:u64le>` for a zero-padded finite binary64 scalar. The trailer is empty, exactly two bytes, or the fixed zero frame `FE F6 00{16}`. The production is assigned only when it consumes the complete suffix. Preserve the three decoded prefix atoms, exact prefix code, evaluation encoding, evaluation, and trailer independently of their field-specific semantic roles. A suffix with another prefix, evaluation, or trailing form remains outside this production.
