...
An object whose structurally selected owner record is paired with a complete numeric-tuple entity containing exactly two finite binary64 items carries one planar point coordinate. A relation occurrence from a field class named `2DPoint` binds that point to a sketch when the selected target field is named `PRTSketch`, its payload is terminator-only, its head has no storage selector, and its payload has no references. A relation to another field in the same design object does not select the sketch declaration. The point has one sketch membership only when all qualifying relation occurrences select one target design object. A point coordinate field is semantically closed when its complete payload is one standalone reference to that sketch declaration followed by the terminator; additional fields retain additional unresolved semantics.
