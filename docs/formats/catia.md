...
A design object whose every field is a complete empty declaration with a positionally paired complete definition-bound value, whose field class equals that selected definition, and whose definitions all select the same `CircPattern` catalog entry is one circular-pattern feature. These declaration records are consumed by the feature. This establishes the operation identity and circular form independently of its unresolved seeds, axis, count, angle, field roles, and outputs. A payload-bearing field, field/definition class mismatch, missing definition value, distinct selected entry, or distinct selected name leaves the feature identity unresolved.
