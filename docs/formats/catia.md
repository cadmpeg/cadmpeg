...
Equal trailing face ordinals retain one incidence with that face and leave the second incidence slot unresolved. Assign unresolved slots simultaneously. The unique assignment is the one in which every used face vertex has degree two and every face decomposes into closed endpoint cycles. Assignments that differ only by permuting rows with the same unordered endpoint identities and the same analytic curve carrier are one edge-identity gauge. Zero or multiple non-equivalent assignments invalidate the incidence graph.
