...
The fixed bytes in the inline production are structural and the complete inline body is preserved. A `7C08`, `7C0B`, or `7C02` candidate contained by another complete root extent is payload data, not an independent graph, value block, or catalog. A nested payload begins at the unique `7C0A` frame whose stored total length lands exactly on the enclosing `7C09` end and ends with an unconsumed `FE` terminator; bytes after that terminator and a length-framed field that consumes it invalidate the graph. Literal `7C0A` bytes in the variable head do not delimit the payload, and multiple length-closing candidates invalidate the graph. A childless record that does not consume the complete inline production invalidates the graph. It is the **feature/object-ownership layer**, not the expanded face→loop→coedge table or the port→vertex collapse. A separator-form `7C09` head is `<lead> 01 <owner_ref> <class_ref> [storage_ref]`. Compact heads encode their roles in the lead: `02 <owner_ref>`, `12 <owner_ref> <class_ref>`, `16 <class_ref> <storage_ref> <owner_ref>`, and `52 <owner_ref> <class_ref> <storage_ref>`. Other separator-free leads do not assign reference-shaped tokens to these roles. Its ordinary references use `80..d0 → byte−80` and its paged references use `d1..e4 <byte> → (prefix−d1)·256+byte+1`. A paged prefix without its low byte and bytes outside those assigned forms remain literals; they do not create references. `owner_ref` is an entity identity in the paired `7C05` run. Class refs are zero-based per-file schema ordinals rather than entity identities or global type codes. `7C0A` atoms use the same compact and paged forms, plus raw `0x51..0x7F` and escaped `0x80 <value:u32le>`. The escaped form is assigned only when its four-byte value leaves the payload's final `FE`; otherwise `0x80` is the compact atom zero. A fixed-width reference is `0x32 <entity_id:u32le>` under the same final-terminator boundary; its four identity bytes are not independent atoms. `E5 <length:u32le> <bytes[length]>` is a blob only when its declared extent leaves the payload's final `FE`. An extent landing at the payload end consumes the required terminator and invalidates the graph; an extent beyond the payload boundary leaves `E5` as an atom. Assigned E5 blobs inside `7C0A` are templated descriptor records (≈59 or 46 bytes), not NURBS payloads. The class30 pair records and `76 ac 7f`-delimited handle table are coedge/half-edge sub-tables, not the port→vertex relation.
