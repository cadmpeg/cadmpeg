...
A class-`25` scalar definition begins `82 <allocation operand> <allocation operand> <persistent operand>`. An allocation operand uses the compact grammar, `06 <u8>`, `0a <u16le>`, or one raw nonzero byte congruent to 2 or 3 modulo 4. The persistent operand uses the compact grammar, `0a <u16le>`, or `0b <u16le>`; the two explicit leads remain distinct encodings of the same numeric lane. The remaining payload is either 7, 8, 9, or 10 finite `f64le` scalars, or five finite `f64le` scalars followed by marker `82` and 5–7 finite `f64le` scalars, or marker `83` and 8–9 finite `f64le` scalars. The marker is a scalar-lane boundary, not part of either adjacent scalar.
