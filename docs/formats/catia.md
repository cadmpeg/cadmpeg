...
Coordinate binding is one joint constraint problem over the complete quotient. Assign one distinct coordinate row to every logical-vertex component, restrict each assignment to that component's intersected domain, and require every edge with an explicit unordered endpoint-pair domain to select one stored pair across its two assigned components. The coordinate binding is resolved only when this constrained bijection has exactly one solution; uniqueness of the independent component domains is insufficient.
