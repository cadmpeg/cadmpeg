...
- A `5e1a` edge-stride record contains the fixed tagged-one prefix `10 01 00 00 00` at `+7`, five nonzero tagged `u32le` allocation values `[T,X,Y,T−1,T−2]` at `+12,+17,+22,+27,+32`, and terminal byte `21` at `+37`. A `2569` header contains `10 01 00 00 00` at `+7`, count byte `82` at `+12`, and its two base columns as tagged `u32le` values at `+13,+18`. Each following `0638` record contains the same tagged-one prefix at `+7`, count byte `83` at `+12`, its side number at `+13`, and two tagged global-record references at `+18,+23`.
- Vertex-incidence families `050b`, `0510`, and `0515` contain the tagged-one prefix at `+7`, count bytes `82`, `83`, and `84` at `+12`, and respectively two, three, and four tagged global-record references from `+13` at stride 5. Each complete incidence record is immediately followed by its `5d06` vertex-owner record.
