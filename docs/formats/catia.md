...
- Packed loop sense orients each lifted endpoint pair. Consecutive oriented occurrences satisfy `end[j] = start[(j+1) mod n]` within `2e-3` mm. A single unlifted occurrence between two lifted occurrences has endpoints `[end[j-1], start[j+1]]`. Two occurrences are radial twins when their unordered endpoint pairs are uniquely equal within the same tolerance; ambiguous or unpaired endpoint groups do not form a physical edge.
