...
- A `5fxx` face record contains `10 01 00 00 00` at `+7`, count byte `0x80+n` at `+12`, `n` tagged `u32le` allocation values `[R0, R1, ..., Rm]` from `+13` at stride 5, and one terminal control byte. It defines ordered loop terminals `T[j] = R0 - R[j+1]`. The complete face roster aligns positionally with the complete surface-support-run roster. Concatenate loops in face-record order and each loop's members in serialized order. For an owned `21xx` support occurrence with local slot `s` at `+12`, its first-lane loop member is `A = T - s`. `A` identifies a face-local support occurrence rather than a global `0638` identifier.
