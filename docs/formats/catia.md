...
Resolve every in-range selector to the exact entry in the block's source schema and preserve both the entry identity and its UTF-8 string value. A selector begins one stored value; its complete encoded value is every following token before the next selector or the end of the payload. The first token is the value's primary typed token. Preserve selector order and payload offset. Preserve the absent-schema sentinel as a selector without an entry link, name, or value assignment.
