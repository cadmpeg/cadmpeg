...
Same-incidence spline rows with the same complete bipartite endpoint relation bind by allocation rank: partition the relation into its two vertex-row sets and order each set by vertex allocation. When both partition cardinalities equal the row count, equal ranks pair with rows in serialized edge order. When exactly one partition cardinality equals the row count, equal ranks bind that endpoint and the opposite endpoint retains its complete partition domain. If an earlier constraint has already bound distinct row-ranked endpoints, identical complete opposite domains of row cardinality bind their equal ranks. Singleton relations already fixed by an adjacent edge on either shared face are excluded before testing completeness. Circle rows with one identical carrier and one identical endpoint relation bind the relation's lexicographically ordered pairs by equal rank when their cardinalities match. Rows with the same two incident faces use serialized edge order. Rows with exactly one common face and distinct partner faces use partner-face allocation order. Each rank-binding stage consumes the endpoint domains fixed by preceding stages. An incomplete relation, repeated partner face, or cardinality mismatch establishes no binding.
...
- `b5 03 30` (offset surface): payload `82 result_carrier_ref source_surface_ref <distance:f64le> <carrier_kind:u8> <u0:f64le> <u1:f64le> <v0:f64le> <v1:f64le>`. The bounds are strictly increasing. `carrier_kind` is `15` for a plane result carrier, `05` for a cylinder, `09` for a sphere, `0d` for a torus, `19` for an `a8 03 32` rolling-ball result carrier, and `01` for a class-`31` result cache. An analytic result and source form an offset only when they have the same family and their geometry agrees with the stored distance magnitude: parallel planes have that normal separation; cylinders are coaxial with that radius difference; spheres are concentric with that radius difference; and tori are concentric and coaxial with equal major radii and that minor-radius difference. The first reference supplies the result geometry and parameter chart; the second reference and signed distance define the exact offset construction.
...
An immediately preceding `<inclusive-length:u8> <name:utf8> <selector>` binds a schema role to that text field when `inclusive-length` equals one plus the nonzero name-byte length, `name` begins with an alphabetic character or underscore and otherwise contains only alphanumeric characters or underscores, and `selector` is either `80 <identity:u32le>` or `<page:d1..e4> <low:u8>`. The paged identity is `(page−d1)·256+low+1`. Preserve the role-name offset, name, and selector identity. Bytes between the selector and text opener, a zero selector identity, or a role name outside this identifier grammar prevent the binding.

An identity interval containing exactly two schema text fields is a typed relation when the second field has the complete form `(<clause>{,<clause>}) : <result-type>` with unique parameter names. A clause is `<parameter> : #In <value-type>` or `<parameter> : #Out <value-type>`. At most one output clause is present. `VoidType` requires one output clause; every other result type requires none. The first field is the exact expression or rule program. Preserve both field offsets and values, ordered inputs, optional output, result type, the first field's `body` selector when present, and the second field's `param` selector when present. When the `body` selector equals the containing identity and the `param` selector selects an identity in the same run, that selected identity is the relation's parameter identity. A different role name, a non-self `body` selector, or a `param` selector outside the run leaves the parameter identity unresolved without discarding either selector.
...
A legacy scalar prefix is `FE <form> 88 82 FE`, where `form=84` is the named form and `form=85` is the standalone form. `E6 <bits:u64le>` follows it for a finite binary64 evaluation; `E7` follows it for an unset evaluation. The complete opcode and payload must remain inside one identity interval. Preserve the prefix offset, form, containing identity, evaluation state, exact binary64 bits, and a stable scalar identity formed from the run ordinal and prefix offset. When an identity interval contains exactly one scalar packet and exactly one role-bound text field named `name`, that field supplies the scalar's stored name; preserve the name-field offset and text. Resolve its declared type from the identity's unique type descriptor: a literal terminates the resolution, while a selector continues at the selected identity's unique descriptor. A repeated identity or an identity with zero or multiple descriptors leaves the type unresolved. A named-form scalar with this unique name and a resolved terminal type of `LENGTH`, `ANGLE`, `Real`, `R`, `Integer`, or `I` is a typed parameter. `E7` produces an unset parameter with no evaluated value. For `E6`, `LENGTH` values are millimetres, `ANGLE` values are radians, `Real` and `R` are dimensionless, and `Integer` and `I` require an exact signed-64-bit integral binary64 value. Standalone forms, unresolved descriptor chains, other literal types, nonintegral integer values, and out-of-range integer values do not form a neutral parameter.
...
When the entity's paired `7C09` field has an owner identity, its definition-bound value belongs to that owner-grouped design object. Preserve these entity identities in the serialized order of their paired fields. An unowned field retains its definition-bound value without assigning it to a design object.
...
A relation-expression entity has two equal definition selectors naming `body`. The six-selector placeholder-state framing orders local placeholder, source expression, `param`, source type signature, `opened`, `RelationExpFct`. The six-selector Boolean parser-version framing orders `Boolean`, source expression, `ParserVersion`, `param`, source type signature, `RelationExpFct`. The five-selector parser-version framing orders source expression, `ParserVersion`, `param`, source type signature, `RelationExpFct`. The seven-selector opened Boolean parser-version framing orders `Boolean`, source expression, `ParserVersion`, `param`, source type signature, `opened`, `RelationExpFct`. Preserve the selected entry identity and UTF-8 value for every role. A missing, additional, or reordered selector, unequal definition selector, or different fixed role name is not a relation-expression entity. The typed signature form is `() : <result-type> [LF]` or `(<parameter> : <space*> #In <space*> <input-type> [,<parameter> : <space*> #In <space*> <input-type>]*) : <result-type> [LF]`. Preserve inputs in order after excluding surrounding whitespace from each parameter and type. Parameters are nonempty and distinct and input and result types are nonempty. In placeholder-state framing, a nonempty input list begins with the trimmed local placeholder and an empty input list requires an empty trimmed local placeholder. Other signature strings remain preserved without typed roles.

A formula relation has two equal definition selectors naming `Formula`. Its paired object payload is the exact field sequence atom `249`, atom `4`, formula-entity reference, relation-expression-entity reference, parameter-entity reference, atom `129`, terminator. The first reference equals the formula entity identity. The second resolves within the same graph to an entity carrying a complete relation-expression program. The third is the stored parameter identity and need not resolve within that graph. A relation source symbol is either `#<digits>_` or `#<digits>_<ASCII-whitespace*>/<digits>`. Each occurrence selects the named parameter-value entity in the same graph whose binding equals the complete symbol. Preserve the occurrence text and selected entity in source order only when that binding is unique; an absent or repeated binding contributes no dependency link.

A lead-`2` named parameter-value entity has exactly two value selectors, naming the parameter and its scope, expression, or presentation binding. Its record suffix is either `85 96 82 6A E7 81 52` for an unset evaluation or `85 96 82 6A E6 <value:f64le> 81 52` for one finite evaluated scalar. Preserve both selected entry identities and values and the exact scalar bits. Other leads, selector cardinalities, suffixes, and non-finite scalars are not named parameter values.

A lead-`2` constraint-range entity has exactly two value selectors. The first names `Range`. The second and the record-suffix prefix code form one of three exact pairs: `CstAttr_Dimension` with `B8`, `CstAttr_Dimension` with `C1`, or `ComplexCst` with `C9`. The suffix prefix atoms are `84 96 82`, the payload is direct `E7` unset or `E6 <value:f64le>` finite scalar, and no trailer follows. Preserve both selected entry identities and values, the exact framing pair, and the evaluation bits. Other selector names, role/code pairs, suffix framings, and non-finite scalars are not constraint-range values.

A formula assigns each input and result scalar semantics from its typed relation signature. `LENGTH` values are millimetres, `ANGLE` values are radians, `Real`/`R` values are dimensionless binary64 scalars, and `Integer`/`I` values are exact integral binary64 scalars in the signed 64-bit range. A nonintegral or out-of-range scalar does not satisfy an integer signature. Every declared input occurs in the expression and resolves to one distinct named parameter identity. Repeated occurrences of one input name one dependency in first-occurrence order. A complete declared input set assigns those input types independently of whether the formula's result identity resolves. An `E7` input retains its typed parameter identity without an evaluated value and prevents formula evaluation; an `E7` result retains the typed expression and dependencies without an evaluated value. A parameter is a formula result only when exactly one formula relation selects its identity; multiple selecting relations are ambiguous even when their programs agree. An ambiguously defined parameter that is independently a complete input retains its stored scalar literal and input type without a defining expression. Conflicting input signatures for one identity do not establish an independent scalar fallback. When a parameter is both one formula's result and another formula's input, its defining expression and dependencies replace the generated scalar literal after the input type and evaluated value agree. A downstream input type that disagrees with that unique defining result invalidates the downstream formula without erasing the definition. A formula component is closed only when dependency expansion reaches scalar roots without a cycle.

A transferred named parameter belongs to a transferred feature when its entity's positionally paired object record belongs to that feature's design object. A transferred child feature whose structural owner is an earlier transferred feature and an owned parameter occur in that owner's source-content sequence at the child object's first field and the parameter's paired object field, respectively. Order the mixed sequence by those object-field byte offsets. An absent object record, an unowned object record, or a design object without a transferred feature leaves the parameter at document scope.

The transferable arithmetic expression grammar is finite decimal literals, `mm`, `cm`, `m`, `rad`, and `deg` unit suffixes separated from the literal by zero or more ASCII whitespace bytes, the dimensionless constants `PI` and `E`, resolved relation symbols, parentheses, `sin`, `cos`, `tan`, `asin`, `acos`, `atan`, `sinh`, `cosh`, `tanh`, `asinh`, `acosh`, `atanh`, `log`, `ln`, `exp`, `ceil`, `floor`, `int`, `round`, `mod`, `abs`, `sqrt`, `min`, `max`, and `LinearInterpolation` calls, right-associative `**` powers, unary `+` and `-`, products and quotients, then sums and differences in precedence order. Power binds more tightly than unary sign, so `-2**2` is `-(2**2)`; a signed exponent is permitted. Parentheses, function arguments, unary operators, and recursively nested powers share a maximum nesting depth of 128. A function accepts at most 128 comma-separated arguments. Length literals convert to millimetres; a `deg` literal converts to radians. `PI` is π and `E` is the base of the natural logarithm. Expression dimensions are signed 32-bit integer exponents of length and angle; multiplication adds exponents and division subtracts them with checked arithmetic. A power exponent must be dimensionless. A dimensionless base accepts any exponent whose result is finite. A dimensioned base requires an integral signed 32-bit exponent and multiplies both dimension exponents by it with checked arithmetic. Addition and subtraction require equal dimensions. `min` and `max` accept one or more equal-dimension arguments and retain that dimension. `sin`, `cos`, and `tan` accept an angle in radians or a dimensionless radian value and produce a dimensionless result. `asin` and `acos` require a dimensionless argument in `[-1,1]` and produce an angle; `atan` accepts any finite dimensionless argument and produces an angle. Hyperbolic and inverse-hyperbolic functions require a dimensionless argument and produce a dimensionless result; `acosh` requires an argument at least one and `atanh` requires an argument strictly between negative one and one. `log` and `ln` require one positive dimensionless argument; `log` computes the base-10 logarithm and `ln` computes the natural logarithm. `exp` requires a dimensionless argument. `ceil`, `floor`, `int`, and the one-argument `round` require a dimensionless argument and produce a dimensionless integral value. `int` truncates toward zero. `round` resolves a half-integer to the nearest even integer. `mod` requires a dimensionless dividend and a nonzero signed-64-bit integral divisor; it truncates the dividend toward zero and returns the division remainder as a dimensionless value. `abs` retains its argument dimension. `sqrt` requires a nonnegative argument with even length and angle exponents and halves both exponents. `LinearInterpolation(start,end,fraction)` requires exactly three dimensionless arguments and returns `start + (end - start) * fraction`. Division by zero, exponent overflow, non-finite evaluation, trailing syntax, unsupported units, unsupported function names, invalid arity, invalid function domains, and excess nesting or argument count do not form a transferable formula result. The evaluated dimension must equal the declared result type. An `E6` result must equal the evaluated binary64 value; an `E7` result requires the same syntactic and dimensional closure but has no stored value to compare.

`<prefix0:compact_atom> <prefix1:compact_atom> E8 <type-selector:u16le> 37 <layout:one_byte_atom> <value:one_byte_atom> <item+> FE+` is an embedded numeric value packet. An item is `E6 <bits:u64le>` or one zero-payload control byte in `E7..E9`, and at least one item is binary64. The first prefix atom begins at a lossless value-field boundary. The second prefix atom and `E8` are either separate compact-atom and opcode tokens or the combined `87 E8` marker token. Marker-shaped bytes inside another typed field do not begin a packet. Preserve the packet offset, prefix atoms, fixed-width little-endian selector, layout and value atoms, ordered items with payload offsets, and terminator count. Overlapping candidate packets are not assigned. A production that consumes the complete payload and has at least two terminators is assigned only as the complete numeric-tuple production.
...
The fixed bytes in the inline production are structural and the complete inline body is preserved. A `7C08`, `7C0B`, or `7C02` candidate contained by another complete root extent is payload data, not an independent graph, value block, or catalog. A nested payload begins at the unique `7C0A` frame whose stored total length lands exactly on the enclosing `7C09` end and ends with an unconsumed `FE+` terminator run. Preserve every terminator in the run. A non-`FE` byte after the run and a length-framed field that consumes every terminator invalidate the graph. Literal `7C0A` bytes in the variable head do not delimit the payload, and multiple length-closing candidates invalidate the graph. A childless record that does not consume the complete inline production invalidates the graph. It is the **feature/object-ownership layer**, not the expanded face→loop→coedge table or the port→vertex collapse. A separator-form `7C09` head is `<lead> 01 <owner_ref> <class_ref> [storage_ref]`. Compact heads encode their roles in the lead: `02 <owner_ref>`, `12 <owner_ref> <class_ref>`, `16 <class_ref> <storage_ref> <owner_ref>`, and `52 <owner_ref> <class_ref> <storage_ref>`. An extended owner-only compact head is `12 <nonzero owner_ref> 80 <suffix> 00 00`, where the decoded head contains six or seven tokens including the lead; `80` is the null class handle and the nonempty suffix remains unassigned. An extended class-storage-owner head retains the `16 <class_ref> 80 <nonzero owner_ref>` prefix before either `16|17 00 00 80 <reference> 00 00` or `00 00 80 <token> 1c 00 00`; the null storage handle and complete suffix remain distinct from the three assigned roles. Other separator-free leads do not assign reference-shaped tokens to these roles. Its ordinary references use `80..d0 → byte−80` and its paged references use `d1..e4 <byte> → (prefix−d1)·256+byte+1`. A paged prefix without its low byte and bytes outside those assigned forms remain literals; they do not create references. `owner_ref` is an entity identity in the paired `7C05` run. Class refs are zero-based per-file schema ordinals rather than entity identities or global type codes. `7C0A` atoms use the same compact and paged forms, plus raw `0x51..0x7F` and escaped `0x80 <value:u32le>`. The escaped form is assigned only when its four-byte value leaves the payload's final `FE`; otherwise `0x80` is the compact atom zero. A fixed-width reference is `0x32 <entity_id:u32le>` under the same final-terminator boundary; its four identity bytes are not independent atoms. The same direct fixed-width form denotes a reference when it occurs as a `3B` list item. After a reference tag `81`, both `80 <entity_id:u32le>` and `32 <entity_id:u32le>` are fixed-width reference values under that boundary; identity bytes are opaque to terminator recognition. `E5 <length:u32le> <bytes[length]>` is a blob only when its declared extent leaves at least one final `FE`. An extent landing at the payload end consumes the required terminator and invalidates the graph; an extent beyond the payload boundary leaves `E5` as an atom. Assigned E5 blobs inside `7C0A` are templated descriptor records (≈59 or 46 bytes), not NURBS payloads. The class30 pair records and `76 ac 7f`-delimited handle table are coedge/half-edge sub-tables, not the port→vertex relation.

All `7C09` records in one graph carrying the same `owner_ref` are fields of one serialized design object. Design objects are ordered by the byte offset of their first field in the graph; each retains that first-field offset and its zero-based position in this order. Preserve each group's field source order and class-specific storage forms. A `storage_ref` equal to an identity in the paired entity table selects that exact positionally paired field and its design object when the selected field has an owner. A storage identity absent from the entity table remains unresolved. Preserve the distinct resolved field-class entries and names in first field order as the object's field vocabulary. When `owner_ref` equals an identity in the paired entity table, it selects the positionally paired `7C09` record. The design object grouping that contains this selected record is the object's structural owner; a selected record contained by its own group contributes no owner link. In separator-form groups, retain that record's resolved class entry, class name, and `storage_ref` as the owner class and owner storage form. In compact groups, the selected record is an identity anchor rather than a class declaration; leave owner class and storage unset and use the group's ordered field-class vocabulary as its type evidence. An unresolved owner identity does not invalidate the field group. A schema class name labels its field record and is not by itself a neutral feature operation. Within a structurally owned design object, one or more owner-bound `PRTSketch` fields selecting the same catalog entry establish one planar sketch and its history identity. Repeated fields retain their distinct source identities but share the design object's sketch identity. A plain `Sketch` field is a property and does not establish a sketch identity. A declaration field is completely transferred only when it has no `storage_ref`, no references, and the one-byte payload `FE`; other declaration payloads remain unresolved without erasing the sketch identity. When both a sketch design object and an earlier structural owner establish sketches, the owner link is the child sketch feature's containing feature. One or more co-owned `xy-plane`, `yz-plane`, or `zx-plane` fields selecting one catalog entry, each with the same empty storage form, place that sketch on the corresponding origin plane. The ordered plane-name axes define `(normal,u)` as `xy=(+z,+x)`, `yz=(+x,+y)`, and `zx=(+y,+z)`. Distinct principal-plane names or entry identities leave placement unresolved. A structurally owned design object whose complete field set consists of one or more empty declarations selecting one principal-plane name and catalog entry is that standalone built-in reference plane. Transfer `xy-plane` as top, `yz-plane` as right, and `zx-plane` as front. A mixed field class, distinct catalog entry, storage selector, reference, or nonempty payload prevents the standalone plane transfer.

A design object whose every field is a complete empty declaration with a positionally paired complete definition-bound value and whose definitions all select the same admitted pattern catalog entry is one pattern feature. `CircPattern` establishes circular form; `RectPattern` establishes linear translation form. The declaration field classes are independent operation roles and need not equal the selected definition. These declaration records are consumed by the feature. This establishes the operation identity and form independently of unresolved seeds, directions, counts, spacing, angles, field roles, and outputs. A payload-bearing field, missing definition value, distinct selected entry, distinct selected name, or definition outside the admitted pattern set leaves the feature identity unresolved.
...
- A `5fxx` face record contains `10 01 00 00 00` at `+7`, count byte `0x80+n` at `+12`, `n` tagged `u32le` allocation values `[R0, R1, ..., Rm]` from `+13` at stride 5, and terminal control `03` or `05`. It defines ordered loop terminals `T[j] = R0 - R[j+1]`. The complete face roster aligns positionally with the complete surface-support-run roster. Concatenate loops in face-record order and each loop's members in serialized order. For an owned `21xx` support occurrence with nonzero local slot `s` at `+12`, its first-lane loop member is `A = T - s`. `A` identifies a face-local support occurrence rather than a global `0638` identifier.
...
- A `5e1a` edge-stride record contains the fixed tagged-one prefix `10 01 00 00 00` at `+7`, five nonzero tagged `u32le` allocation values `[T,X,Y,T−1,T−2]` at `+12,+17,+22,+27,+32`, and terminal byte `21` at `+37`. A `2569` header contains `10 01 00 00 00` at `+7`, count byte `82` at `+12`, and its two base columns as tagged `u32le` values at `+13,+18`. Each following `0638` record contains the same tagged-one prefix at `+7`, count byte `83` at `+12`, its side number at `+13`, and two nonzero tagged allocation values at `+18,+23`.
