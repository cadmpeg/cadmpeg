...
A relation-expression entity has two equal definition selectors naming `body` and exactly six value selectors in this order: local placeholder, source expression, `param`, source type signature, `opened`, `RelationExpFct`. Preserve the selected entry identity and UTF-8 value for every role. A missing, additional, or reordered selector, unequal definition selector, or different fixed role name is not a relation-expression entity. The typed signature form is `(<parameter> : <space*> #In <space*> <input-type>) : <result-type> [LF]`; surrounding whitespace is excluded from the three retained names, and `parameter` equals the local placeholder after trimming its surrounding whitespace. Other signature strings remain preserved without typed roles.
