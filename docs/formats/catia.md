...
For `b5 03 2a`, the 153-byte payload is `80 center:3f64le scaled_x:3f64le scaled_y:3f64le scaled_axis:3f64le radius:f64le tail:6f64le`. The radius is positive. Each scaled direction has length `radius`, and their normalized forms satisfy `direction_x × direction_y = axis`. The first two tail-scalar pairs are individually increasing. The exact sphere carrier has `center`, `radius`, polar `axis`, and zero-azimuth `direction_x`.
