...
- `b5 03 27/28/2d` analytic surfaces: plane origin+normal, cylinder origin+axis+radius, revolution profile reference+axis origin+direction+angular gauge. The profile reference uses the same positional-byte token forms as loop members; the axis fields immediately follow its variable-width token.
