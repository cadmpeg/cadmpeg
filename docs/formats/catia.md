...
A design object whose every field is a complete empty declaration with a positionally paired complete definition-bound value, whose field class equals that selected definition, and whose definitions all select the same admitted pattern catalog entry is one pattern feature. `CircPattern` establishes circular form; `RectPattern` establishes linear translation form. These declaration records are consumed by the feature. This establishes the operation identity and form independently of unresolved seeds, directions, counts, spacing, angles, field roles, and outputs. A payload-bearing field, field/definition class mismatch, missing definition value, distinct selected entry, distinct selected name, or definition outside the admitted pattern set leaves the feature identity unresolved.
