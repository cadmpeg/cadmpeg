...
- `b5 03 62` and wide-header `a8 03 62` (loop node): payload `<0x80 + n_refs> (pcurve_ref edge_ref)* surface_ref <tail>`, `n_refs` odd. Member ref tokens use a positional-byte mask: `08 <lo>`, `10 <mid>`, `18 <lo><mid>`, `20 <page>`, `28 <lo><page>`, `30 <mid><page>`, and `38 <lo><mid><page>`. Omitted id bytes are zero; present bytes occupy bits 0–7, 8–15, and 16–23 respectively. A single byte in `80..ff` names object id `byte - 80`. The tail begins with `<0x80 + edge_count>`; its remaining topology metadata does not alter member identity or order.
