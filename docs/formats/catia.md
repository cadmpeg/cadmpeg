...
- `b5 03 06` (parameter incidence): payload `<0x80 + count> curve_ref{count} <0x80 + count> (<parameter:f64le> 05){count}`. Each finite parameter is index-aligned with its curve reference. References to typed pcurves evaluate at their paired parameter and lift through the pcurve support to the vertex locus.
