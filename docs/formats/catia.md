...
An object graph is preceded by a contiguous entity-table run containing one `7C05` record for each serialized `7C09` record. A `7C05` record is `7C 05 <total_len:u32le> <lead:u8> 7C 06 <definition_len:u32le> <definition_prefix> EA <entity_id:u32le> <tail>`. Within `definition_prefix`, `32` begins a fixed-width five-byte atom, so an `EA` among its four value bytes is not the identity delimiter. The tail contains a length-framed `7C07` record. Entity identities are nonzero and strictly increase across a run but may be sparse. One `DE` byte separates the run from its paired `7C08` marker, and the run has the same record count as that graph. Pair `7C05` and `7C09` records by serialized position. Preserve the complete definition prefix and tail.
