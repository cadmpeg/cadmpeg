...
Resolve every in-range selector to the exact entry in the block's source schema and preserve both the entry identity and its UTF-8 string value. An in-range selector begins one stored value; its complete encoded value is every following token before the next in-range selector, absent-schema sentinel, or end of the payload. The first encoded token is the value's primary typed token. Consecutive delimiting selectors give the first selector an empty encoded value and no primary token. Preserve selector order and payload offset. Preserve the absent-schema sentinel as a selector without an entry link, name, or value assignment.
