...
A relation-expression entity has two equal definition selectors naming `body` and exactly six value selectors in this order: local placeholder, source expression, `param`, source type signature, `opened`, `RelationExpFct`. Preserve the selected entry identity and UTF-8 value for every role. A missing, additional, or reordered selector, unequal definition selector, or different fixed role name is not a relation-expression entity. The typed signature form is `(<parameter> : <space*> #In <space*> <input-type> [,<parameter> : <space*> #In <space*> <input-type>]*) : <result-type> [LF]`. Preserve inputs in order after excluding surrounding whitespace from each parameter and type. Parameters are nonempty and distinct, input and result types are nonempty, and the first parameter equals the trimmed local placeholder. Other signature strings remain preserved without typed roles.
