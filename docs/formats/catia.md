...
A complete entity-record suffix value is `<prefix-atom0:one_byte_atom> <prefix-atom1:one_byte_atom> <prefix-atom2:one_byte_atom> <prefix-code:u8> E7 <trailer>` for an unset value or the same prefix followed by `E6 <bits:u64le> <trailer>` for a finite binary64 scalar. The trailer is empty, exactly two bytes, or the fixed zero frame `FE F6 00{16}`. The production is assigned only when it consumes the complete suffix. Preserve the three decoded prefix atoms, exact prefix code, evaluation, and trailer independently of their field-specific semantic roles. A suffix with another prefix or trailing form remains outside this production.
