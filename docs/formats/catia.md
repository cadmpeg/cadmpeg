...
A lead-`2` entity with exactly one catalog-resolved definition selector, a `7C07` payload consisting only of its terminator, and a complete suffix evaluation binds that definition entry to the evaluation. Additional or unresolved definition selectors, any other `7C07` field, and non-evaluation suffix payloads do not form a definition-bound evaluation.
