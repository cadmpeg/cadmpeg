...
- A `5e1a` edge-stride record contains six `10 <u32le>` global-record references at `+7,+12,+17,+22,+27,+32` and terminates with `21` at `+37`. References two and three are the edge's two adjacent support records. Both must select `21xx`, `276a`, `288a`, `29b8`, `2bc8`, `34c8`, or `345e` records; otherwise the adjacent-support pair is unresolved atomically. A `2569` header contains `10 01 00 00 00` at `+7`, count byte `82` at `+12`, and its two base columns as tagged `u32le` values at `+13,+18`. Each following `0638` record contains the same tagged-one prefix at `+7`, count byte `83` at `+12`, its side number at `+13`, and two tagged global-record references at `+18,+23`.
...
- Packed loop sense orients each lifted endpoint pair. A complete oriented endpoint tape exists only when consecutive occurrences satisfy `end[j] = start[(j+1) mod n]` within `2e-3` mm. A single unlifted occurrence between two lifted occurrences has endpoints `[end[j-1], start[j+1]]`; two or more unlifted occurrences leave the tape absent. Two occurrences are radial twins when their unordered endpoint pairs are uniquely equal within the same tolerance. Unique radial pairs establish connected face-incidence components. A coincident endpoint group partitions by those components and forms one physical-edge candidate for each component containing exactly two occurrences. Other ambiguous or unpaired endpoint groups do not form a physical-edge candidate. Incidence components do not assert body or shell membership.
