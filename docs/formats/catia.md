...
Port-domain intersections preserve physical-edge pair correlation. For every edge with distinct endpoint ports and an explicit pair domain, at least one pair must place one member in each of the edge's two current port components. Equal endpoint ports encode a closed edge: both endpoint positions bind the same logical vertex, and its admissible domain consists of diagonal pairs `[v,v]`. Check this constraint after each complete boundary-cycle merge and on the complete quotient.

Coordinate binding is one joint constraint problem over the complete quotient. Assign one distinct coordinate row to every logical-vertex component, restrict each assignment to that component's intersected domain, and require every edge with an explicit unordered endpoint-pair domain to select one stored pair across its assigned components. A closed edge selects one diagonal pair. Coordinate rows with equal stored XYZ form one coordinate class whose capacity is its row population. The binding is resolved only when the capacity-constrained class assignment has exactly one solution; permutations among rows of the same class do not change vertex placement. Uniqueness of the independent component domains is insufficient.
