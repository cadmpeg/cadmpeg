...
A `7C0A` reference field and every reference item in a `0x3b` list select a one-based `7C09` record ordinal in the same graph. The selected record's `owner_ref` identifies the referenced design object. Resolve object dependencies in first reference order, collapse repeated references to the same owner, and omit self-references. References to records without a materialized owner group remain unresolved record links.
