...
- **`62xx` loop** has count byte `0x81+2e` at `+12`, followed by alternating tagged `u32le` lanes `A[0], Q[0], ..., A[e-1], Q[e-1], T`. The even lane satisfies `A[j] = T − g − j`. Each `Q[j]` is a one-based global-record reference. The trailer is `0x80+e`, one loop-class byte, `ceil(3e/8)` packed sense bytes, then `01`. Loop class `0x50` is inner (hole); `0x41`/`0xc1` is non-inner. The `0x50` count equals the hole count. The outer loop is first, followed by inner loops in ascending terminal-id order.
- A `5fxx` face record contains `10 01 00 00 00` at `+7`, count byte `0x80+n` at `+12`, `n` tagged `u32le` allocation values `[R0, R1, ..., Rm]` from `+13` at stride 5, and terminal control `03` or `05`. It defines ordered loop terminals `T[j] = R0 - R[j+1]`. The complete face roster aligns positionally with the complete surface-support-run roster. Concatenate loops in face-record order and each loop's members in serialized order. For an owned `21xx` support occurrence with local slot `s` at `+12`, its first-lane loop member is `A = T - s`. `A` identifies a face-local support occurrence rather than a global `0638` identifier.
