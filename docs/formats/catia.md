...
A relation-expression entity has two equal definition selectors naming `body`. The six-selector placeholder-state framing orders local placeholder, source expression, `param`, source type signature, `opened`, `RelationExpFct`. The six-selector Boolean parser-version framing orders `Boolean`, source expression, `ParserVersion`, `param`, source type signature, `RelationExpFct`. The five-selector parser-version framing orders source expression, `ParserVersion`, `param`, source type signature, `RelationExpFct`. Preserve the selected entry identity and UTF-8 value for every role. A missing, additional, or reordered selector, unequal definition selector, or different fixed role name is not a relation-expression entity. The typed signature form is `() : <result-type> [LF]` or `(<parameter> : <space*> #In <space*> <input-type> [,<parameter> : <space*> #In <space*> <input-type>]*) : <result-type> [LF]`. Preserve inputs in order after excluding surrounding whitespace from each parameter and type. Parameters are nonempty and distinct and input and result types are nonempty. In placeholder-state framing, a nonempty input list begins with the trimmed local placeholder and an empty input list requires an empty trimmed local placeholder. Other signature strings remain preserved without typed roles.
