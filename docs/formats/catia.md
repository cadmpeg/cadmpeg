...
A complete entity-record suffix value is `<prefix:bytes[4]> E7 <trailer>` for an unset value or `<prefix:bytes[4]> E6 <bits:u64le> <trailer>` for a finite binary64 scalar. The trailer is empty, exactly two bytes, or the fixed zero frame `FE F6 00{16}`. The production is assigned only when it consumes the complete suffix. Preserve the exact prefix, evaluation, and trailer independently of their field-specific semantic roles. A suffix with another trailing form remains outside this production.
