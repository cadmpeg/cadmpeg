...
- **`b2/b3/b4 03 18` parameter-space record:** header token `5`, then payload prefix `05 <control:u8>`. Payload length `0x12` stores `(u,v):2×f64le`, `0x1a` stores `(station,u,v):3×f64le`, and `0x2a` stores five unsplit `f64le` values. Every scalar is finite.
...
- A `5e1a` edge-stride record contains six `10 <u32le>` global-record references at `+7,+12,+17,+22,+27,+32` and terminates with `21` at `+37`. A `2569` header contains `10 01 00 00 00` at `+7`, count byte `82` at `+12`, and its two base columns as tagged `u32le` values at `+13,+18`. Each following `0638` record contains the same tagged-one prefix at `+7`, count byte `83` at `+12`, its side number at `+13`, and two tagged global-record references at `+18,+23`.
- Vertex-incidence families `050b`, `0510`, and `0515` contain the tagged-one prefix at `+7`, count bytes `82`, `83`, and `84` at `+12`, and respectively two, three, and four tagged global-record references from `+13` at stride 5.
