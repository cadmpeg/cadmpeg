...
- `b5 03 37` (support-bound surface construction): payload `85 result_carrier_ref support0_ref support1_ref pcurve0_ref pcurve1_ref <control0:u8> <control1:u8> <radius:f64le> <control2:u8> <control3:u8> <zero:f64le> <control4:u8> <control5:u8>`. The radius is positive and the second scalar is zero. Each pcurve begins with the reference of its index-aligned support. The first reference supplies the exact result geometry and parameter chart independently of support-carrier resolution. For a cylinder result the radius equals its analytic radius; for a torus result it equals the minor radius. An `a8 03 32` result reference supplies the complete rolling-ball jet and its persistent carrier identity.
