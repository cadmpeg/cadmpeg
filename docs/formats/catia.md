...
An object whose structurally selected owner record is paired with a complete numeric-tuple entity containing exactly two finite binary64 items carries one planar point coordinate. A relation occurrence from a field class named `2DPoint` binds that point to the target design object when the target contains a class field named `PRTSketch` whose payload is terminator-only, whose head has no storage selector, and whose payload has no references. The point has one sketch membership only when all qualifying relation occurrences select one target design object. A point coordinate field is semantically closed when its complete payload is one standalone reference to that sketch followed by the terminator; additional fields retain additional unresolved semantics.
