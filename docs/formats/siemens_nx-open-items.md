...
- The relation between graph-only `OFFSET_SURF` constructions and FACE-owned NURBS carriers remains unspecified for varying-weight rational supports, periodic supports, differing spline bases, and nonplanar supports whose derivative control nets do not certify one regular orientation. Common signed distance and partition identity do not establish that the NURBS faces are solved caches of those constructions.
