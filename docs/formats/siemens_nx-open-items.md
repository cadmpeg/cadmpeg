...
- The relation between graph-only `OFFSET_SURF` constructions and FACE-owned NURBS carriers remains unspecified for differing spline bases and nonplanar supports whose rational derivative bounds do not certify one regular orientation within the subdivision limit. Common signed distance and partition identity do not establish that the NURBS faces are solved caches of those constructions.
