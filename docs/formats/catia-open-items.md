...
- The value production and entity binding for typed `Boolean` parameters and active configuration state are unresolved. Exact `Configuration` records and `configrow` successor links establish native incidence only. They do not assign a configuration name, active state, body membership, parameter override, or replay order. Boolean- and activity-named field classes include compound object payloads and do not identify scalar values.
