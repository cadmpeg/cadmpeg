...
- The binding from compound relation-program instances to ordered inputs and `paramout` parameter entities is unresolved. The exact program-identity slot establishes incidence with its selected entity but does not assign the other payload identities to operand or output roles.
