...
- The semantic incidence from complete `Range`/`CstAttr_Dimension` and `Range`/`ComplexCst` values to individual constraints is unresolved. Exact incoming payload-reference occurrences are retained, but `ListAggregator` references mix these identities with unrelated graph members and repeat identities outside the counted list, so they do not establish a constraint owner or operand relation.
