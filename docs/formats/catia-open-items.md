...
- The binding from compound relation-program instances to ordered inputs and `paramout` parameter entities is unresolved. The exact program slot, repeated atom/reference identity, compact-form `ref(h)` context identity and selected class, and separator-form trailing entity identity and selected class establish incidence but do not assign operand or output roles.
