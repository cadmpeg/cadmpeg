...
- The binding from compound relation-program frame slots to `paramout` result entities is unresolved. A selected typed relation expression and its uniquely bound source symbols establish complete ordered inputs independently; the exact program slot, repeated atom/reference identity, compact-form `ref(h)` context identity, separator-form trailing identity, and every selected entity class do not assign any frame reference as an operand or output.
...
- The entity binding for evaluated typed `String` relation inputs and results is unresolved. Complete compound relation-program inputs can select unset named parameters, but schema-catalog type signatures do not identify the evaluated string-value packets used by a relation.
- The value production and entity binding for typed `Boolean` parameters and active configuration state are unresolved. Exact `Configuration` records select one paired value-schema entry and retain one independent entity incidence; complete `configrow` successor chains establish row order, and source-ordered chains retain the entities in each open row-to-successor identity interval. The semantic role of the selected schema, entity, and intervening records remains unresolved. They do not assign a configuration name, active state, body membership, parameter override, or feature replay order. Boolean- and activity-named field classes include compound object payloads and do not identify scalar values.
...
- The semantic incidence from complete `Range`/`CstAttr_Dimension` and `Range`/`ComplexCst` values to individual constraints is unresolved. Exact incoming payload references and object-head storage selectors are retained as distinct structural incidences, but `ListAggregator` references mix these identities with unrelated graph members and repeat identities outside the counted list, so neither incidence form establishes a constraint owner or operand relation.
