...
- The value production and entity binding for typed `Boolean` parameters and active configuration state are unresolved. Exact `Configuration` records select one paired value-schema entry and retain one independent entity incidence; complete `configrow` successor chains establish row order. The semantic role of the selected schema and entity remains unresolved. They do not assign a configuration name, active state, body membership, parameter override, or feature replay order. Boolean- and activity-named field classes include compound object payloads and do not identify scalar values.
