...
- The value production and entity binding for typed `Boolean` parameters and active configuration state are unresolved. Exact `Configuration` records and complete `configrow` successor chains establish native incidence and row order only. They do not assign a configuration name, active state, body membership, parameter override, or feature replay order. Boolean- and activity-named field classes include compound object payloads and do not identify scalar values.
