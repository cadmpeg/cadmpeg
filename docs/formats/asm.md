...
**Construction-cache ownership**: the outer non-`ref` procedural subtype owns the record's solved curve or surface cache. A B-spline block inside a subtype nested by that construction belongs to the nested support, source, guide, or child field and is not a candidate for the outer construction's cache. The family layout fixes the owning block's ordinal among the B-spline blocks directly owned by the construction. Cache editing uses that family ordinal at the stream's integer width; nested B-spline blocks and blocks that decode only at another width are not writable carriers of the outer construction.

**`helix_int_cur`**: the current form starts with a tagged integer ASM release word; the earlier form omits it. The remaining payload is a finite angle interval, axis-start position, major-radius vector, minor-radius vector, pitch vector, apex-factor double, and unit axis vector, optionally followed by the solved curve cache and its fit tolerance. The current form encodes the axis start as a position token and the next three triples as vector tokens; the earlier form encodes all four as position tokens. Position and radius-vector components and the cache fit tolerance are lengths. The major and minor vectors have equal magnitude. Their orientation about the axis records handedness; the pitch vector records axial rise per revolution, and the apex factor records linear radial growth per revolution fraction. Without the solved cache, this complete construction is the exact curve carrier. For parameter `t` in the stored interval `[t0,t1]`, let `q=(t-t0)/(2π)` and `s=1+apex_factor·q`; the curve is `C(t)=center+s·(major·cos(t)+minor·sin(t))+pitch·q`.

For reversal, let `h=(t1-t0)/(2π)` and `k=1+apex_factor·h`. A reversed construction has interval `[-t1,-t0]`, center `center+pitch·h`, major vector `k·major`, minor vector `-k·minor`, pitch vector `-pitch`, and apex factor `-apex_factor/k`. The axis is unchanged. These fields give `C'(-t)=C(t)`. A zero `k` gives a zero radius at the new interval start and cannot form a nondegenerate reversed helix carrier.
...
**`sweep_spl_sur` law-driven layout**: the explicit profile and frame prefix is followed directly by a recursive law instead of a branch integer. An integer, two-scalar interval, vector, integer, boolean, path curve, two-scalar interval, scalar, and boolean precede a second recursive law. A final integer, recursive formula, and boolean precede the common solved-surface cache and discontinuity tail. The text-law form stores each of the two law slots as one string token instead of a recursive law tree; the token text is preserved exactly and its internal expression is not re-tokenized. For a straight path `spine(v) = q + v * d` with an identity rail and scale vector `s(v)`, the section evaluates as `S(u,v) = F + s(v) ⊙ (p(u) − F) + v * d + law1(v) * (t_hat(u) × d_hat)`, where `F` is the profile-frame point or the sweep-frame origin when no profile frame is stored, `⊙` is component-wise multiplication, `p` is the profile, `q` is the path's stored point, `d` is its direction, and `t_hat` and `d_hat` are unit tangents. A unit scale vector gives the unscaled section. The stored path point anchors the path carrier and does not add an absolute section translation. A rail formula `DOMAIN(VEC(1,0,0),a,b)` with no binding is the identity rail. A `ROTATE(DOMAIN(VEC(1,0,0),a,b),TRANS1)` formula with its vector transform binding uses the first three binding vectors as the columns of a proper rigid linear basis; the fourth vector is zero, the binding scale is one, and the binding flags are `[true,false,false]`. The profile-frame vector parallel to the path tangent preserves the profile parameter direction; an antiparallel vector reverses it, with the stored profile interval mapped affinely onto the profile curve's native interval. In the revision-gated form, tail form `0` selects the stored solved surface cache, including its parameterization and derivatives; tail form `2` selects cacheless construction evaluation. A form-`2` record never consumes a retained form-`0` cache when its construction is unsupported. Native generation retains both law carriers, the formula, both curves, and the complete construction graph.
...
**`defm_spl_sur` / `defmsur`, revision `22506`, mode 3**: the revision integer follows the subtype name. The support surface then carries four optional U/V range fields, each encoded as a presence boolean and, when present, a double. Mode `3` stores four vectors, a scalar, three booleans, a model-space point, two vectors, a scalar, two booleans, three scalars, five booleans, a scalar, and a trailing long. The shared revision-gated surface tail follows this mode payload: its approximation-cache enum, selected cache or parameterization, six discontinuity arrays, and terminating boolean. This revision form has no additional carrier-specific or post-tail fields. Native generation retains the revision, support ranges, mode payload, and shared tail.
...
A procedural spline construction and its directly owned solved NURBS cache use the same U/V chart. A pcurve in a support slot therefore attaches to that cache without a fitted or derived chart transform. The cache is the final surface block directly owned by the construction scope; surface blocks in nested directrix, support, source, guide, or child scopes do not carry the construction chart. When the cache closure enum is `periodic` (`2`) in one direction, coordinates outside that direction's knot domain are reduced modulo the positive knot-domain span before cache evaluation. A nonperiodic support relation does not require evaluation outside its knot domain.
...
For a complete two-support constant-radius circular construction, `slice(v)` is the section center. Each side's primary pcurve evaluated at `v` in that side's support chart is the corresponding contact point. The surface section is the minor circular arc of the absolute radius from the first contact at `u = 0` to the second contact at `u = 1`. The slice, both pcurves, and the surface share `v`; differentiating their stored curves gives the section's exact first partials. Tail form `0` evaluates the stored solved surface cache, including its first derivatives and oriented normal. Tail form `2` evaluates the construction graph and does not consume a retained cache when the cacheless construction is unsupported.
