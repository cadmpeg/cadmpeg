...
- CLI command reports use `schema_version` 6: top-level `status` is `ok` or `refused`, and `refusal` is either null or `{ stage, code, message }` from `ConversionRefusal` (`stage` ∈ plan|decode|validate|export). Independent of `CadIr.ir_version` and of decode-sidecar `version`.
