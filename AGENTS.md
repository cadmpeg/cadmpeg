...
- Do not add `[build] target-dir` to the worktree's `.cargo/config.toml`. The repository tracks that file, so the change shows as a modified tracked file in every `git status`. Export `CARGO_TARGET_DIR` from the shell you also commit from; `git` gives the variable to the pre-commit hook, so the gate reuses the same cache.
