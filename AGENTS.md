...
- Project report and CADIR JSON with `cadmpeg query {summary,coverage,findings,losses,counts} FILE` (alias `arenas`; `-` reads stdin) instead of exploratory `jq`: it detects the artifact kind and prints tab-separated rows. The raw paths, when `jq` is unavoidable: native arenas under `.native.<codec>.arenas`, findings under `.validation_report.findings`, losses under `.validation_report.losses` or `.decode_report.losses`, coverage under `.decode_report.coverage` (absent when empty). The printed error count and exit status 1 count the `error` and `blocking` findings.
